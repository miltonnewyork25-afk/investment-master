...
```mermaid
flowchart TD
    A[MSCI 三层价值引擎] --> B[增长层 证据中高]
    A --> C[利润层 证据中]
    A --> D[扩张层 证据中低]
    B --> B1[PCS run-rate +16%]
    B --> B2[PCS new sales +44%]
    B --> B3[PA 整体利润 -220bp]
    B --> B4[非 PCS PA 反推 0至-5%]
    C --> C1[Incremental 75% 强信号]
    C --> C2[绝对 59.3% 未突破]
    C --> C3[Segment: Index 75.6%]
    D --> D1[M&A/Buyback 10% 单季]
    D --> D2[3 起并购: Vantager/Compass/PM Insights]
    D --> D3[再平衡信号, 非 pivot]
    B4 --> E[三段 SOTP 加总]
    C3 --> E
    D3 --> E
...
```mermaid
flowchart LR
    BR[BlackRock] --> D[Data: Preqin]
    BR --> W[Workflow: eFront + Aladdin]
    BR --> I[Index: 路线图]
    BR --> X[Distribution: iShares + 机构网络]
    D --> M1[MSCI PCS]
    W --> M2[MSCI portfolio tools]
    I --> M3[MSCI 私募 Index]
    X --> M4[MSCI 最大客户]
```
