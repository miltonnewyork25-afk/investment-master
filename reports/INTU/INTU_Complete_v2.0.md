...
> **版本**: v2.0 | **日期**: 2026-03-24 | **框架**: v19.7
> **评级**: 中性关注(偏积极) | **期望回报**: +5-8% | **置信度**: 52%
> **股价**: $457 | **市值**: $127B | **SOTP**: $490 | **PW EV**: $480
> **WACC**: 10.0% (CAPM推导, 68% CI: 9.5-10.5%)
...
**三个看空理由(限制上行的因素)** [v3.0调整: 按业务影响排序, SBC降级]:

1. **AI颠覆时间高度不确定** [v3.0: 从#3升至#1] — P4红队分析(RT-4)的"免费AI报税"情景不是一个概率问题，而是一个时间问题。IRS Direct File已经存在，AI辅助报税的能力每年在提升。问题在于: 我们无法确定TurboTax从"必需品"变为"可选品"的时间线——可能是3年(如果GPT-5级模型+政策推动)，也可能是10年(如果监管保守+用户惯性)。这种时间不确定性本身就会压制估值倍数，因为理性投资者不会为一个不确定何时到期的看涨期权支付太高溢价。
...
3. **SBC增速快于收入增速** [v3.0: 从#1降至#3, 重新定位为成本效率问题] — FY2025 SBC $2.0B(占收入10.4%), 回购/SBC=1.41x[DM-CF-005]。管理层用回购覆盖了SBC稀释(净缩股-0.35%/年), 但SBC年增15-18%快于收入增速16%→剪刀差在扩大。这是成本效率问题而非估值核心——INTU的SBC/Rev(10.4%)远低于CRWD(23%)/DDOG(22%)/WDAY(17%), 且回购offset率141%是SaaS行业最高之一。真正的风险不是SBC本身, 而是上述业务变量(AI颠覆/Mailchimp)是否会导致增速放缓→回购资金减少→SBC覆盖率下降的连锁反应。
...
3. **IES ARR增速<10% + CK增速<15%同时持续2季** [v3.0修改: SBC条件→业务条件] — 两个增长引擎同时熄火意味着"平台转型"论点失败, INTU回退为纯税务软件估值(PE 15-16x vs 当前18x)
...
## Appendix B: DM锚点注册表 + Kill Switch扩展 + CI深化
