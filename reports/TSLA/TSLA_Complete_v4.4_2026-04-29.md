# Tesla (TSLA) v4.4 — 隐含估值法: Q1 2026

**版本**: v4.4 (彻底统一估值口径为价格倒推法 / 删除SOTP目标价痕迹 / 删除模拟大师quote / 修正数据冲突)
**日期**: 2026-04-29
**当前股价**: $378.67 | **市值**: ~$1,420B | **52周区间**: $270.78 - $498.83
**估值方法**: 价格倒推 (Reverse Engineering) — **唯一主方法**。SOTP/EV-OAB/PE 仅作 sanity check 交叉验证, 不作目标价。
...
| P(FSD全面成功) | **~35-40%** | 历史基准率: Tesla重大目标达成率30-50%; FSD"by 2021"已6年延期 |
...
**我们的判断**: 综合历史基准率 + Q1 2026硬数据, 我们认为 P(FSD全面成功) **≈ 25-35%**, 略低于市场隐含的35-40%。**因此维持"审慎关注 (临界, 高争议)" — 不给单点目标价, 不给加权目标**。
...
- v4.1 SOTP sanity check: ~$199 (含Auto/Capex调整)
- v4.2/v4.3 Energy修正: 中性Energy SOTP +$30B = +$8.5/share
- v4.4 SOTP sanity check: ~$210 (中性下) — **仅作 validity check, 不构成目标价, 主估值仍是 Reverse DCF 隐含假设**
...
| **隐含 P(FSD全面成功)** | **35-40%** | 历史基准率Tesla重大目标达成率30-50%; 略偏乐观 |
...
**END (v4.4)** — 报告字符: ~138K / 18章主文 + 9附录 / 326+ DM锚点 / 17 Mermaid图 / 因果密度 ~3.2/万字 / 评级: 审慎关注 (临界, 高争议)

**v4.4 核心修正 (基于用户反馈)**:
1. **估值方法彻底统一为 Reverse DCF 唯一主估值** — 删除所有 SOTP "目标价"/"加权目标"措辞, 不在执行摘要列per-share价格作核心结论
2. **删除主文模拟大师quote** — Ch14 圆桌讨论 + Ch15 圆桌异议简化为"投资风格分歧表 + P(FSD成功)映射"; 完整模拟移到附录I并标注"模拟视角, 不代表本人观点"
3. **数据冲突修正**:
   - Capex: "vs 2025 2.9x / vs 2024 2.2x / vs Q1 annualized 2.5x" 三口径并列, 删除单一"4x"模糊表达
   - 2026E FCF: 统一口径 -$2B 至 +$2B (旧 DM "-$10~15B" 标注废弃)
   - "7引擎全部成功"过度确定表达修正为 "对 FSD/Robotaxi/Optimus 给较高权重", 与 Reverse DCF 推出的 P(FSD成功)~35-40% 一致
4. **附录B/C SOTP 计算重新定位为 sanity check** — 标题加"(非估值结论, 仅 validity check)"
5. **结构重定位**: 主文报告 + 附录 sanity check + 附录I 模拟大师视角 — 三层分明

**保留的 v4.3 优势**:
- 价格倒推 (Reverse Engineering) 框架的所有9个子章节 (Ch9.1-9.9)
- 确定性分层 (已证明/高概率/可能/信仰) 4层
- FSD成败二叉树概率反推
- 共识估计隐含的假设
- 全部 Q1 2026 v4.2 数据修正 (Energy 39.5% / EV/OAB 三口径 / GAAP TTM PE 347x / FSD ARR 降温 / Services GM 9.2% / Owner Earnings 双口径 / Net cash $35.5B / AI5 措辞)
- 全部核心模块 (护城河 / 红队 / HW3 / 范畴重分配 / 管理层 track record / 竞争格局 / Q2预测 / 8季度回顾) — 报告字符: ~155KB / 19章主文 + 8附录 / R-1归因 + R-2剪刀差 (5个独立) + R-3圆桌 (7位大师 + Q1 2026 specific reactions + 独立估值) + R-4认知边界量化 / 332+个DM锚点 / Mermaid 15图 / 因果密度 ~30/万字 / 评级: 审慎关注 (临界, 高争议)
