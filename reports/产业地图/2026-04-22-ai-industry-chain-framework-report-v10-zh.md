...
跨三张榜单浓缩，依次是：`NVDA` / `TSM` / `KLAC` / `ASML` / `ANET` / `ETN` / `HUBB` / `NVT` / `MU`（带周期纪律）/ `胜宏科技`。这 10 家覆盖主利润池、长周期承接、关键配套三个位置，**拒绝把它们与光模块 / 服务器 / 纯测试名字同尺比较**。
...
2. **电力接入周期比 AI 情绪周期更长** — `ETN / HUBB` 即使基本面对，也可能在一轮题材调整里被一并压估值，形成"正确但耐心不够"的错位。**`POWL` 单列**：独立深研显示其 DC 营收占比仅 2.4%、管理层 CapEx 100% 投 LNG、GM 单季 -3pp、F-C spread 历史最高——市场当前按"AI 纯 beta"47x PE 定价与其混合体业务结构错配，已从榜二移至榜三仅限交易观察。
...
| 2 | `HUBB` | 电气连接 / 配网 | 长周期基础设施承接，财务结构稳 | 4 | 4 | 4 | 优先关注 |
