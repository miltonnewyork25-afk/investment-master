...
以下十家是当前最值得持续跟踪的名字——但刻意**不给跨池总排名**，因为结构核心池、当前研究优先池、交易回调池三种资产性质不该同尺比较。每家列出所属池、当前观察动作、一条最关键触发器。

| 名字 | 所属池 | 观察动作 | 最关键触发器 |
|---|---|---|---|
| `NVDA` | 结构核心 | 核心跟踪，回调优先看 | 数据中心收入增速降档 >30pp 或毛利率回落 >3pp |
| `TSM` | 结构核心 | 核心跟踪，回调优先看 | CoWoS 利用率首次低于 90% 或 FY26 CapEx 下修 >5% |
| `KLAC` | 结构核心 | 核心跟踪 | 前沿 CapEx 显著放缓（`TSM / Samsung / Intel` 任一推迟先进产线） |
| `ASML` | 结构核心 | 核心跟踪 | 同上 |
| `ANET` | 结构核心 | 重点研究 | AI 网络订单季度环比下降或主力客户集中再升 >40% |
| `ETN` | 当前研究优先 | 中期研究 / 优先深挖 | backlog 季度环比下降 |
| `HUBB` | 当前研究优先 | 中期研究 | AI 相关需求占比不上升 |
| `POWL` | 当前研究优先 | 中期研究 | 订单与估值双高时的波动放大 |
| `MU` | 当前研究优先（带周期纪律） | 研究观察 | HBM 供给释放明显快于需求 |
| `胜宏科技` | 当前研究优先 | 跟踪 | 高端产品占比不升反降 |

**刻意留白**：`LITE / COHR / FORM / VRT / ORCL / HPE` 不进这十家——它们属于榜三交易回调池，**跟踪语言是"等价格"而不是"持续跟踪"**，混进 Top 10 会把两种动作语言混成一种。
...
**每行迷你证据块格式**：每张榜的主表后都附一张迷你证据块，每行含"最强证据（带日期化来源）+ 最强削弱 + 关键财务口径 + 最近更新时间"四列。读者无需翻到证据卡章节，就能在榜单本身看到一个名字的分数来自哪里。
...
|---|---|---|---|---|
| `NVDA` | 数据中心收入增速与毛利率同步高位（最近一期 10-Q） | `AVGO / MRVL` 吃平台 ASIC 边际增量 | 数据中心 yoy 增速 / 系统级毛利率 | 2026-04-20 |
| `TSM` | FY25 CapEx 指引维持高位 + CoWoS 利用率长期 >90% | `Intel Foundry` 或 `Samsung` 突破先进节点 | CoWoS 利用率 / FY26 CapEx 指引 | 2026-04-20 |
| `KLAC` | 过程控制高集中度，AI 芯片复杂度推高订单 | 中国成熟制程"国产替代"（较小但确定） | WFE 订单 / 客户扩产披露 | 2026-04-20 |
| `ASML` | EUV 单点垄断，前沿制程唯一通道 | `TSM/Samsung/Intel` 同步推迟下一代产线 | frontier CapEx 指引 | 2026-04-20 |
| `ANET` | Arista FY2025 业绩与指引均超预期（2026-02 法说会） | `NVDA` NVLink/Spectrum-X 挤压系统层 | AI 网络订单增速 / 客户集中度 | 2026-02 |
| `AVGO` | 自研 ASIC 订单持续放量（最近一期 10-Q） | 大客户项目节奏降速 | ASIC 大客户披露 / 内容量 | 2026-04-20 |
| `LRCX` | AI 芯片设计复杂度推动 WFE 订单 | logic/memory CapEx 同步收缩 | 设备订单 / 客户扩产 | 2026-04-20 |
| `AMAT` | 广谱覆盖 + 高稳健现金流 | 不如 KLAC / ASML 尖锐，难给同等估值 | 细分产品线收入 | 2026-04-20 |
| `MSFT` | Azure AI 收入增速与 CapEx 回流同步（FY25 Annual Report） | AI ROI 不及预期或 Copilot 渗透不达标 | Azure AI 收入 / CapEx 指引 | 2025 Annual |
| `GOOGL` | TPU 投入加速 + 2025 Q4 法说会确认 AI CapEx 上调 | 自研 TPU 内部消耗，第三方收入承接不足 | TPU 容量 / Cloud AI 收入占比 | 2026-01 Q4 法说 |
| `META` | AI 货币化 + Reels/广告效率提升 | 高 CapEx 压制自由现金流 | CapEx / 广告 ROI | 2026-04-20 |
...
| 公司 | 最强证据（带日期口径） | 最强削弱 | 关键财务口径 | 最近更新 |
|---|---|---|---|---|
| `ETN` | trailing PE `38.91` + backlog 上行 + book-to-bill >1.0 | 电力项目周期 18-36 月，情绪波动更快 | backlog / book-to-bill / lead time | 2026-04-20 |
| `HUBB` | trailing PE `32.36`，中长期基础设施受益 | AI 相关需求占比上升不及预期 | 电气连接收入占比 / 订单 | 2026-04-20 |
| `POWL` | trailing PE `46.98`，debt/equity 约 `0.22` 净现金 | 小盘高纯度，订单与估值双高时波动放大 | 订单 / 现金头寸 | 2026-04-20 |
| `NVT` | trailing PE `51.80`，数据中心电气配套不可绕开 | 多业务混合，AI 纯度低于 ETN/HUBB/POWL | 数据中心业务占比 | 2026-04-20 |
| `胜宏科技` | 年报摘要披露 AI PCB 2024-2029 CAGR 显著高于行业; forward PE `18.81`; 已进入 NVDA/AMD 供应链（定增说明书） | 高端 PCB 客户议价能力仍在买方，毛利率上限有限 | 高端产品占比 / 毛利率 | 年报 2026-03-13 / 定增 2025-02-18 |
| `MU` | HBM 收入占比快速提升，AI 级有效供给仍"二强一追赶" | 叠加 memory 周期，DRAM/NAND 下行拖累整体 | HBM 占比 / 传统 memory 价格 | 2026-04-20 |
| `沪电股份` | 高速板与交换机/服务器板卡重要受益方 | 客户仍以买方议价主导 | 高端产品占比 / 毛利率 | 2026-04-20 |
| `深南电路` | 高端板 / 封装基板综合能力 | 业务更综合，AI 纯度稀释 | AI 相关订单占比 | 2026-04-20 |
| `TTMI` | TTM 2025 Factsheet 明确 AI/DC/HPC 为重点终端 | debt/equity `0.63x`、净债/EBITDA `1.53x` 可控但不宽裕 | FCF / 高端占比 | Factsheet 2025-05-30 |
...
| 公司 | 最强证据（带日期口径） | 最强削弱 | 关键财务口径 | 最近更新 |
|---|---|---|---|---|
| `VRT` | Q3 2025 organic orders +60%，adoption 真实 | trailing PE `90.13`，拥挤度高 | organic orders / adoption | Q3 2025 结果 |
| `ORCL` | AI 云订单加速（FY2026 Q2） | debt/equity `5.09x`、net debt/EBITDA `3.90x` | 净杠杆 / CapEx 指引 | FY26 Q2 |
| `COHR` | AI 光器件综合平台，业务分散 | 利润质量 + ROIC 偏弱 | ROIC / 分业务毛利 | 2026-04-20 |
| `FORM` | HBM 与先进封装测试需求真实（Altius, 3D packaging） | trailing PE `198.86`，竞争充分 | HBM/先进封装收入占比 | 产品资料 |
| `DELL` | AI 服务器收入快速上行 | net debt/EBITDA `1.69x`，权益口径失真，资本结构实际偏重 | 经营净杠杆 / FCF | 2026-04-20 |
| `SMCI` | AI 服务器放量，弹性大 | 财务披露信任度受损 | 披露口径变化 / 库存周转 | 2026-04-20 |
| `LITE` | 800G 出货放量，上游 EML 份额高 | debt/equity `392.48`、current ratio `0.61`、net debt/EBITDA `19.55x` | 杠杆三指标同时恶化 | 2026-04-20 |
| `HPE` | AI 服务器并入后收入增长 | 净债/EBITDA `6.32x`，权益 `-155.59` PE 代理失真 | 净杠杆 / ROIC | 2026-04-20 |
...
### 27.5.2 结构质量分的公式化五维打分（驱动榜一）

每维 1-5 分，满分 25。**每维打分都绑定具体指标 + 阈值**，不是感觉判断——目标是"换个人复盘也能打出大体相同的分"。

| 维度 | 指标 | 1 分 | 3 分 | 5 分 |
|---|---|---|---|---|
| **结构关键性** | 节点玩家数 + 可替代路径时长 | >6 家 + 存在替代 | 3-6 家 + 替代需 3-5 年 | 1-2 家 + 替代需 >5 年或不存在 |
| **利润池厚度** | 毛利率稳定性 × ROIC vs WACC | 毛利率波动 >10pp / ROIC<WACC | 波动 5-10pp / ROIC=WACC+0-5pp | 波动 <5pp / ROIC>WACC+5pp |
| **供给刚性** | 扩产周期 + 认证门槛 | <12 个月 + 低认证 | 12-24 月 + 中认证 | >24 月 + 高认证（EUV 光刻、CoWoS、HBM 代际验证） |
| **财务韧性** | 净债/EBITDA + current ratio | >5x 或 <1.0 | 1-3x / 1.0-1.5 | 净现金 / >2.0 |
| **当前定价性价比** | trailing PE 相对行业中位数 + FCF 转化 | PE>行业中位数 +50% + FCF 转化差 | 接近中位数 | PE<中位数 -20% 或 FCF 转化 >80% |

**工作样例**（每家"为什么是这个分数"必须列公式反查）：

| 公司 | 结构关键性 | 利润池厚度 | 供给刚性 | 财务韧性 | 当前定价 | 合计 | 反查 |
|---|---|---|---|---|---|---|---|
| `NVDA` | 5（通用 GPU 单点 + CUDA 难替代 >5 年） | 5（毛利率 >70% 稳定 / ROIC 远超 WACC） | 5（CoWoS + HBM 供给锁定 >24 月） | 5（净债/EBITDA 约 `0.07x` / current ratio `3.91`） | 2（trailing PE `41.16` 高于行业中位数 >50%） | **22 / 25** | 定价分压低总分，前四维满分锁定榜一前列 |
| `TSM` | 5（先进节点单点 + 替代需 >5 年） | 5（毛利率稳定 / ROIC 高） | 5（CoWoS 扩产 24+ 月） | 5（净债/EBITDA 约 `0.20x` / current ratio `2.62`） | 3（trailing PE `31.86` 接近行业中位数） | **23 / 25** | 定价分优于 NVDA，总分并列最高 |
| `KLAC` | 5（过程控制高集中度） | 5（定价权强） | 5（设备门槛极高） | 5（净债/EBITDA 约 `1.77x` / current ratio `1.32`） | 3 | **23 / 25** | 五维均衡，典型"高质量卖方市场" |
| `ANET` | 4（寡头但非独家） | 4（ROIC 高但毛利率稳定性次于 NVDA） | 3（扩产周期中等） | 4（近似净现金 / current ratio `3.05`） | 3（trailing PE `59.72` 中高） | **18 / 25** | 结构核心但非顶级单点 |
| `MU` | 5（HBM 二强一追赶） | 4（HBM 利润深但叠 memory 周期） | 5（代际验证 >12 月） | 4（净债偏低 / current ratio 稳定） | 3（forward PE 偏低但含周期风险） | **21 / 25** | 结构核心但榜二更合适（带周期纪律） |

**结构质量分 ≥20 = 榜一候选**。NVDA 22 因定价分低于 TSM/KLAC（同为 23），但三家同处榜一前列，定价分差异不重排。

### 27.5.3 当前定价分的公式化五维打分（驱动榜二）

| 维度 | 指标 | 1 分 | 3 分 | 5 分 |
|---|---|---|---|---|
| **重要性 / 热度剪刀差** | 结构关键性 − 过去一年涨幅分位 | 涨幅 >80 分位 + 结构分 <3 | 持平 | 结构 ≥4 + 涨幅 <50 分位 |
| **PE vs 利润留存能力** | trailing PE + ROIC vs WACC | PE >100x + ROIC<WACC | PE 合理 + ROIC 达标 | PE <中位数 + ROIC >WACC+5pp |
| **订单与 backlog 可见性** | 公司是否披露 backlog + book-to-bill | 不披露 | 披露但无 book-to-bill | backlog 显著抬升 + book-to-bill >1.0 |
| **拥挤度** | 过去一年涨幅 + 主题 ETF 持仓占比 | 涨幅 >300% 或持仓 >20% | 100-300% | <100% 且 <10% |
| **失效触发清晰度** | 证据卡是否写出可验证触发 | 无触发器 | 含糊 | 精确到指标 + 阈值 |

**工作样例**：

| 公司 | 剪刀差 | PE/ROIC | backlog 可见性 | 拥挤度 | 触发清晰度 | 合计 | 反查 |
|---|---|---|---|---|---|---|---|
| `ETN` | 5（部署硬约束 + 涨幅 +343% 但市场仍按外围定价） | 4（trailing PE `38.91` + ROIC 达标） | 5（明确披露 backlog + book-to-bill） | 3（涨幅 +343% 但非极端拥挤） | 3（触发器部分可验证） | **20 / 25** | 定价分最高，榜二第 1 |
| `HUBB` | 5（电气基础设施重要性仍未完全 AI 化定价） | 4（trailing PE `32.36`） | 4（披露但不如 ETN 细致） | 3 | 3 | **19 / 25** | 与 POWL 并列第 2-3 |
| `POWL` | 4（高纯度但涨幅 +352%） | 5（PE 合理 + 净现金结构 debt/equity `0.22`） | 4 | 3 | 3 | **19 / 25** | 并列第 2-3 |
| `MU` | 4（AI 级带宽卡点 + forward PE 偏低） | 3（forward PE 低但带周期风险） | 3（供给披露有但不稳定） | 3 | 3 | **16 / 25** | 榜二但"带周期纪律"——非跨周期厚利润 |
| `TTMI` | 4（AI/DC/HPC 重点终端） | 3（PE 中等 + ROIC 一般） | 3 | 3 | 2（FCF 质量触发器偏感受） | **14 / 25** | 榜二末尾，不进优先级 |
| `胜宏科技` | 5（已进入 NVDA/AMD 供应链 + forward PE `18.81` 明显低于高端 AI PCB 隐含价值） | 4（forward PE 低 + 利润留存上升） | 3 | 3 | 2（AI 产品占比披露粗） | **17 / 25** | 榜二第 5 |

**当前定价分 ≥16 = 榜二候选**。17-20 分是榜二主力，14-15 分是末尾（跟踪但不进优先深挖）。

### 27.5.4 财务韧性分的公式化五维打分（驱动榜三）

| 维度 | 指标 | 1 分 | 3 分 | 5 分 |
|---|---|---|---|---|
| **净债/EBITDA** | 账面口径 + 经营净杠杆交叉核对 | >5x | 1-3x | 净现金或 <1x |
| **Current Ratio** | 最近一期披露 | <1.0 | 1.0-1.5 | >2.0 |
| **FCFE 方向** | 过去 4 季度趋势 | 转负或持续下行 | 稳定 | 正向上行 |
| **客户集中度** | Top 1 客户占比 | >30% | 15-30% | <15% |
| **Goodwill / Total Assets** | 最近一期披露 | >50% | 15-50% | <15% |

满分 25，**汇总时 ÷ 5 换算成 5 分制**，与榜三"财务韧性分 X/5"对齐。

**工作样例**：

| 公司 | 净债/EBITDA | Current Ratio | FCFE 方向 | 客户集中度 | Goodwill/Assets | 合计 | ÷5 | 位置 |
|---|---|---|---|---|---|---|---|---|
| `VRT` | 3（约 `0.76x`） | 3（`1.55`） | 3（稳定） | 3（中） | 3 | 15 | **3 / 5** | 榜三中段（可反弹交易） |
| `ORCL` | 1（约 `3.90x`） | 1（`0.75`） | 3 | 3 | 1（并购重，商誉占比大） | 9 | **1.8 / 5** | 取整 2，榜三偏后 |
| `LITE` | 1（约 `19.55x`） | 1（`0.61`） | 1（方向恶化） | 1（客户集中） | 3 | 7 | **1.4 / 5** | 取整 1，榜三末（回调先回避） |
| `HPE` | 1（约 `6.32x`） | 3（`1.01`） | 1 | 3 | 1 | 9 | **1.8 / 5** | 取整 1-2，榜三末 |
| `FORM` | 5（净现金） | 5（`4.50`） | 3 | 1（HBM 客户集中） | 3 | 17 | **3.4 / 5** | 取整 2-3，榜三中段（信用不是核心矛盾，估值是） |

**财务韧性分 ≤2（10 分以下，÷5 后）= 榜三"回调先回避"子档；3 分 = 榜三可反弹**。`LITE / HPE` 属于前者，`VRT / FORM` 属于后者。
