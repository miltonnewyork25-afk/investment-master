# Launch Brief: SMCI
> 自动生成 by tier3_launch.sh v1.0 | 2026-02-21 14:53
> **AI必须在Phase 0开始前完整阅读本文件**

## 复杂度评估
- **行业**: 半导体
- **同行业报告数**: 8 份
- **同行业参考**: AMAT(304K) AMD(341K) ASML(422K) INTC(209K) KLAC(254K) LRCX(470K) MU(178K) TSM(451K) 
- **同行业平均**: 329K chars
- **★ 目标字符范围**: 263K - 411K chars
...
## 进化教训 (最近3份报告)
  - 扩写Agent输出零DM标注 — 组装流程需增加DM注入步骤; 首次competitive-benchmarking skill成功应用
  - 扩写Agent不继承DM/CI标注规范 — CG8/CG9双FAIL的根因; Agent提示词需注入DM密度要求; CG脚本有解析bug待修
  - DM锚点集中注入+附录化比逐章散布更高效; CG脚本bug修复(EVO-ETN-003)是工具层必要投资; 产业链分析对工业公司的洞见增量>>科技公司

## Phase -1 知识检索
- knowledge_context.md: 1567 chars
- 状态: ✓ 完成
...
4. [x] checkpoint.yaml 已自动创建 (target_chars: 329006)
