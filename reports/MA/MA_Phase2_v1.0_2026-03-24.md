...
**47%的VAS OPM意味着什么**: VAS的利润率大约是Payment Network的72%(47%/65%)。这比Phase 1 Bear case中假设的35-40%更高——说明**VAS增长不是"低质量增长"**,但也确认**VAS OPM确实低于核心支付**。
...
**这是Phase 2最重要的发现之一**: 如果VAS增速维持23%而Payment增速仅10-12%→VAS占比自然从40%升至~50%(FY2028)→**混合OPM将下降~3pp**。这不是危机——56%的OPM仍然极优——但它意味着**市场不应该期望OPM持续扩张**。正常化OPM未来3年的合理轨迹是56-57%(而非向60%+迈进)。

**对估值的影响**: OPM -3pp → EPS -5%(敏感性分析: OPM ±1pp = 价格±3.3%)→ EPS下修~$1.7/share → 价格影响-$43(-5%)。这是温和的负面影响,但需要在Phase 5估值中纳入。
...
Phase 1 CEO沉默分析标记了"CI分层趋势"为🔴高风险。现在定量化"如果大客户CI+20%而SMB CI+8%"的净效应：
...
> Phase 2最重要的产出之一——将分散在各处的V-MA对比整合为单一表格。
...
| DM-NORM-001~003 | 三重验证 | OPM区间56-59%(中点57.5%) |
| DM-CI-004~005 | V框架+情景 | CI三情景(16%/19%/25% FY30) |
| DM-EPS-001 | Python 5Y | EPS收入77%/回购14%/税率6% |
| DM-BW-006~007 | 概率+联动 | 加权风险-13.1%+双墙$300-325 |
| DM-CMP-003 | 综合25指标 | V-MA全维度对标 |
| DM-ACQ-003 | Coindesk | BVNK $1.8B预评估 |
| DM-SEG-001~002 | Python反推 | 分部OPM(VAS=47%)+占比影响 |
| DM-TR-001~002 | GDV+FMP | Take Rate(净30.9bps/V 22.9bps) |
| DM-SENS-001 | Python | 四维敏感性(CAGR最敏感±6%) |
| DM-QFCF-001 | 估算 | 季度FCF季节性模式 |
| DM-CIQT-001 | 分层模型 | CI转折点=净收入增速14.65% |
