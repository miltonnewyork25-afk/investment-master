...
为什么分拆会释放如此大的价值? 因为当前$313包含了**集团折价**(conglomerate discount)——市场给"清算+非清算混合体"统一的28x PE而非分别给清算40x+非清算15x。**如果CME被分拆→是35份报告中唯一一家分拆后估值显著增加的公司**——这使反垄断风险(KS-07)变成了一个**不对称的风险: 如果触发→可能反而增值**[DM-TIMING-003A]。
...
**概率加权期望股价(12个月)**: 0.35×$240 + 0.15×$140 + 0.25×$335 + 0.10×$370 + 0.15×$250 = **$266** (vs 当前$313 = **-15%含分红-11%**) → 与评级"审慎关注(偏中性)"一致[DM-TIMING-009]。
...
| 入场价 | 胜率 | 赔率 | Kelly f* | 半Kelly | 建议仓位 |
|--------|------|------|---------|---------|---------|
| $313 | 45% | 0.8x | **-6%** | 0% | **不建仓** |
| $280 | 55% | 1.2x | 13% | 6-7% | 试探 |
| $250 | 65% | 1.8x | 28% | 14% | 核心 |
| $220 | 75% | 2.5x | 45% | 22% | 大仓位 |

@$313的Kelly为负值——这是数学上的"不应该买入"信号。只有在$270以下Kelly才变为正值[DM-ENTRY-010]。
...
| 分红削减 + PE压缩 | 正协同(强) | 15% | yield卖出→PE↓→进一步卖出 |
| Treasury成功 + VIX↑ | 反协同(双正) | 10-15% | 增速↑+利息↑=最佳情景 |
| Cloud宕机再发 + 低VIX | 弱协同 | <2% | 信任受损+FMX乘虚而入 |
