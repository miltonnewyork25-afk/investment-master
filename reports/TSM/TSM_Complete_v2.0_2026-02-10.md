...
**争议核心**: 冲突概率~13%, 封锁概率~9% [DM-GEO-001 v1.0], 但TSM PE 25.7x低于科技行业42.7x。是否隐含过高的地缘折价? 还是$165B美国投资正在消除这一折价?
...
3. 台海冲突~13%, 封锁~9%, "Justice Mission"为PLA最大规模演习 [DM-GEO-001 v1.0]
...
|:---:|------|:---:|:---:|:---:|:---:|:---:|------|
| PM-1 | 台海冲突 (2026年底前) | 地缘 | **13%** | 负面 | -65% | **-8.5%** | [硬数据: Polymarket, 2026-02-10] [DM-GEO-001 v1.0] |
...
graph LR
    subgraph 地缘风险趋势
        A["2025-10<br>台海冲突30%"] -->|大幅下降| B["2025-12<br>台海冲突17%"]
        B -->|持续下降| C["2026-02<br>台海冲突13%"]
...
**趋势解读**: 地缘风险概率在过去4个月内几乎腰斩(冲突30%->13%, 封锁24%->9%), 这与TSM同期股价从~$180涨至$218(+21%)高度相关。但值得注意的是, 宏观衰退概率26%维持稳定, 暗示市场并未完全消化关税/通胀风险。[合理推断: 地缘概率下降与美台$250B贸易协议签署直接相关, 但7月关税审查窗口可能逆转趋势]
...
| 台海冲突概率 | 13% | [硬数据: Polymarket, 2026-02-10] |
| 隐含地缘折价 | ~45% | [合理推断: TSM PE 23x vs 行业41.8x, 即使调整增长差异后仍有显著折价] |

**推理链**: TSM的23.1x远期PE相对SOXX的41.8x TTM PE存在约45%的折价。即使考虑到TSM作为"大盘低增长蓝筹"应有约15-20%的正常折价, 剩余25-30%的折价需要由地缘风险来解释。但台海冲突概率仅13%——按期望值计算, 13%概率 x (-65%冲击) = -8.5%的预期损失, 远不足以解释25-30%的估值折价。
...
|---------|:---:|---------|------|
| 台海冲突概率(冲突13%+封锁9%加权) | **15%** | (1-0.15) × 0.6 | 0.510 |
| 广泛半导体关税/制裁概率 | **25%** | (1-0.25) × 0.4 | 0.300 |
| **地缘模块** | | | **0.810** |

[硬数据: Polymarket, 2026-02-10; 合理推断: 冲突概率取台海冲突13%和封锁9%的加权值~15%]
...
**PMSI解读**:
- **76.8/100 = 偏乐观**: 地缘风险下降(冲突概率仅13%)和技术领先(N2良率80%)是主要贡献
...
1. **地缘折价是当前TSM最大的错定价来源**。台海冲突概率已从30%降至13%, 但TSM的23x远期PE仍隐含约25-30%的地缘折价(vs行业41.8x), 远超期望值计算的8.5%合理折价。如果地缘概率继续在10-15%区间盘整, TSM有15-20pp的估值修复空间。[主观判断: 但PE折价可能部分来自ADR结构性因素和台湾资本管制]
...
> **DM锚点**: [DM-GEO-001 v1.0] 冲突~13%, 封锁~9% | [DM-GEO-002 v1.0] Arizona $165B | [DM-GEO-003 v1.0] Japan/Germany扩产
...
|------|:---:|:---:|:---:|:---:|
| ADIZ年度侵扰架次 | 380 | ~1,700 | **5,709** | 15倍增长(5年) |
...
**定义**: 解放军持续、渐进式军事施压——演习频率上升、ADIZ侵扰常态化、海警常驻毗连区、网络攻击、经济胁迫——但不升级为直接动能冲突。"正义使命-2025"所展示的正是这一情景的最新形态。
...
**触发因素监控**: PLA演习频率(当前2次/年->如升至4次为警告)、ADIZ月均侵扰(当前~475架次/月->如>600为警告)、中美外交降级事件。
...
**Polymarket数据**: "Will China invade Taiwan by end of 2026?" 市场交易量已超$3.3M, 当前Yes约13%(含冲突+封锁)。值得注意的是2025年末该概率一度冲高至~30%, 后回落。[硬数据: Polymarket, 2026-02; DM-GEO-001 v1.0]

#### 情景C: 全面军事冲突 (概率3-5%)

**定义**: 台海爆发全面军事冲突, 导致一方寻求建立军事控制。
...
|:---:|:---:|---------|------|---------|
| KS-GEO-1 | **T1致命** | Polymarket台海冲突概率 > 25% | 立即清仓 | 13% — 安全 |
| KS-GEO-2 | **T2严重** | PLA年度大规模演习 > 3次 | 减仓50% | 2次(2025) — 关注 |
| KS-GEO-3 | **T3警告** | 海警毗连区常驻 > 12艘 | 减仓25% | 6-8艘 — 关注 |
| KS-GEO-4 | **T3警告** | ADIZ月均侵扰 > 600架次 | 减仓25% | ~475架次 — 安全 |
...
**[硬数据: Polymarket, 2026-02-10]**: 当前台海冲突概率13% [硬数据: Polymarket, 2026-02-10]，较2025年末30%高点下降17个百分点，降幅57% [硬数据: coinfomania.com, 2026-01-05]。
...
- 无风险PE = 42.9x (行业平均)
- 台海冲突概率 = 13% [硬数据: Polymarket, 2026-02-10]
- 冲突发生时股价损失 = 70-90%(参考俄乌冲突俄罗斯资产) [主观判断: 依据=历史战争案例]
...
**[主观判断: 依据=背离幅度25-28%]**: 当前市场隐含34.3%地缘折价，但按13%冲突概率+70-90%损失幅度计算，理论折价应为11.7-12.4%，**存在12-16个百分点的超额折价(过度悲观)**。
...
**更新数据**:
- 台海冲突概率: 30%(2025年末) → 13%(2026-02-10) [硬数据: Polymarket]
- 台海封锁概率: 9%(维持) [硬数据: Phase 0]
- Arizona Fab 1量产: 已实现 [硬数据: Phase 0 geopolitics.md]

**更新评分**:
```
地缘得分 = 100 × (1 - 冲突概率 × 0.7 - 封锁概率 × 0.3)
         = 100 × (1 - 13% × 0.7 - 9% × 0.3)
         = 100 × (1 - 9.1% - 2.7%)
         = 100 × 0.882 = 88.2分
```
...
**情景定义**: 台海爆发军事封锁或冲突，导致台积电台湾工厂停产。

**概率评估**: [硬数据: Polymarket] Polymarket"Will China invade Taiwan by end of 2026?"当前交易概率~13%(此前2025年底曾触及~30%后回落)。"Will China blockade Taiwan by June 30, 2026?"单独市场也在运行中。[硬数据: Polymarket实时数据] 学术/智库共识: 未来5年内冲突概率5-15%，封锁概率15-25%。[合理推断: 综合CSIS/AEI/Rand研究]

```mermaid
graph TD
    T1[触发: PLA对台军事行动(封锁/冲突)] --> C1[传导1: TSM台湾工厂停产<br/>占全球90%+先进产能]
    ...
    I2 --> R2[Kill Switch: 冲突概率>25%→清仓<br/>Polymarket持续监控]
...
2. **产业链连锁**: Apple(iPhone/Mac)、NVIDIA(GPU)、AMD(CPU/GPU)、Qualcomm(手机SoC)均面临供应归零。[硬数据: 各厂商对TSM代工依赖度] 全球GDP损失$2.5T/年(极端冲突情景GDP萎缩10.2%，封锁情景5%)。[硬数据: AEI研究/TradingView分析]
...
**Kill Switch**: Polymarket台海冲突概率>25% → 减仓50%; >40% → 清仓。辅助监控: 美国航母战斗群调动、台海军事演习频率、半导体设备禁运升级。[主观判断: 基于风险管理框架]
...
| 地缘面风险 | 4 | 台海冲突13%是非零尾部风险，价格未充分反映 |
...
|--------|----------|----------|--------|-------------|
| PM-GEO-001 | 台海冲突(2026年底前) | ~13% | $3.3M+ | ↓↓↓ 极端负面 |
...
**PM-GEO-001: 台海冲突 (概率~13%)**
- [主观判断:] 如果发生，TSM ADR预期跌幅: -60%至-80%
- 逻辑链: 冲突→台湾晶圆厂停产→全球芯片供应链崩溃→TSM ADR面临退市风险(资本管制)→ADR实际价值取决于海外资产(美国/日本厂)
- TSM海外产能占比<15%，极端冲突情景下绝大部分产能丧失
- **概率加权影响**: 13% × (-70%) = **-9.1%**

**PM-GEO-002: 中国封锁台湾 (概率~9%)**
- [主观判断:] 如果发生，TSM ADR预期跌幅: -30%至-50%
- 逻辑链: 封锁→物流中断(化学品/气体进口)→产能利用率暴跌→但晶圆厂物理完好→国际斡旋后可恢复
- 封锁持续时间是关键变量: <1月影响可控, >3月则接近极端冲突情景
- **概率加权影响**: 9% × (-40%) = **-3.6%**
...
3. **Polymarket** 的地缘事件概率(冲突13%/军事冲突16%)暗示的风险溢价在TSM股价中反映不足
4. 这构成了一个系统性的"乐观偏差"——当所有信息源都偏乐观时，下行风险被系统性低估

```mermaid
graph TD
    subgraph 预测市场信号强度
        PM1[台海冲突 13%<br/>影响: -70%<br/>加权: -9.1%<br/>🔴 高关注]
...
**背离描述**: Polymarket台海冲突概率13%隐含的预期损失(EL)为-9.1%，加上封锁(-3.6%)和军事冲突(-4.0%)，地缘事件总EL达-16.7%。然而TSM股价交易于52周新高附近，几乎没有体现任何地缘折价。

**数据支撑**:
- [硬数据:] TSM当前$355.41，52周高$359.60，仅差1.2%创新高
- [硬数据:] Phase 2 DCF模型使用30%地缘折价率(WACC +3%地缘溢价)
- [硬数据:] Polymarket地缘事件合并概率(冲突OR封锁OR军事冲突)约32-35%(非简单相加，存在重叠)
...
**投资含义**:
- 地缘风险是**非线性**的：0→1事件(冲突)不会给渐进式做空者带来利润，因为事件发生时ADR可能暂停交易
- Put期权是比减仓更精确的地缘对冲工具
...
- [合理推断:] 这不是真正的"矛盾"，而是机构的**理性风险管理策略**: "Long Equity + Long Put"(持多+买Put)。具体而言：
  ...
  - **买Put保护**: 对冲台海地缘尾部风险(冲突13%)和宏观系统性风险(CAPE 40.58)
...
| 子指标 | 数据 | 评分(0-100) | 权重 |
|--------|------|-------------|------|
| 台海冲突概率 | Polymarket 13% (↓自2025年底30%) | 62 | 35% |
...
**评分逻辑**:
- [硬数据:] 台海冲突概率13%较2025年底高峰30%显著下降→情绪改善，评分62 (高于50中性)
...
graph LR
    subgraph PMSI仪表盘
        GEO[地缘: 57.7/100<br/>🟡 略高于中性<br/>冲突13%↓ / 关税25%↓]
...
|------|-------------|----------|--------|
| 台海紧张升级(冲突概率→25%) | 57.7→40 | 73.0→65.9 | 看多(下沿) |
| 台海局势缓和(冲突概率→5%) | 57.7→75 | 73.0→79.9 | 看多(接近极度看多) |
| 冲突发生 | 57.7→5 | 73.0→51.9 | 中性(实际将更低) |
...
| Engine 5 预测市场 | 概率加权净期望-12.1%，地缘风险定价不足 | 冲突13%×(-70%)=-9.1%, AI65%×(+7.5%)=+4.9% |
...
| CQ6 | 地缘风险 | Polymarket冲突概率13%(↓自30%)，但市场定价不足(-16.7% EL) | M10c-ST1/M12 | 中(65%) |
...
| KA-GEO-001 | 台海冲突概率<15% | Phase 2 | 确认: Polymarket 13%(↓自30%) | 置信度→(70%) |
...
|--------|---------|------|---------|--------|
| KS-01 | Polymarket台海冲突概率>25% | 减仓50% | M10c-ST1 | 🔴 |
| KS-02 | Polymarket台海冲突概率>40% | 清仓 | M10c-ST1 | 🔴 |
...
### DP-10: Polymarket台海冲突概率~13%

| 项目 | 详情 |
|------|------|
| **原始声称** | Polymarket台海冲突概率约13% |
...
[合理推断: "军事冲突"(clash)概率高于"台海冲突"(invade),说明市场区分了灰色地带冲突和全面冲突的概率。这与Phase 1的三情景框架(灰色地带55-65%/封锁8-12%/全面冲突3-5%)逻辑一致]
...
## Bear1: 台海地缘事件 -- 冲突/封锁/灰色地带冲突

### 核心论据

**数据支撑1**: [硬数据: Polymarket 2026-02-07] 台海冲突概率(2026年底前)13%, 中国vs台湾2027年前军事冲突概率19%, 封锁概率约9%。单一钱包"Caspersmc"曾以$37,000押注冲突事件。预测市场隐含的"18个月内某种形式军事紧张升级"累计概率约25-30%。
...
- Polymarket冲突/封锁概率突破25%

### 概率评估 (18个月)
[合理推断: 基于Polymarket + 防务分析师共识] 极端冲突: 3-5% | 封锁/隔离: 5-8% | 灰色地带冲突(导弹试射/军演升级): 10-15% | 合计"某种形式的显著军事紧张": 15-20%

### 影响量化
- **极端冲突**: ADR下跌80-95%, 可能停牌/退市, 目标价$0-$50 [主观判断: 基于全球供应链彻底中断假设]
...
### 钢人论证 (最强看空逻辑)
台海风险不可对冲也不可分散。即使极端冲突概率仅5%, 其后果是"全部归零"级别的灾难性损失。在凯利公式框架下, 当单一事件可能导致-80%以上损失时, 即使概率仅5%, 合理的仓位上限也应极低。更关键的是: 预测市场的13%并非"低概率" -- 这意味着大约每8年会发生一次。投资者为一个年化EPS增长30%的公司承担13%的"全额减值"风险, 风险调整后回报可能为负。巴菲特看到了这一点, 所以他选择了日本商社而非TSMC。
...
**最可能的反证**: 预测市场的13%冲突概率严重低估了实际风险, 因为: (1) Polymarket参与者以短期交易者为主, 缺乏地缘政治专业知识; (2) 低概率事件在预测市场中存在系统性定价不足(参考: COVID之前的大流行概率<1%); (3) 中国的军事准备在加速而非放缓。

**具体论据**:
1. [硬数据: Polymarket 2025年底台海概率曾达~30%后回落至13%] 概率的"回落"可能只是短期情绪缓和, 而非风险实质性降低。中国军事能力在持续增强(每年下水吨位超过整个英国皇家海军)。
2. [合理推断] "硅盾"论述(台海冲突不会发生因为需要TSMC)存在致命缺陷: (a) 中国正在通过SMIC自主替代(7nm→5nm→3nm路线图), 每年缩小差距; (b) 可能选择"封锁"而非"全面冲突", 不摧毁但控制TSMC; (c) 如果Xi Jinping的政治遗产需要, 经济代价不是决定性因素。
...
**核心回应内容**: 三情景量化: 灰色地带55-65%(ADR -5~-15%)、封锁8-12%(ADR -30~-50%)、全面冲突3-5%(ADR -80~-90%)。概率加权损失-11.4%，而隐含PE折价14-21%，超额折价3-10%。[硬数据: Polymarket冲突13%、封锁9%] Section 232关税25%已生效但被美台贸易协议(关税上限15%)和TSM $250B美国投资承诺缓冲。PPDA背离1指出地缘风险未充分反映在52周新高股价中(地缘EL -16.7%)。
...
- **ST1**: Polymarket冲突概率>25% -> 减仓50%; >40% -> 清仓。辅助: 航母调动、演习频率、保险费率
...
TSM面临一个核心的"不可控风险" -- 台海地缘政治。[硬数据: Polymarket台海冲突概率11.5%, 封锁概率9%; Buffett 2024年完全退出TSM头寸] 即使概率低(11.5%)，影响却是灾难性的(ADR -80%以上)。[硬数据: Phase 3 M10c-ST1台海冲突压力测试] 19个Kill Switch体系完善，但KS-01(台海>25%减仓50%)和KS-02(台海>40%清仓)在实际操作中反应时间极有限 -- 一旦冲突爆发，ADR交易可能直接暂停。[合理推断: 2022年俄乌冲突爆发后，俄罗斯股市暂停交易的先例] 除地缘外: 周期风险(25-30%)可控，竞争风险(Samsung/Intel)可监控但不可控，估值风险(EV/Sales历史新高)是买入时机选择的问题。预警等级3.6/5(警告级)。[硬数据: Phase 3 M11 Engine 4]
...
|------|-------|-------|-------|-------|
| **触发条件** | Polymarket台海冲突概率突破25% | Polymarket台海冲突概率突破40% | 对台湾半导体产品关税超过50% | 美国航母战斗群部署台海 |
| **具体阈值** | >25% (连续3日均值) | >40% (单日收盘) | >50% (Section 232扩展) | 2个以上航母战斗群进入台海200海里 |
| **当前状态** | [硬数据: Polymarket] 台海冲突2026年底前~11.5-13% | 同左 ~11.5-13% | [硬数据:] Section 232已征25% | [硬数据:] 无异常部署报告 |
...
graph TD
    subgraph "KS状态仪表盘 (2026-02-10)"
        subgraph "安全区 (距触发远)"
            G1["KS-02 冲突概率>40%<br/>当前11.5% | 距28.5pp"]
        ...
        subgraph "注意区 (需定期监控)"
            Y1["KS-01 冲突概率>25%<br/>当前11.5% | 距13.5pp"]
...
| 3/31 | Polymarket "台海冲突by 3/31"到期 | 大概率利好(到期归零) | CQ6 | 如归零, 短期地缘情绪改善 | P1 |
...
| 6/30 | Polymarket "台海冲突by 6/30"到期 | 大概率利好 | CQ6 | 同上 | P1 |
...
| 12/31 | Polymarket "台海冲突by 2026"到期 | 大概率利好 | CQ6 | 年度地缘风险出清 | P1 |
...
| B5 | 检查Polymarket 3/31到期合约走势 | 地缘情绪 | 冲突概率是否持续下降 | P1 |
...
地缘风险是TSM估值永久折价的根源, 也是最难量化的CQ。[硬数据: Polymarket台海冲突概率~11.5%(下降趋势); Buffett完全退出TSM持仓; 概率加权地缘期望损失(EL) -16.7%] Phase 4对抗审查揭示了两个矛盾: (1) Polymarket概率下降(利好), 但可能低估尾部风险; (2) Buffett退出暗示"知道某些市场不知道的东西", 或仅仅是风险敞口管理。核心判断: 台海地缘风险是"低概率-高影响"的二元事件, 线性折价模型(如Phase 3的30%折价)不适合, 应用概率加权二元模型(87%正常 + 13%极端)。Arizona fab长期能收窄地缘折价(从20%→12%), 但这是5-10年的过程。当前$355的股价隐含了约10-15%的地缘折价, 低于概率加权EL的16.7%, 意味着地缘风险定价不足。
...
**3. Kill Switch关联**:
- **KS-01**: Polymarket台海冲突概率>25% → 减仓50%
- **KS-02**: Polymarket台海冲突概率>40% → 清仓
...
**5. "如果我们错了"**:
如果台海发生军事冲突(封锁或极端冲突, Polymarket当前概率11.5%), TSM ADR面临灾难性下行: (1) 封锁情景: ADR跌至$50-150(-58%至-86%), TSM台湾工厂无法出货, 全球芯片断供; (2) 全面冲突: ADR跌至$0-50(退市风险), 基础设施可能被毁。概率评估: 封锁5-8%/冲突2-4%/合计8-12%。[主观判断: 这是投资TSM的"不可对冲尾部风险", 投资者需要通过仓位控制(建议不超过组合5-8%)和期权对冲(Long Put Delta -0.20, 行权$280-300)来管理]
