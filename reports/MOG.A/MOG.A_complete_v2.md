# Moog Inc. (MOG.A) — 深度研究报告 v2.0

> **作者**: 买方研究 | **完成**: 2026-04-10 | **价格锚**: $313.25 (2026-04-09 收盘)
...
### 段 1 — 市场剧本: 为什么 Moog 看起来像一台 rerating 机器

过去 12 个月 Moog 股价从 $147 涨到 $313.25, **翻 2.1 倍**. 市场把它当作 A&D rerating 篮子里"质量最差但弹性最大的落后者" — Parker Hannifin / Howmet / Curtiss-Wright / Heico / TransDigm 已经拉到 EV/EBITDA 18.2x-37.9x [DM-PEER-EVEBITDA], MOG 的表观 15.1x (真实 **22.2x** [DM-EV-004]) 似乎还有追赶空间. 支撑这个叙事的三个承重点都在财报里读得到: (a) 12 月 backlog YoY **+30%** [DM-BACKLOG-001]; (b) Q1 FY26 book-to-bill **2.1x** [DM-BTB-001], 十年罕见; (c) adjusted OM 从 FY24 10.9% 升到 FY25 **13.0%** [DM-OPM-001], 市场相信继续扩到 14-15%. 这三个点都是真的. 不是市场编出来的. 如果 backlog 能稳定翻成股东现金, 这个剧本就是对的.

### 段 2 — 母裂缝: backlog 翻成了利润, 但没有翻成现金

问题恰恰在这里. 在市场追加 $7.7B 市值的同期, Moog 的 5 年 FCF 均值纹丝不动在 **$99.6M** [DM-FCFF-007] — **自由现金流没有参与这轮 rerating**. 更刺眼的是 FCFE (Free Cash Flow to Equity, 股东最终拿到的现金) — 6 年累计 **负值**, 同期 GAAP 净利润累计 $613M [DM-FCFE-002]. 利润在报表里释放, 现金在 inventory (+$190M)、CapEx ($145M/年) 和营运资金 (CCC 196 天) 里被锁住. FCF/净利润转化率 6 年均值 **22%**, 同业中位数 **105%** [DM-FCF-CONV-001] — Moog 每赚 $1 的账面利润, 只有 $0.22 变成了可分配现金, 同业是 $1.05. ROIC **9.31%** [DM-ROIC-001] 低于 WACC **9.5%** [DM-WACC-001], spread −19bp — 这是一家正在毁灭价值的企业. 同时, FY2026 美国国防基础预算 **下降 6.3%** [DM-DEFENSE-001], 但整个 A&D 篮子继续创新高 — rerating 驱动是"欧洲 ReArm + 导弹 supplemental + backlog catch-up"的混合久期资产, 市场按最长久期打分, 把一次性 catch-up 和结构性增长混在同一个倍数里.

### 段 3 — 新地图: 不看 backlog 有多满, 看现金翻译效率

**市场在按 backlog 定价, Moog 实际上应该按现金翻译效率定价.** 这是这份报告的母钉子. 真正决定长期股东回报的第一变量不是 backlog YoY, 不是 book-to-bill, 不是 adjusted EPS, 而是 **TTM FCF/NI 转化率** — 它衡量的是"每一美元报表利润中, 有多少变成了股东口袋里的现金". Moog 的 22% 是同业的 1/5. 估值方法必须从 Peer-relative EV/EBITDA 切换到 **Owner Earnings DCF**: EV/EBITDA 系统性忽略 CapEx 吸收 (MOG 的 CapEx/D&A 1.54x 意味着 EBITDA 的 35% 被再投入吃掉), 只有 Owner Earnings 才能反映"backlog 翻成利润后, 利润再翻成现金的真实效率". 6 个独立估值模型收敛到 $73-$175 区间, 加权中心 **$104** [DM-EXEC-001]. Reverse DCF 反推当前 $313 隐含 Owner FCF CAGR **43%** [DM-RDCF-IMPLIED] — 历史上没有任何 A&D Tier-2 达成过这个增速.

### 段 4 — 评级、边界、Kill Switch

**评级: 审慎关注 (临界).** 三点估值 **$73 (30%) / $100 (50%) / $175 (20%)**, 期望回报 −66.0% [DM-EXEC-002]. 黑箱 **32%** 触发 R-4 硬约束: 不提供单点目标价, 改为区间 + 条件评级 [DM-EXEC-003]. Kill Switch 第一触发窗口 **2026-04-24** Q2 FY26 earnings: 红灯 = Rev YoY ≤ +10% 且 BTB ≤ 1.2x; 下修 = FCF > $200M 且 CapEx ≤ $140M 且 ROIC 回升到 10.5%+. **圆桌 5/5 bear consensus**: Buffett (ROIC < WACC)、Munger (too hard)、Marks (周期位置)、Klarman (margin of safety 为零)、Druckenmiller (等 Q2 timing). 五位大师无一支持当前价格.

```mermaid
flowchart LR
    A["Backlog +30%"] --> B["GAAP 净利润 $236M"]
    B --> C["Adj EPS $8.28"]
    C --> D["市场按 PE×EPS 定价"]
    D --> E["$313"]
    B -."❶ Inventory +$190M".-> F["OCF $273M"]
    B -."❷ CapEx $145M".-> F
    B -."❸ WC +$70M".-> F
    F --> G["FCF $128M"]
    G --> H["FCFE: 负值"]
    H --> I["股东实际拿到: 零"]
    style E fill:#fdd,stroke:#900,stroke-width:2px
    style I fill:#fdd,stroke:#900,stroke-width:2px
    style B fill:#ffe,stroke:#333
```

**↑ 这是全文母图.** 后续每一章都在解释这张图的某个环节: Ch 1 解释为什么左半部分 (backlog → EPS → 股价) 看起来合理; Ch 2 解释护城河保护的是左半部分的订单, 不是右半部分的现金; Ch 3 拆开三道断裂点的具体机制; Ch 5 证明只要继续用左半部分的语言定价, 就会系统性高估 Moog.

> **以后再看 Moog, 不要先问 backlog 有多满, 要先问: 这些订单最后能不能变成股东手里的现金.**

---

## Ch 1 市场在买什么: 旧地图的成立与崩塌

> **本章回答母问题的第一个侧面**: 市场为什么相信 backlog 能支撑 rerating? 这个信念在哪一层崩塌?
...
```mermaid
graph LR
    A[FY22 市值 $3.8B] --> B[FY25 市值 $9.9B]
    A --> C[EBITDA 增长 +$2.3B ≈ 30%]
    A --> D[倍数扩张 +$5.4B ≈ 70%]
    C --> B
    D --> B
...
```mermaid
graph TB
    A[市场看的三个承重点] --> B[Backlog +30%]
    A --> C[Book-to-Bill 2.1x]
    A --> D[Adj OM 13.0%]
    E[解释不通的两件事] --> F[FY26 US Base -6.3%]
    E --> G[ROIC 9.31% vs 18x 倍数]
    B -.被 F 解构.-> H[Rerating 驱动是混合久期, 不是美国 base]
    D -.被 G 解构.-> I[数学上不可能持续向 PH 追赶]
    H --> J[真实问题: 久期与持续性分离]
    I --> J
    J --> K[需要切换估值语言到 Owner Earnings DCF]
    style E fill:#fee,stroke:#900
    style K fill:#eef,stroke:#009
```

### 1.4 全篇只回答一个问题

> **Moog 的 backlog, 为什么没有稳定翻成股东现金?**

这是整份报告的母问题. 后续每一章都在回答它的一个侧面:

- **Ch 2 (业务底盘)**: Moog 的护城河保护的是 backlog 和订单份额, 但护城河不保证现金能出来
- **Ch 3 (财务断裂)**: backlog 如何顺利翻成 GAAP 收入和 adjusted EPS, 却在 inventory / CapEx / WC 三道闸门里丢掉了 78% 的现金
- **Ch 4 (竞争格局)**: 同业用同样的护城河类型, FCF/NI 转化率却是 Moog 的 2.4-3.5 倍 — 证明这是 Moog 特有问题, 不是行业通病
- **Ch 5 (估值)**: 只要继续用 EV/EBITDA 定价, 就会系统性忽略 backlog 到现金的断裂; 切到 Owner Earnings 后, $313 的成立前提变成了历史上从未有 A&D Tier-2 达成过的增速
- **Ch 8 (红队)**: 如果我们错, 最可能错在"现金翻译效率改善速度比我们预期快"
- **Ch 9 (Kill Switch)**: 以后只看 FCF/NI 转化率、CapEx/D&A、CCC 这三个数字, 就知道 thesis 是成立还是破裂
...
这份报告不是在说服读者"Moog 会跌到 $100". 它是在做一件更具体的事: **把估值语言从市场默认的 backlog / EPS / EV-EBITDA, 切换到 FCF/NI 转化率 / Owner Earnings / FCFE**. 切换语言之后, 同样的公司、同样的财报, 得出的价值判断完全不同.

| | 市场语言 | 本文语言 |
|---|---|---|
| **第一变量** | Backlog YoY | FCF/NI 转化率 |
| **利润指标** | Adj EPS | Owner Earnings (NI − excess CapEx − WC drag) |
| **估值方法** | Peer-relative EV/EBITDA | Owner Earnings DCF |
| **backlog 含义** | "未来收入已经锁定" | "backlog 锁定的是收入, 不一定是现金" |
| **合理区间** | $305-400 | $73-175 |

**回到母钉子**: 市场在按 backlog 定价, Moog 实际上应该按现金翻译效率定价. 从这里开始, 全文每一章都在展开这个判断的一个侧面.

---

## Ch 2 业务底盘: Moog 的护城河到底保护了什么

> **本章回答母问题的第二个侧面**: Moog 的护城河保护的是订单 (backlog), 还是保护股东现金 (FCF)?
...
```mermaid
graph LR
    A[MOG 总收入 $3.86B] --> B[F-35: ~6-7%]
    A --> C[737 MAX actuators: ~5%]
    A --> D[S&D hypersonics: ~5-6%]
    A --> E[787: ~3-4%]
    A --> F[F/A-18 + KC-46: ~4-5%]
    A --> G[Patriot/NSM tactical: ~6-8%]
    A --> H[其他 ~65%]
    style B fill:#fee,stroke:#900
    style D fill:#fee,stroke:#900
    style G fill:#eef,stroke:#009
```
...
**回到母钉子**: Moog 的护城河更像"订单护城河", 而不是"现金护城河". 它能保护 backlog, 不一定能保护股东现金. 所以**市场按 backlog 定价这家公司, 用的是护城河最擅长保护的那个维度; 但投资者最终收到的回报, 取决于护城河不擅长保护的那个维度 — 现金翻译效率**. 下一章我们拆开 backlog 到现金之间的三道闸门.

---

## Ch 3 财务断裂点: backlog 是怎么在报表里变亮、在现金里变暗的

> **本章是全文的机制中枢.** 它只回答一个问题: **为什么 backlog 可以顺利翻成收入和 adjusted EPS, 却无法稳定翻成 FCF 和 FCFE?**
>
> **机制总句**: Moog 的会计系统会把 backlog 翻成收入和 adjusted EPS, 但它的经济系统会在 inventory、CapEx 和营运资金这三道闸门里, 把现金拦下来. 本章用 6 年数据拆开这三道闸门的具体吸收金额.

### 3.1 本章的任务

Ch 1 建立了旧地图 (backlog → EPS → 股价) 并指出了裂缝. Ch 2 展示了护城河保护的是订单而不是现金. Ch 3 的任务是进入母图 (执行摘要) 的**右半部分** — 打开三道断裂点, 用数字解释 backlog 到现金之间的差额.
...
**回到机制总句**: Moog 的会计系统把 backlog 翻成了收入和 adjusted EPS, 但它的经济系统在 inventory (+$190M)、CapEx ($735M 累计)、WC (CCC +60 天锁住 $635M) 三道闸门里, 把现金拦了下来. 真正的问题不是订单会不会来, 而是订单来了以后, 现金能不能出来. 下一章我们把 MOG 与同业做系统对比, 验证这个"现金翻译断裂"是 MOG 特有, 还是行业通病.
...
## Ch 5 估值语言切换: 为什么 EV/EBITDA 会系统性高估 Moog

> **本章回答母问题的第五个侧面**: 为什么市场继续用 EV/EBITDA, 就会系统性高估 MOG? 切换到 Owner Earnings 后, 当前价格的成立前提是什么?

市场现在用的是一种天然会高估 Moog 的语言. 它把 backlog、EBITDA、adjusted EPS 当成价值代理. 对于多数 A&D 公司, 这没问题 — 因为它们的 FCF/NI 转化率在 90-150%, EBITDA 和 Owner Earnings 之间的差距不大. 但对于 CapEx 吸收结构性偏高 (CapEx/D&A 1.54x)、营运资金长期恶化 (CCC 196 天且 6 年扩张 60 天)、FCFE 常年为负的公司, EV/EBITDA 会系统性忽略 "backlog 翻成利润后, 利润再翻成现金" 这一步的效率损失. 所以必须切换到 Owner Earnings. **这不是选择一种"更悲观的方法", 而是选择一种"能看到 Moog 特有问题的方法"** — EV/EBITDA 对 PH/HEI/TDG 没有盲区, 但对 Moog 有.

### 5.1 为什么要建六个模型

六个独立模型的价值在于: 只要它们都指向 $50-$180 区间 (而不是 $200-$400), 结论的稳健性就**不依赖于任何单一模型的正确性**. 每个模型回答母问题的一个子问题:
...
```mermaid
graph TD
    A[历史基准率<br/>10yr 5 programs<br/>legacy 剔除后 37.5%] --> D[三锚综合<br/>胜率 ~30%]
    B[反例调整<br/>MEA spec 偏移 -8pp<br/>Parker 787 经验 -3pp<br/>F-35 spillover +2pp<br/>净 -9pp] --> D
    C[自然实验<br/>T-7A 2025 输给 Parker<br/>FLRAA 仅拿 secondary<br/>压力测试 -2pp] --> D
    D --> E{需要 50% 胜率<br/>才能 maintain share}
    E -->|30% < 50%| F[存量份额年均<br/>流失 1.5-2% MA Rev<br/>10yr 累计 -18%]
    F --> G[Military Aircraft 分部<br/>FY35 Rev $710M<br/>vs 当前 trajectory $888M]
    style D fill:#ffe
    style G fill:#FFB6C1
```
...
```mermaid
graph TD
    A[Phase 2 六模型加权<br/>**$104 /股**] --> B[博弈 1 B-21/NGAD<br/>三锚胜率 30%<br/>**−$8.7**]
    A --> C[博弈 2 Industrial 剥离<br/>价格已内化 Ch 5<br/>时点滑点 **−$5**]
    A --> D[博弈 3 3-region 零和<br/>EU 折价 + US base + FMS<br/>**−$0.55**]
    B --> E[三场博弈综合<br/>**−$14.25**]
    C --> E
    D --> E
    E --> F[Phase 2 + 博弈<br/>**$89.75**]
    F --> G[Polymarket 6 情景<br/>Ukraine 24% × Taiwan 13.5% × Budget<br/>**−$0.57**]
    G --> H[Phase 3 v2 调整后<br/>**$89.18/股**<br/>**vs 当前 −71.5%**]
    style A fill:#e0f7fa
    style H fill:#FFB6C1
```
...
```mermaid
graph LR
    A[US DoD Base FY25<br/>$824B] -->|FY26 request<br/>**-6.3%**| B[US DoD Base FY26<br/>$773B]
    C[MOG share price<br/>$168 → $313<br/>**+86%**] --> D[MOG EV/EBITDA<br/>14.7x → 22.2x]
    B -.->|矛盾| D
    style B fill:#FFB6C1
    style D fill:#FFB6C1
```
...
## Ch 12 默认值替换: 以后怎么看 Moog

这一章不是总结. 前面 11 章已经把证据链展开完了. 这一章的任务是: **把新地图变成读者以后默认会调用的入口**. 读完这篇报告, 合上文档, 下次再想到 Moog 时, 默认问题应该已经从"backlog 有多满"变成"这些订单能不能变成股东现金".

### 12.1 回到母问题

全文追的母问题是: **Moog 的 backlog, 为什么没有稳定翻成股东现金?**

答案在 Ch 3 的三道闸门里: inventory 吸收 (+$190M)、CapEx 吸收 ($145M/年, CapEx/D&A 1.54x)、营运资金吸收 (CCC 196 天, 6 年扩张 60 天). 这三道闸门让 FCF/NI 转化率 6 年只有 **22%**, 同业是 **105%** [DM-FCF-CONV-001]. 结果是 ROIC 9.31% [DM-ROIC-001] < WACC 9.5% [DM-WACC-001], 每多投入一美元资本, 公司价值就多损失一点.

### 12.2 回到母钉子

**市场在按 backlog 定价, Moog 实际上应该按现金翻译效率定价.**

| | 市场默认入口 | 读完本文后的新入口 |
|---|---|---|
| **第一个问题** | "backlog 增了多少?" | "这些 backlog 有多少能变成股东现金?" |
| **第一变量** | Backlog YoY / BTB | **FCF/NI 转化率** (当前 22%, 同业 105%) |
| **估值方法** | Peer EV/EBITDA | **Owner Earnings DCF** |
| **什么算好消息** | backlog +30% | FCF/NI > 50% 且连续 2Q |
| **什么算坏消息** | backlog 下降 | CapEx/D&A > 1.5x 且 CCC > 190 天 |

**具体跟踪** (每季度): (1) TTM FCF / TTM NI — 如果连续 2Q > 50%, 上修信号; (2) CapEx/D&A — 如果降到 < 1.2x, CapEx cycle 拐点确认; (3) CCC — 如果缩到 < 175 天, WC 改善确认.

### 12.3 回到一个具体画面

回到执行摘要的母图. Backlog 很满. Adjusted EPS 很亮. 但 inventory、CapEx、营运资金这三道闸门把现金拦下来了. 报表左半部分运转正常 — backlog 翻成收入, 收入翻成 GAAP 利润, 利润翻成 adjusted EPS. 市场按左半部分定价. 但报表右半部分 — 利润翻成 OCF, OCF 减 CapEx 翻成 FCF, FCF 减利息和债务翻成 FCFE — 每一步都在漏. 6 年累计, 股东从 equity 层面拿到的净现金是零.
...
2. **CapEx/D&A 多少?** 如果 > 1.3x 且公司不披露 maintenance vs growth 拆分, 默认 treat 为 "FCF 陷阱".

> **市场看到的是一台 backlog 机器. 我们看到的是一台现金翻译机, 而且它的翻译长期失真. 以后再看 Moog, 不要先问订单有多满, 要先问这些订单最后能不能变成股东手里的现金.**
