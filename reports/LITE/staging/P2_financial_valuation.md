...
1. **极低票息成本**：年化利息支出仅$12.6M——这是聪明的融资。$1.79B债务每年只花$12.6M利息，因为利率用转股权支付了。

2. **全部深度实值**：两批可转债都深度实值（最低也是4.4倍实值），这意味着转换几乎确定。唯一的问题是何时转换以及以现金还是股票结算。

3. **潜在稀释有限但真实**：Treasury Stock Method下增加12.1M股（从71.1M到83.2M），稀释率17%。但如果公司选择全部股票结算（最大稀释），增加14.3M股到85.4M。
