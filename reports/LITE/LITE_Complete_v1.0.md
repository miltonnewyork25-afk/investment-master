...
Lumentum未在10-K中披露中国收入占比——因为大部分通过分销商和间接渠道销售。但我们可以从间接证据推断：

**直接层**（Cloud Light模块→中国客户）:
- Cloud Light总部位于香港，在中国和泰国有制造设施 [DM-ACQ-001]
- 收购前Cloud Light的主要客户包括中国数据中心运营商
- 估计直接中国收入: ~5-8%总收入

**间接层**（EML芯片→中国模块商→终端客户）:
- 中际旭创（Innolight）是全球最大800G模块商，2025年收入~$3.3B [DM-IND-064]
- 旭创几乎100%使用外购EML芯片，LITE是主要供应商
- 估计通过旭创的间接收入: ~10-15%总收入
- 但旭创的终端客户主要是全球Hyperscaler(不是中国本土客户)→这部分不直接受中国出口管制影响

**总中国暴露度估计**: ~15-23%收入（直接5-8% + 间接10-15%）
- 其中真正面临管制风险的(sell-to中国本土客户): **~5-10%**
- 其余通过旭创出口到全球Hyperscaler的: 不直接受管制但受旭创-LITE关系变化影响
...
| **S3 中国反制** | 中国对InP原材料/稀土实施反制管制 | 5% | 供应链中断风险 |
...
**TSMC依赖度**: LITE的InP芯片在**自家工厂**生产(San Jose + 即将启用的Greensboro)——不依赖TSMC。这意味着LITE不受台海冲突的直接供应链中断风险，但间接影响仍存在（客户的GPU/交换机芯片依赖TSMC）。
