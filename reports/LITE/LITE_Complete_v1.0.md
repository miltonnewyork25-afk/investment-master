...
商誉$1,061M(Cloud Light $366M + NeoPhotonics $695M)+ 无形资产$397M = **$1,458M(占总资产30%)** [DM-BAL-004]。
...
**减值Kill Switch**: 如果LITE任何分部连续2年GAAP ROIC<WACC(10%)→减值测试几乎确定触发。当前GAAP ROIC=6.2%<WACC→**已在观察区**。
...
**10-K披露的地理数据（ship-to basis）** [DM-GEO-001]：
...
**S3概率上调至10%的原因** [DM-GEO-002]：Agent发现**中国已于2025年2月对InP晶圆衬底实施出口许可管制**（继2023年8月GaAs管制之后）。InP衬底供应商AXT（在中国制造）报告每张许可需~60个工作日处理，Q4 2025收入已受约束。如果LITE的InP衬底有任何中国来源→直接供应链风险。这不是假设性风险——**管制已经在执行中**。
...
**TSMC依赖度=0%（Agent验证）** [DM-GEO-003]：LITE的InP芯片**完全自有工厂**生产——
...
| Kill Switch | 触发条件 | 监控 | 当前状态 |
|------------|---------|------|---------|
| **G-KS1** | 200G+光芯片被纳入BIS管制清单 | BIS公告/Federal Register | 🟢 未纳入 |
| **G-KS2** | 中国对InP/GaAs原材料实施出口管制反制 | 中国商务部公告 | 🟢 未实施 |
| **G-KS3** | 台海冲突概率>15%(Polymarket) | Polymarket | 🟢 当前<10% |
...
| 指标 | 当前读数 | 方向 | 红灯阈值 | 含义 |
|------|---------|------|---------|------|
| **L1: Hyperscaler合计AI CapEx增速** | +67-74% YoY (2026E) [DM-IND-069] | ↑ 加速中 | <+10% YoY | 需求引擎失速 |
| **L2: 800G模块ASP趋势** | 稳定→微升(供不应求) | → | YoY<-20% | 定价权崩溃 |
| **L3: EML供需缺口** | 需求超供给25-30% [DM-BIZ-003] | 缺口在收窄(产能扩张中) | 供过于求 | 从卖方变买方市场 |
| **L4: LITE DIO趋势** | 121天(改善中) [DM-RAT-003] | ↓ 改善 | >150天且连升3Q | 库存积压/需求放缓 |
| **L5: CapEx/OCF比率(Hyperscaler)** | 90% (2026E) [DM-IND-069] | ↑ 危险 | >100%(FCF转负) | 超大规模客户不可持续 |
| **L6: SiPh 800G渗透率** | 40-45% (2026年中) [DM-COMP-002] | ↑ 快速 | >60% | EML技术路线受威胁 |
...
**CapEx/OCF比率——最关键的可持续性指标** [DM-IND-073]：
...
**投资缺口已升级**：Bain在2025年9月第六次年度报告中将AI投资缺口从$800B**升级为$2万亿**——到2030年需要$2T年收入来支撑AI计算需求 [DM-IND-074]。即使企业将所有本地IT预算转移到云端+将AI节省的20%重新投入DC CapEx，**仍然短缺$800B**。

**AI应用层真实收入**：生成式AI应用层2025年收入仅$19B(Menlo Ventures数据 [DM-IND-075])——vs 基础设施投入$380B = **20倍缺口**。这比我们之前引用的"6:1"更极端，因为$50-60B包含了GPU/云服务收入（卖铲收入），而纯应用层收入远低于此。

### 800G模块ASP趋势——被低估的风险 [DM-IND-076]
...
**离散度极高是LITE的本质特征，不是估值失败。** CQ加权置信度35.4%意味着我们对LITE的理解比典型报告(60-70%置信度)低得多。$270-$310是在承认不确定性下的最诚实判断——给出$280±$5的精确数字反而是虚假精度。

---

## Ch-U5: RT-6概率三重锚定补充（反思升级）

> **来源**: R2审计发现P4 RT-6噩梦清单概率缺三重锚定。
...
### 关键修正：半导体CEO公开买入基准率（Agent验证） [DM-INS-010]

R2审计建议验证"LITE内部人零买入是否异常"。Agent搜索结果**改变了我们对CQ6的解读**：
...
之前判断：LITE "8年零公开市场买入"是"极端负面信号"(CQ6置信度65%)——暗示管理层不相信自己的股价。
...
- CQ6置信度：从65%下调至**55%**（信号强度因基准率校准而减弱）
- **加权平均CQ置信度**：从35.4%微降至**34.2%**（CQ6权重下降1.25pp的传导）

**但内部人信号并未完全失效**——关键区别在于：
1. AMAT CEO在暴跌30%时买入——**如果LITE管理层也在下次暴跌中买入，那才是真正的看多信号(KS-3)**
