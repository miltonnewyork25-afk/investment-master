...
| **中远海能 (1138.HK)** | HK$19.55 | **HK$27.47** | **+40.5%** | 关注 | 111.4% |
...
**关注(中远海能)的理由**: 期望回报+40.5%配合PB 1.2x底部保护(最大下行-28% vs FRO -55%)，东方通行证已从假说升级为A级事实。OFAC制裁是唯一致命风险(15-25%概率)，但概率加权后净正期望。
...
**修正后EV = $25.0 → 当前$31.30 → 隐含高估20%!**

这比早期的$34.0估计更低(因为加入了PE压缩效应: 周期顶部应给更低PE)。

**这改变了结论**: FRO不仅上行有限(+44% in S4)，概率加权后是**负期望** → **FRO在当前价位不应该买入**。
...
| **FRO(作为参照)** | 6.5 | 准一阶/准二阶 | 当前价位不推荐(负期望) |
...
早期结论"FRO EV $25.0(-20%高估)"被修正为**$46.73(+49.3%)**。修正原因:

1. **隐形税校正**: 早期扣除$55K/天隐形税(假设FRO进波斯湾) → 确认FRO在非PG航线运营，实际增量成本已包含在报告TCE中 → 不再双重扣减
2. **校准方法**: 从Q4 2025实际EPS数据反推 → 校准更精确
3. **PE假设修正**: 明确: 高EPS×低PE(8x) 或 低EPS×高PE(12x)

**这个修正意味着什么?** 早期结论——"中远优于FRO"——在绝对回报上被推翻(FRO +49.3% > COSCO +34.3%)。但在**风险调整后**的结论仍然成立(见下)。
...
| 数据点 | 原因 | 风险 |
|--------|------|------|
| 中远A/H溢价 (DM-COS-007) | 未获取实时数据 | 影响A/H选择建议 |
