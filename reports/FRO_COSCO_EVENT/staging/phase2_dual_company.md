...
这比Phase 2之前的$34.0估计更低(因为加入了PE压缩效应: 周期顶部应给更低PE)。
...
# Chapter 20B: 二阶受益者四层验证 — Lancashire/Cheniere/造船

## 20B.1 用户二阶框架的系统应用

Phase 0的UU搜索发现了多个二阶受益者候选。用四层验证框架逐一检验:
...
1. **FRO概率加权后是负期望(-15.5%)**: Ch10逆向估值证明 — 周期顶部应给低PE → $100K TCE × 14x PE ≈ 当前价 → 上行空间被PE压缩吞噬。**FRO在$31.30不应买入**。

2. **中远海能是更优标的**: 东方通行证(A级)+COSCO Captive保险+低杠杆+PB 1.2x底部 → 运价$40K时中远仍PE 16x(vs FRO 142x)。**下行保护质量不在一个级别**。

3. **OFAC是中远唯一致命风险**: 15-25% SDN概率 → 但概率加权后期望损失-8% < 东方通行证期望收益+6-13% → **净正期望**。

4. **保险分割是最不可逆的结构性变化(Ch16)**: 伦敦P&I垄断被打破 → 即使冲突结束，中国保险体系已建立 → 中远的结构性优势长期化。

5. **二阶受益者验证**: Lancashire(7.7分真二阶) > 造船(7.3但延迟) > Cheniere(4.3伪二阶)。FRO本身仅6.5分(准一阶/准二阶)。

6. **修正后EV**: FRO **$25.0(-20%高估)** / 中远 HK$22.5(+15%) → **中远优于FRO的差距从6pp扩大到35pp**。

7. **最佳策略排序**: 纯做多中远H股 >> Lancashire(待理赔验证) >> 不做FRO(负期望) >> Pair trade(如有执行能力)。
