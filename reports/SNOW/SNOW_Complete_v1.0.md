...
| Iceberg | -0.5~1.0pp/年 | FY29-FY33 | 低(渗透速度不确定) |
| **叠加** | **-2.5~4.5pp/年** | | |

**但双平台共存是更可能的终局**: 历史上，企业数据平台市场几乎从未出现"赢家通吃"(Oracle/SQL Server/Teradata共存了20年)。更可能的结果: SNOW在结构化数据/SQL workload保持优势，Databricks在AI/ML workload领先，Fabric覆盖Azure生态的"足够好"需求。三平台各占25-35%市场份额[DM-COMP-018-EST]。
...
### 13.2 护城河迁移: 从制度锁定到偏好锁定

SNOW的护城河正在经历一次根本性迁移——用读者容易理解的方式说: **从"客户想走但走不了"到"客户能走但选择不走"**[DM-MOAT-003-EST]。

| 阶段 | 护城河类型 | 时间 | SNOW状态 |
|------|-----------|------|---------|
| **Phase 1** | 制度锁定(数据格式私有) | FY20-FY25 | ✅ 已过 |
| **Phase 2** | 迁移中(Iceberg降低锁定) | **FY26-FY29** | **← 当前** |
| **Phase 3** | 偏好锁定(性能+功能+生态) | FY30+ | 取决于Cortex AI成功 |

迁移中的关键变量: **Cortex AI能否在Iceberg侵蚀数据锁定之前建立新的AI workload锁定?** 如果能→新护城河(AI推理引擎的性能/生态)替代旧护城河(数据格式锁定)→CQI可能回到55+。如果不能→护城河持续弱化→CQI可能降至40以下→估值倍数面临系统性压缩。

**历史先例**: Oracle的护城河迁移从"数据库锁定"→"应用生态锁定"用了10-15年(2000-2015)。SNOW的窗口更短(3-5年)因为开源替代的速度远快于Oracle时代。
...
> **核心结论**: SNOW的风险结构呈现"协同三角"特征——三层竞争(Databricks/Fabric/Iceberg)之间存在强化协同: Iceberg降低迁移成本→Databricks/Fabric更容易争取客户→客户迁移增加→Iceberg采用更广→更多竞争者。温水煮青蛙概率: 35%(最可能的负面场景)[DM-RISK-005]。

### 14.1 风险协同矩阵

```mermaid
graph TD
    R1["R1: Databricks竞争<br>-1.5~2.5pp增速"] <-->|"互相强化<br>DBR成功→证明开源可行→更多企业用Iceberg"| R3["R3: Iceberg侵蚀<br>数据锁定降级"]
    R2["R2: Fabric底部蚕食<br>-0.5~1.0pp增速"] <-->|"互相强化<br>Fabric+Iceberg=免费+开放→双重替代"| R3
    R1 <-->|"间接强化<br>DBR+Fabric争夺→SNOW S&M被迫加码"| R2

    R1 --> IMPACT["叠加影响<br>-2.5~4.5pp/年"]
    R2 --> IMPACT
    R3 --> IMPACT

    style R1 fill:#ea4335,color:white
    style R2 fill:#ff9800,color:white
    style R3 fill:#9c27b0,color:white
    style IMPACT fill:#000,color:white
```

**协同概率34%**: 三个风险中任意两个同时恶化的概率(基于独立概率计算后调整相关性)。如果三者同时恶化→增速影响从-2.5pp上升到-4.5pp/年→Base Case增速从22%降至17-18%→FV从$134降至$100以下[DM-RISK-006-EST]。

### 14.2 温水煮青蛙场景

**概率: 35%**(最可能的负面场景)

这不是一个戏剧性的崩溃——而是一个缓慢的、不容易被察觉的过程:

```
FY27: 增速21%(管理层指引)→"符合预期, 但不惊艳"
FY28: 增速18%→"稍微减速, 但RPO仍在增长"
FY29: 增速15%→"减速加深, 但AI叙事仍在支撑估值"
FY30: NRR 118%→"增长引擎放缓, 但还在120%以上"
FY31: 增速12%, NRR 112%→"温水煮青蛙, 增速已降至SaaS均值"
```

**为什么"温水煮青蛙"比"急剧崩塌"更危险?** 因为每一年的减速都"看似合理"→投资者每一年都"再给一个季度看看"→等到意识到时增速已经从30%降到12%→累计损失>逐步减持→**沉默的估值侵蚀**[DM-RISK-007-EST]。

**Kill Switch对应**: NRR<118% 连续2Q是温水煮青蛙的最早预警信号。
...
**双平台共存(45%概率)的含义**: SNOW保持SQL/结构化数据优势, DBR保持AI/ML优势, 两者在"full stack data platform"领域重叠50-60%。这意味着: (1)两家的增速都不会达到各自的Bull Case, (2)市场份额从"赢家通吃"变成"三足鼎立"(+Fabric), (3)估值倍数从"高增长SaaS"向"成熟平台"压缩(EV/Sales从11x→7-8x over 5年)。
...
| Fabric未计入 | Fabric $2B ARR/60%增速→底部蚕食加速 | **新增风险** |
| SBC FY31<15% | 6家先例: 只有NOW/CRM做到, 30%概率平台期 | **执行风险** |
| AI增量支撑 | AI仅$100M=2.3%消费, 68%试水非生产 | **规模远小于暗示** |

### 15.3 G(差距): 核心预期差量化

| 差距 | 量化 | 对FV影响 | 置信度 |
|------|------|---------|--------|
| **G1: 增速高估** | 市场隐含27% vs 实际可能20-22% = +5-7pp gap | **-$20~30/股** | 中(DBR数据不确定) |
| **G2: Fabric未定价** | $2B ARR/60%增速未反映在SNOW折价中 | **-$5~10/股** | 低(Fabric数据不单独报告) |
| **G3: RPO被低估** | RPO +42%是最硬证据, 但市场可能已部分定价 | **+$5~15/股** | 中 |

**净预期差: -$10~25/股** → 支持当前$154偏贵的结论(FV $130-145区间)[DM-EG-005-EST]。

### 15.4 T(触发器): 什么会触发预期修正

| 触发器 | 概率 | 时间窗口 | 变量类型 | 影响 |
|--------|------|---------|---------|------|
| T1: Databricks IPO(S-1) | 60% | FY27-28 | **迁移变量** | 强制市场直接对比→竞争折价可能加深 |
| T2: FY27Q1增速>25% | 30% | 3个月 | **校验变量** | 验证RPO转化→可能上调至中性 |
| T3: NRR<118% 连续2Q | 20% | FY27-28 | **迁移变量** | 增速引擎失效→下调确认 |
| T4: Cortex AI>10%消费 | 25% | FY28 | **迁移变量** | 新护城河建立→根本改变投资论文 |
| T5: Fabric>$5B ARR | 35% | FY28-29 | **约束变量** | 底部蚕食加速确认 |

**变量四分法标注**(致命缺口B3修复): T1/T3/T4是**迁移变量**(推动状态转移的关键)——它们发生时需要重新评估评级。T2是**校验变量**(验证Base Case是否成立)——不触发评级变化但调整概率权重。T5是**约束变量**(外部不可控)——监控但不影响即时动作[DM-EG-006-EST]。
