...
> **P/E数据说明**: 本章39.9x来自Phase 0数据预取时点(DM-060), 执行摘要中36.1x来自Phase 2更新时点(DM-080, FMP compare实时TTM)。TTM P/E随季度财报滚动更新, 两者均为各自时点的硬数据。详见附录D.1A方法离散度声明。
...
### 10.3 制度范式视角 — 三次计算制度变迁与认知偏差

上述三种类比本质上对应三种**制度变迁假说**——理解制度层面的演化规律, 比预测某个季度的财务数字更有洞察价值。
...
1. **锚定效应**: $4.31T市值本身成为心理锚——看多者认为"已经这么大了, 不会再跌太多", 看空者认为"这么大了, 一定要跌"。两种锚定都是错误的。正确的问法不是"$4T是不是太大", 而是"$120B净利值几倍"。

2. **可得性偏差**: Cisco 2000、矿潮2021这些生动的崩盘记忆极其鲜明, 使分析者过度权重"泡沫破裂"情景。但同样鲜活的反例被忽视: Microsoft从2000年"崩盘"到2024年$3T+的路径, AWS从2006年"烧钱"到2024年$100B+收入——**有些看似泡沫的东西确实改变了世界**。

3. **叙事谬误**: "AI是新电力"和"AI是新矿潮"都是简洁有力的叙事, 人脑天然偏好选择一个并忽视另一个。现实更可能是: AI既改变世界(如电力), 又经历周期(如矿潮)——**两个叙事不互斥, 但时间维度不同**(周期3-5年, 范式20年+)。
...
### 71.3A 数学不可能性测试 — $4.31T隐含了什么?

KLAC报告(4.5/5)的信念反演方法论移植: 将隐含假设翻译为物理世界的量——测试是否"数学上不可能"。

**反推过程**:

当前市值$4.31T [硬数据: FMP], P/E 36.1x [硬数据: FMP], 净利$120B [硬数据: FMP], 净利率55.6%。

假设投资者要求年化回报10%(含风险溢价), 则5年后目标市值需达~$6.9T。假设成熟后P/E压缩至22x(情景B终态), 则隐含5年后净利需达$314B, 隐含收入~$565B(按55%净利率)。

NVIDIA FY2026数据中心收入$198B [硬数据: FMP], 其中AI GPU占~90%。假设数据中心占总收入90%(保守), 则AI GPU相关收入~$178B。

**隐含AI GPU TAM测试**:

如果NVIDIA在5年后仍保持70%份额(从当前~90%保守下降), 且AI GPU收入达$508B(=$565B×90%), 则隐含AI GPU市场TAM需达~$726B。

当前全球半导体市场(2025年): ~$580B [合理推断: SEMI/SIA行业数据]。AI GPU在全球半导体中占比需从当前~31%上升至~125%——**超过整个半导体市场**。
...
1. **半导体市场总量突破历史增长率**(CAGR 8%→12%+)
2. **AI GPU品类占比超过任何历史先例**(>40%→85%)
3. **NVIDIA在扩大的市场中仍保持70%+份额**(自研侵蚀<30%)
...
| **CI-05** | 分析师共识与市场定价逻辑不兼容——FY2030E收入↓2.8%却给P/E 36x, 要么分析师错(收入不降)要么市场错(P/E过高) | 方法论 | 70% | FMP estimates FY2030↓2.8% [硬数据: FMP] + P/E 36.1x [硬数据: FMP] + B-1×B-4冲突(Ch 71) | 所有高P/E股票 | ±20-30% |
...
|------|:--:|
| 总字符(Phase 1-5) | ~193K |
| v2.0增补 | ~10K(审计+O7+CI-05+B-4+情景E) |
...
- DM-060 | P/E (TTM): 39.9x | P/E (FY2027E): 21.7x | 来源: FMP quote + estimates | 注: Phase 0时点数据, Phase 2更新见DM-080(36.1x), 差异为TTM滚动更新
...
**CI冠军候选**: CI-01"史上最贵周期股"(8.0) + CI-05"分析师-市场不兼容"(9.0, 方法论级别, 可验证性满分)。
...
**DM覆盖率: 93%** — 报告中291个DM锚点覆盖了所有核心财务数据点、估值参数和关键假设。未覆盖的7%主要集中于行业类比参数(历史CapEx周期长度等定性判断)和极端情景假设(情景D/E的部分参数)。

### E.2 来源分布

| 来源层级 | 占比 | 代表性数据 | 可信度 |
|---------|:---:|---------|:----:|
| **P0 MCP一手数据** | 52% | FMP income/balance/cashflow/estimates/compare | ★★★★★ |
| **P1 分析师/机构** | 10% | FMP estimates共识、100baggers策略分析 | ★★★★ |
| **P2 跨报告引用** | 14% | SEMI_EQUIPMENT横向对比、ANET Token经济、INTC护城河迁移 | ★★★★ |
| **P3 Web搜索** | 7% | 行业TAM数据(SEMI/SIA)、产品规格、主权AI投资 | ★★★ |
| **P4 推算/建模** | 17% | 五情景P&L建模、PPDA概率分布、CUDA半衰期估计 | ★★★ |

### E.3 验证详情

<details>
<summary>E.3.1 DM可信度分布</summary>

| 可信度等级 | DM数量 | 占比 | 典型来源 |
|-----------|:-----:|:---:|---------|
| ★★★★★ (一手硬数据) | 152 | 52% | FMP financial statements, FMP quotes |
| ★★★★ (分析师共识) | 29 | 10% | FMP estimates, 100baggers |
| ★★★★ (跨报告交叉) | 41 | 14% | Phase间引用(已追溯原始DM) |
| ★★★ (Web多源) | 20 | 7% | WebSearch结果, 行业报告 |
| ★★★ (推算) | 49 | 17% | 模型输出, 概率估计 |

**注**: P4(推算)的49个DM中, 38个有明确的计算公式或模型假设链条(可复现), 11个为定性判断(已标注[主观判断])。

</details>

<details>
<summary>E.3.2 Phase 0锚点验证</summary>

Phase 0预取的183个DM锚点在Phase 1-5中的使用情况:
- **直接引用**: 156个(85%) — 原始数据在后续分析中被引用
- **更新替代**: 18个(10%) — Phase 2/3获取了更新数据(如P/E从39.9x更新为36.1x)
- **未使用**: 9个(5%) — 预取但分析方向调整后未引用(如部分竞品详细财务)

Phase 0→Phase 2的P/E变化(39.9x→36.1x)属于数据时间差, 非幽灵数据。详见附录D.1A方法离散度声明。

</details>

<details>
<summary>E.3.3 估值模型输入验证</summary>

| 输入参数 | Phase 2值 | FMP验证 | 偏差 | 影响 |
|---------|----------|---------|:---:|------|
| FY2026收入 | $216B | $215.9B [硬数据: FMP] | <0.1% | 无 |
| FY2027E收入 | $356B | $356.3B [硬数据: FMP] | <0.1% | 无 |
| FY2028E收入 | $464B | $463.9B [硬数据: FMP] | <0.1% | 无 |
| P/E (TTM) | 36.1x | 36.1x [硬数据: FMP] | 0 | 无 |
| 毛利率 | 71.1% | 71.1% [硬数据: FMP] | 0 | 无 |
| WACC | 10.5% | N/A(模型假设) | — | 敏感性±1%→±$0.5T |
| 终端增长率 | 3.5% | N/A(模型假设) | — | 敏感性±0.5%→±$0.3T |

</details>

<details>
<summary>E.3.4 竞争对标一致性</summary>

| 对标 | NVDA报告数据 | FMP验证 | 一致? |
|------|-----------|---------|:----:|
| AMD P/E | 77.0x | 77.0x [硬数据: FMP compare] | ✓ |
| AVGO P/E | 67.1x | 67.1x [硬数据: FMP compare] | ✓ |
| QCOM P/E | 28.8x | 28.8x [硬数据: FMP compare] | ✓ |
| AMD ROE | 5.5% | 5.5% [硬数据: FMP compare] | ✓ |
| AVGO ROE | 57.5% | 57.5% [硬数据: FMP compare] | ✓ |

所有竞品对标数据均来自同一次FMP compare调用, 内部一致性100%。

</details>

<details>
<summary>E.3.5 数据异常与局限</summary>

**已知异常**:
1. **P/E双版本**: 报告中出现39.9x(Phase 0时点)和36.1x(Phase 2时点)两个P/E值。这不是数据错误, 而是时间差(TTM P/E随季度更新滚动)。详见附录D.1A。
2. **FMP SBC=$0**: FMP对NVIDIA的SBC报告为$0(已知平台bug), 实际SBC约$4.3B [硬数据: 10-K推算]。所有使用SBC的分析已手动修正。
3. **FY2030E覆盖有限**: 仅12名分析师提供FY2030预估(vs FY2027的35+名), 中位数可靠性较低。

**方法论局限**:
- 五情景概率分配基于历史类比+主观判断, 非统计模型
- CUDA半衰期(5-8年)基于4个类比数据点, 样本量不足
- 推理弹性(1.5-3.0)无直接历史参照, 依赖跨行业类比
