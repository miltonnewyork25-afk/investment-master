...
> **评级**: 审慎关注 [贵×改善中×可能有催化]
> **SOTP概率加权**: $226.6 vs 市价$307.50 = **-26.3%**
> **三维状态**: 价值状态[贵] × 方向状态[改善中] × 催化状态[可能有]
> **护城河**: 3.5/5 | **CQ加权**: 53.4% | **圆桌**: 5/5一致审慎关注
> **黑箱比例**: ~25-30% / 业务复杂度: 4/5 → 区间估值 + 信心度低标注
...
## 0.1 市场现在怎么看COHR，以及这个看法解释不通什么

市场把Coherent当作AI光通信成长股——受益于800G/1.6T升级周期、NVIDIA $2B背书、Hyperscaler CapEx加速，按Forward PE 41x定价，对标FY2027E Non-GAAP EPS $7.47 [DM-VAL-003]。Networking/Datacom贡献69%收入，增速+34% YoY [DM-BIZ-015]。这个框架在三件事上解释不通。

**第一**，COHR 17.5%增速 vs LITE 65.5%增速，Forward PE仅差6x（41x vs 47x）[DM-CORE-001]。如果两家都是纯AI增速故事，市场给COHR每单位增速的溢价是LITE的3倍以上。这意味着COHR的41x中有一部分不是在买AI增速，是在买别的东西。**第二**，31%的收入（Industrial）在萎缩-10% YoY [DM-BIZ-032]，但市场给整个公司一个统一的41x PE，没有对萎缩业务折价——一个正增长+负增长的混合体不应该获得纯增长公司的估值方法。**第三**，FY2025→FY2028E的EPS增量$10.16中，我们拆解后发现$3-4来自非增长因素——D&A递减每年释放~$0.50-1.00 [DM-FIN-008]、利息节省每年+$0.35 [DM-FIN-009]、剥离低margin业务改善OPM——但共识叙事将所有EPS增长归因于"AI收入增长"。

如果继续用"AI光通信成长股"框架看COHR，会抹平三个关键判断：萎缩业务的隐性拖累、EPS改善中非增长因素的贡献被错误归因、以及SiC期权和资本结构事件的估值影响被完全忽略。

## 0.2 实际上是什么：41倍去杠杆

COHR不是AI光通信成长股，而是一台**后合并去杠杆混合体**——三台引擎（AI Networking 69%、Industrial 31%、SiC隐性期权）共享一套晶圆制造基础设施，用D&A递减+债务清偿+混合业务重组机械制造EPS轨迹，但市场用41x Forward PE把它当作纯AI成长股定价。

这个定义成立的核心证据：ROIC 4.2% < WACC 10%，负利差-5.8pp [DM-FIN-026]。一家ROIC低于WACC的公司，每多投入$1资本都在消耗而非创造价值。市场相信"时间换空间"——等AI增长释放ROIC上升——但圆桌5位大师一致指出这个论文的系统性脆弱：等待期本身在消耗价值，2027H1是关键窗口 [DM-RT-007]。

因此，真正决定COHR命运的第一变量不是Networking收入增速（市场默认变量），而是**ROIC何时超过WACC**。增速30%但ROIC<WACC = 在烧钱建产能；增速10%但ROIC>WACC = 开始真正为股东创造价值。估值方法相应切换：从统一Forward PE（隐含整体25%+增长）→分部SOTP（Networking EV/Rev 6x + Industrial EV/EBITDA 10x + SiC概率加权期权）+ 173M稀释股调整 [DM-RT-SUP-013]。

Python SOTP精确计算：Bear $150.7（30%）/ Base $211.8（45%）/ Bull $344.4（25%）→ 概率加权 **$226.6**，vs $307.50 = **-26.3%** [DM-RT-SUP-014]。即使用最乐观的概率组合（Bear 15% / Base 50% / Bull 35%），加权SOTP也仅$249——需要Bull>$500才能justify当前价格，这要求1.6T份额>30%、CPO贡献>$1B、SiC成为行业第二——同时全部兑现的概率<10%。

## 0.3 评级、估值与Kill Switch

**评级：审慎关注**。圆桌5/5一致，零异议 [DM-EXEC-010]。三维状态[贵×改善中×可能有催化]——业务方向是正面的（AI增长+去杠杆+SiC期权），但当前价格已把所有催化剂全额买入，没有安全边际。

**SOTP区间估值**（黑箱25-30%，信心度低）：
- Bear $150.7（-51%）：AI CapEx崩塌 + 标签坍塌至20x PE
- Base $211.8（-31%）：增速从22%降至17-19% + 去杠杆继续
- Bull $344.4（+12%）：1.6T份额>25% + SiC贡献利润 + CPO兑现

**Kill Switch红灯**（任一触发→thesis断裂）：
1. Hyperscaler CapEx同比增速连续两季<+10% → Networking增速将降至个位数
2. COHR Networking收入连续两季YoY<+10% → 标签从AI成长股坍塌为周期混合体
3. 库存$1,848M出现>$200M减值 → FCF进一步恶化 + 去杠杆倒退
4. ROIC在FY2027仍<WACC → "时间换空间"论文系统性失败

**M4标签坍塌概率**：30%±5%，独立于CapEx周期——份额流失+ASP下降+mix恶化也能内生触发 [DM-RT-005]。温水煮青蛙情景40-50%概率，3年累计-36%，年化-14% [DM-RT-014]。

```mermaid
graph TD
    A["当前股价 $307.50<br/>41x FY27E PE"] --> B{"SOTP拆解"}
    B --> C["AI Networking<br/>EV $33B (Base)<br/>6x Rev"]
    B --> D["Industrial<br/>EV $3.5B (Base)<br/>10x EBITDA"]
    B --> E["SiC期权<br/>EV $2.4B (Base)<br/>概率加权"]
    
    C --> F["Equity $36.6B<br/>÷ 173M shares"]
    D --> F
    E --> F
    
    F --> G["Base $211.8/share<br/>vs $307.50 = -31%"]
    
    H["Bear $150.7 (30%)"] --> I["加权 $226.6<br/>-26.3%"]
    G --> I
    J["Bull $344.4 (25%)"] --> I
...
**以后再看COHR，不要先问"Networking增速多少"，要先问"ROIC超过WACC了吗"。**

## 0.4 如何使用这份报告

**90秒读者**：执行摘要0.1-0.3（上面3段）已包含完整判断——评级、估值、Kill Switch。合上报告时记住"41倍去杠杆"这个命名和ROIC vs WACC这个第一变量。

**30分钟读者**：加读Ch1核心争议（为什么市场地图失灵）+ Ch9估值（SOTP三情景）+ Ch14 Kill Switch（什么时候该跑）+ Ch15固化（三个钉子）。

**深度读者**：全文按顺序阅读。Ch2-4是业务理解和资本结构事件——回答"这家公司到底是什么系统"。Ch5-7是护城河和竞争——回答"凭什么拿到收入"。Ch8是财务归因——回答"EPS增长有多少是真实的"。Ch10-11是风险和红队——回答"什么会让thesis断裂"。Ch12圆桌——5位大师从不同角度检验同一结论。

**跟踪者**：A6跟踪指标清单是后续监控的操作手册。每季度检查4个红灯+4个黄灯+4个绿灯信号。

**核心限制**：黑箱比例27%，最大的不确定性来自bookings的firm/soft比例——这决定了bear case是温水煮青蛙（年化-14%）还是Lumentum式断崖（一个季度-20%+）。我们不假装知道答案。

**母图：报告结构导航**

```mermaid
graph TD
    E["0. 执行摘要<br/>41倍去杠杆"] --> C1["1. 核心争议<br/>三引擎一标签"]
    C1 --> C2["2-4. 业务+资本结构<br/>三引擎拆分+去杠杆+稀释"]
    C2 --> C5["5-6. 护城河+竞争<br/>InP垂直整合+CPO演化"]
    C5 --> C7["7-8. 财务归因+剪刀差<br/>EPS工厂解剖"]
    C7 --> C9["9. 估值<br/>SOTP三情景"]
    C9 --> C10["10-11. 风险+红队<br/>R1+R2协同+M4标签"]
    C10 --> C12["12-13. 圆桌+认知边界<br/>5/5一致审慎"]
    C12 --> C14["14-15. Kill Switch+固化<br/>4红/4黄/4绿"]
```
...
> **在"41倍去杠杆"母模型中的位置**：本章回答"COHR到底是什么系统"——三引擎的经济特征差异是SOTP估值的逻辑基础。如果三引擎增速/利润率/周期性相同，统一PE就是合理的；正是因为它们完全不同，统一PE才是定价错位。资本结构事件（preferred转换、NVIDIA投资、D&A递减）则解释了EPS轨迹中非增长因素的贡献——"41倍中有多少是买增长，多少是买去杠杆"的答案在这里。
...
> **在"41倍去杠杆"母模型中的位置**：本章回答"凭什么拿到收入"——护城河评估（3.5/5）直接影响SOTP中各引擎的倍数选择。如果护城河是5/5垄断级，Networking可以给10x Rev；3.5/5的中等护城河意味着6x Rev是合理上限。竞争格局的"三层分化"解释了为什么COHR的估值倍数高于Innolight但低于LITE——市场在给"芯片层壁垒"定价，不是"模块层份额"。
...
> **在"41倍去杠杆"母模型中的位置**：本章回答"什么会让thesis断裂"——R1（AI CapEx下行）+R2（标签坍塌）的协同是最危险组合，概率20-25%，影响-50%+。但更常见的结果是"温水煮青蛙"——渐进恶化不触发止损，3年累计-36%。风险系统的核心发现：COHR的风险不是独立清单，是互相放大的网络。R1触发R2，R2加速R3，R3反馈R2——这种协同结构意味着单一风险的概率低估了系统性风险。Reverse DCF的六个隐含信念中，B4（AI CapEx持续3年+）是最脆弱信念，脆弱度5/5，失败时股价-47%。
...
# P1补充：SiC/OPM深度 + 上行剪刀差/BOM

> **在"41倍去杠杆"母模型中的位置**：P1补充深挖了两个可能改变估值方向的变量——SiC期权价值（Wolfspeed出局后的竞争格局重塑+200mm成本优势）和上行剪刀差（BOM成本下降+ASP预期差+利润率杠杆）。这些是bear/base case向bull case迁移的条件——如果SiC在FY2028贡献利润+800G/1.6T BOM持续下降，SOTP的bull case概率可能从25%上修到30-35%。但当前证据不支持这个上修。
...
> **在"41倍去杠杆"母模型中的位置**：本章是"41倍去杠杆"假说的定量验证。收入归因瀑布揭示80%+增量来自AI单引擎——COHR没有"备用增长引擎"。EPS瀑布解剖证明约48%的GAAP改善来自非增长因素（D&A递减+利息+税率+preferred）。三PE展示暴露Owner PE 93.8x的真实回报率（0.06%）。SOTP三情景概率加权$226.6证明$307.50在任何合理假设下不可justify。这些数字不是我们的判断，是Python模型的输出——我们的判断是概率赋值和倍数选择，模型负责算术。
...
> **在"41倍去杠杆"母模型中的位置**：本章验证了两个核心判断——（1）COHR的竞争优势在芯片层（成本），不在模块层（份额），这解释了为什么800G ASP平稳而非上升，margin expansion来自mix+成本降低而非定价权。（2）CPO不杀InP但改变InP的角色——从EML调制器变成CW外部激光器，BOM占比下降但单位价值上升。InP衬底70%供需缺口+AXT地缘风险使COHR的垂直整合从"成本优势"升级为"战略必需品"——这是41x PE中唯一有坚实基本面支撑的部分。
...
> **在"41倍去杠杆"母模型中的位置**：红队的核心产出是三项修正——概率重校（Bear从25%→30%）、护城河下修（3.8→3.5/5）、以及M4标签坍塌作为独立风险（30%±5%）的发现。P4最重要的新发现是M4可以在CapEx不崩塌时内生触发——份额流失+ASP下降+mix恶化足以让分析师重新审视"这是AI股还是周期混合体"。这意味着即使B4（AI CapEx持续）成立，41x PE仍面临非线性压缩风险。
...
# P4补充：库存Forensic + Bookings质量 + SOTP回流

> **母模型回收**：P4补充是估值修正的最后一步——Python精确计算将SOTP从P4文本的近似值$249修正为$226.6（铁律K要求使用精确值）。差异来源：NVIDIA稀释（173M vs 165M，每share降$12-15）+ Networking下调（rev×mult从$37.7B→$33.0B，每share降$27）+ 概率shift（Bear+5pp，加权再降$3-4）。库存forensic确认了P2的概率加权减值估计（~$90M）是稳健的——II-VI历史减值3-6%在$1.85B基数上对应$56-111M。Bookings质量调查是P4最重要的新增分析——管理层声称"unprecedented visibility"但措辞比LITE（给了月数+到期日）更模糊，与Lumentum 2022年"strong visibility"（后来证伪）高度相似。大量建库存本身暗示bookings的firm比例低——如果真是firm take-or-pay，按需生产即可，不需要$1.85B库存。这是整份报告中黑箱比例最高的变量（贡献黑箱27%中的~10pp），也是bear case速度的关键决定因素。
...
# 14. Kill Switch：4红/4黄/4绿

## 14.1 红灯信号（任一触发→thesis断裂）
...
COHR不是AI光通信成长股。它是一台**后合并去杠杆混合体**，三引擎（AI 69% + Industrial 31% + SiC期权）用D&A递减和债务清偿制造EPS轨迹。市场用41x PE买下这台机器，把资本结构改善伪装成的EPS增长也按AI增速定价——这就是"41倍去杠杆"。

## 15.2 第一变量：以后该盯什么

市场盯Networking收入增速。我们认为真正决定价值的是**ROIC vs WACC差值**（当前-5.8pp）。增速30%但ROIC<WACC = 烧钱建产能。增速10%但ROIC>WACC = 开始为股东创造价值。跟踪：每季度计算NOPAT÷Invested Capital vs WACC。

## 15.3 新估值语言：以后该用什么方法定价

不要再用统一Forward PE给COHR定价——41x PE给萎缩的Industrial也按AI倍数定价。应该用**分部SOTP**：Networking EV/Rev（6-10x）+ Industrial EV/EBITDA（8-12x）+ SiC概率加权期权 + 173M稀释股调整。SOTP的关键参数：Networking EV/Rev倍数（当前base 6x）、Bear概率（当前30%）、稀释股数（当前173M，FY28E ~185M）。

## 15.4 迁移问题：看下一家类似公司时该问什么

看下一家后合并科技混合体时，必问：
1. **EPS增长中多少来自非增长因素（D&A递减/利息节省/mix改善/税率正常化）？** 如果>30%，PE需要打折——你在用成长股倍数买会计正常化。
2. **ROIC vs WACC差值是多少？** 如果ROIC<WACC，"等催化剂"的成本不是零——每等一年消耗WACC-ROIC的隐性成本。
