...
**原因**: ATE是设备（一次性CapEx购买），探针卡是消耗品（跟随wafer start持续替换）。AI芯片第一批部署需要大量ATE首次配置，探针卡需求只跟随wafer start——ATE增速的"初始效应"不会传导到探针卡。

**投资含义**: 用Teradyne/Advantest的增速外推FORM的增速 = 系统性高估。探针卡增长更接近DRAM wafer start增速（~10-15%），不是ATE增速（31%）。市场混淆两者是FORM估值膨胀的原因之一。
