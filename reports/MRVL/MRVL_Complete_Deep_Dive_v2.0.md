# Marvell Technology (MRVL) 深度研究报告 v2.0

> **评级**: 中性关注(偏审慎) | **公允价值**: $80-85 | **当前股价**: $94.88(2026-03-30)
> **期望回报**: -10%至-15% | **分析师**: 投资研究Agent | **框架版本**: v20.0
> **数据截止**: FY2026 Q4(2026-01-31) | **修正EPS**: $4.34(P4修正) | **DM门控**: ≥1.5/千字
> **v2.0变更**: 结构重组(附录分离)+OPM统一(36.5%)+KS标准化(8个)+周期独立章

**报告结构**: 正文(第一~十部分) → 深度补充(A-X) → 研究底稿附录(Phase 1-4)。正文数字为最终版(P4修正后)。
...
| **KS-6: MediaTek ASIC份额>15%** | MediaTek ASIC收入>$8B(CY2028) | 远(当前~$1B) | ASIC竞争格局不可逆恶化→下调custom silicon SOTP倍数 |
| **KS-7: FY2027回购<$0.5B** | SBC覆盖率<80% | 中(Celestial $3.25B挤压) | Owner DCF假设失效→FV从$93收敛至$74(GAAP) |
| **KS-8: Celestial FY2028收入<$100M** | vs管理层$500M ARR目标miss>80% | 中远(FY2028才验证) | $3.25B投资回报存疑→管理层资本配置评分下调 |

#### Kill Switch 12字段标准化 (v2.0新增)

| 字段 | KS-1 AI急刹 | KS-2 MSFT转走 | KS-7 回购缩减 |
|------|-----------|-------------|-------------|
...
| **依赖KS** | 独立 | 与KS-1协同 | 与KS-4协同(Celestial挤压) |
...
## 第四B部分: 周期定位独立结论 (v2.0新增)

### 16B. MRVL周期定位: AI CapEx驱动, 非WFE (独立结论)

**核心判断**: MRVL的周期位置不由WFE(晶圆制造设备支出)决定, 而由AI CapEx决定。这是MRVL与设备股(KLAC/LRCX/AMAT)的根本区别——分析MRVL时应完全忽略WFE周期信号。

**SEMI-L1~L7读数汇总与结论**:

| ID | 指标 | MRVL读数 | 信号 | 对MRVL相关度 | 理由 |
|----|------|---------|------|:----------:|------|
| SEMI-L1 | DIO | 126天(↑) [DM-FIN-025] | ⚠️ | **低** | Fabless存货=成品芯片, 非原材料; DIO↑反映备货量产 |
| SEMI-L2 | CapEx/D&A | 0.27x [DM-FIN-010] | ✅ | **低** | Fabless极低CapEx, 指标不适用 |
| SEMI-L3 | 订单积压 | 18 programs [DM-BIZ-013] | ✅✅ | **★高** | 直接反映custom silicon需求管线 |
| SEMI-L4 | DRAM/NAND价格 | 稳定偏强 | ✅ | 低 | MRVL不做存储, 间接受益HBM需求 |
| SEMI-L5 | WFE | 第3年增长 | ⚠️ | **低** | MRVL是fabless, 不买设备 |
| SEMI-L6 | AI CapEx | >$470B(加速) | ✅✅ | **★★高** | MRVL收入的直接驱动力 |
| SEMI-L7 | 地缘 | 台海3.6% [DM-PMK-002] | → | 中 | 38%中国收入但主要是ship-to |

**周期位置判断**: AI CapEx周期处于**early-to-mid cycle**(2024-2026是第一波大规模AI基础设施投资 [DM-P4-026])。Mid-cycle风险可能在2027-2028(ROI验证期)——如果hyperscaler发现AI投资ROI不及预期→CapEx增速可能从+30%降至+10%(减速但不下降)→对MRVL收入增速影响: 从+30%降至+15%。

**PEP检测结果**:
- PEP-005(催化剂金字塔): ❌ 不适用(MRVL非转型公司, 已有量产收入)
- PEP-006(周期峰值溢价): ❌ 不适用(Forward PE 17.5x远未到历史高位25x+)
- PEP-007(反转PE陷阱): ❌ 不适用(MRVL非存储/商品周期股)

**周期对估值的影响**: 当前AI CapEx early-to-mid cycle→Forward PE 17.5x合理(不需要mid-cycle调整)。但如果AI CapEx在FY2028进入late cycle(增速<+5%)→Forward PE应下调至14-15x(反映增速放缓)。这是KS-1(AI CapEx急刹)的远期版本。

```mermaid
graph LR
    subgraph "AI CapEx周期位置"
    A["Early<br>2023-2024<br>基础设施建设"]
    B["★Mid<br>2025-2026<br>训练+推理规模化"]
    C["Late<br>2027-2028?<br>ROI验证期"]
    D["Downturn<br>仅AI急刹时<br><10%概率"]
    end
    A --> B --> C --> D
...
### 16C. 共识EPS分解与分歧解释 (v2.0新增)

我们的FY2028E EPS $4.34 vs 共识$5.43 — 差距20%。分解:

| 项目 | 我们(P4修正) | 共识(28位中位) | 差距 | 分歧原因 |
|------|-----------|-------------|------|---------|
| **Revenue** | $12.5B | $14.9B | -16% | Custom silicon: 我们$2.5B vs 共识~$4B(Amazon缺口) |
| **Non-GAAP GM** | 57.5% | 59% | -1.5pp | 我们假设custom silicon mix稀释更大 |
| **Non-GAAP OPM** | 36.5% | 37-38% | -0.5~1.5pp | 我们的"第三条路"vs共识乐观 |
| **税率** | 10% | 10% | 0 | 一致 |
| **Shares** | 855M | 850M | +0.6% | 几乎一致 |
| **Non-GAAP EPS** | **$4.34** | **$5.43** | **-20%** | **收入分歧贡献80%+** |

**为什么收入分歧如此大?** 共识$14.9B隐含custom silicon FY2028达$4B+(管理层目标打9折)。我们的$2.5B隐含Amazon缺口$750M-$1B [DM-P4-008]无法被MSFT+emerging完全弥补。

**谁更可能对?** FY2027 Q1(2026-05)是第一个验证点:
- 如果Q1 custom silicon >$500M(QoQ +15%): 共识更可能对→FV上调至$85-90
- 如果Q1 custom silicon <$400M(QoQ -5%): 我们更可能对→FV维持$80-81
- 共识下修通常发生在公司连续2Q miss后→FY2027 Q1-Q2是关键窗口

**分歧对估值的影响**: 收入每变动$1B→FV变动$5-7/股。因此$2.4B的收入分歧→FV差距$12-17/股——这解释了为什么我们的FV $80-85 vs 共识隐含$95(当前股价)有~$10-15的差距。
...
### 参考: MediaTek-Google联盟详细分析 (Phase 4 研究)
...
### 参考: Forward PE估值与共识冲突 (Phase 2 研究)
...
| **Non-GAAP EPS** | **$2.86** (P2初版, 已被P4修正为$4.34) | |

> **P2→P4修正说明**: P2初版EPS $2.86基于保守OPM 27%(GAAP视角)。P3发现"第三条路"OPM ~36.5%(Non-GAAP)后, P4修正EPS至$4.34。以下P2原始分析保留供参考, 正文结论使用P4修正值。

P2初版EPS $2.86低于分析师共识$5.43 [DM-CON-004]。差距巨大(47%)。
...
### 参考: 估值更新与Python DCF验证 (Phase 4 研究)
...
> **以下为Phase 1-4研究过程底稿**, 保留供研究复盘参考。**正文结论以上方第一-十部分+深度补充为准**。底稿中的估值数字/概率/OPM等如与正文不一致, 以正文(P4修正后)为准(铁律K)。

---

## 附录A: Phase 2 估值研究底稿

> **注**: 以下为Phase 2研究过程记录。正文结论以第六部分为准。与正文数字不一致处以正文为准(铁律K)。
...
> **注**: 以下为Phase 3研究过程记录。正文结论以第三部分为准。
...
> **注**: 以下为Phase 1研究过程记录。正文结论以第二部分为准。
...
> **注**: 以下为Phase 3竞争分析过程记录。正文结论以第五部分为准。
...
## 附录G-ext: Phase 2 估值底稿

> **注**: 以下为Phase 2估值模型记录。正文结论以第六部分为准。
...
## 附录H-ext: Phase 3 护城河底稿

> **注**: 以下为Phase 3护城河量化记录。正文结论以第三部分为准。
...
**[报告结束 — Marvell Technology (MRVL) 深度研究报告 v2.0]**
