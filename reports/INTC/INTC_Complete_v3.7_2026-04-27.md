...
**今日 PV 区间**: $18-25 (主锚 $19.76, 8% WACC 折现) / **5 年退出价区间**: $26-35 (加权 $29)
**评级**: 审慎关注 (高争议)
**5 年退出价期望回报**: 约 -65% (基于 5y exit value 加权 $29 vs $82.57)
**今日 PV 折现后隐含 downside**: 约 -76% (基于 today PV 主锚 $19.76 vs $82.57)
...
**市场为什么兴奋**. Intel 在过去 13 个月把股价从 2025 年 3 月低点的 $19 推到 2026 年 4 月 24 日收盘的 $82.57, 涨幅 +335%. Q1 2026 财报 (2026-04-23 release) 给了市场新弹药: 收入 $13.6B 同比 +7%, DCAI $5.1B 同比 +22% (5 年来最强 server 增长), Non-GAAP gross margin 41.0% 同比 +430bp; DCAI 端 Xeon 6 被 NVIDIA DGX Rubin NVL8 选为 host CPU (这是 DCAI / Xeon 产品线胜利, 不是 Foundry external customer 订单), Google Cloud 公开 Xeon 6 多年合作; CHIPS Act 含 $7.86B direct funding + $3B Secure Enclave 合同 + 25% investment tax credit + Trump 政府 10% 直接持股 (~$36B 估值, 持有 433.3M shares + 可按 $20/share 行权额外 5% 的 warrants).

**我们为什么仍然不买**. Intel Foundry external revenue 仅 $174M (季度年化 <$1B, 距离市场假设 5 年累计 $20B+ 差 75%+); Foundry segment operating loss -$2.4B; Q1 GAAP EPS -$0.73 含 impairment / restructuring ~$3-4B 一次性 (operating recovery 与 accounting clean-up 必须分开看); ROIC 仍是 1-4% 远低于 WACC 8%, 5y 内追上 WACC 概率 < 15%; 当前 trailing P/Sales 7.5x 显著高于半导体周期股历史顶部 (4-5x). 即使把 Q1 的所有正面信号都纳入, 估值已经反映过多转型成功假设.

**估值结论**. 三种独立方法都指向"显著低于 $82.57" 的方向, 但区间宽度大. 我们把 DCF today PV $18-25 作为主锚 (中点 $19.76), SOTP $3-15 作为下沿压力测试, peer multiple $17-33 作为上沿 sanity check. 5 年退出价 (Bear $11.5 × 37.5% + Base $31.5 × 47.5% + Bull $65 × 15%) 加权 $29.02, 区间 $26-35.

**行动**. avoid / watch / wait for reset, 不是 high-conviction SELL. Q1 2026 partial 削弱单边 SELL 论点 (DCAI +22%, NVIDIA Rubin Xeon 6, Google Cloud 合作). 加上政府介入提供下行保护, lift size 难把握 (历史半导体周期股 SELL 通常 -30% 后 dead cat bounce +20-30%), 单边 SELL 赔率从早期估算 80%/20% 下降到 65%/35%. 4 种主流投资风格 (质量投资 / Special situations / Deep value / Long-short) 都不建议 BUY, 但只有 1/4 (Long-short) 主动 SELL with caveat, 其余 3/4 是 avoid 或 WATCH. 这种"4/4 不 BUY 但只有 1/4 SELL" 的分歧反映高争议状态.

**5 个触发条件 (6-12 个月监控)**. DCAI 连续 3 季度 +20%+ YoY (确认 trajectory 转折); Foundry quarterly external revenue 突破 $500M (商业化进度 confirm); 18A yield 公开数据 >70% in 2026 H2 (Intel 主动披露); AMD Q1 2026 release / NVIDIA Vera reference design 公布; CHIPS Act / 政府持股政策变化. 3 个或以上 catalyst 在 6 个月内联合 fire 概率 40-50%. 任一显著触发, 我们立即写 v3.7, 不在 v3.6 上叠补丁.

**版本说明**. 这是 v3.6, 在 v3.5 基础上做 6 项必改 (DCAI vs Foundry 订单措辞 / SOTP 现金债务表述 / 三方法 convergence 表述 / 18A yield 推断强度 / 政府 warrants / Q2 GAAP+Non-GAAP guidance) + 4 项可选优化 (执行摘要压缩 / "反护城河"措辞 / "1.6 ppm" 伪精确 / 附录精简). 数据截止 2026-04-27, 涵盖 Intel Q1 2026 actual (2026-04-23 release).
...
| 方法 | 公允估值 | 中位 | 在结论中的角色 |
|------|---------|------|-------------|
| SOTP | $3-15 | $8-10 | 下沿压力测试 |
| 三情景概率加权 DCF (today PV) | $18-25 | $19.76 | **主锚** |
| Peer multiple 加权 | $17-33 | $24 | 上沿 sanity check |

三种方法都指向"显著低于 $82.57" 的方向, 但区间宽度大. 真实情况不是"三种方法严格 converge 到 $15-22", 而是: 保守 SOTP 给出 $3-15 (反映 Foundry segment 在 base case 下贡献负值, 假设较严格), DCF 主锚给出 $18-25 (含 unconditional upside 项如政府 puts adjusted, spinoff 期权, supply 红利), peer multiple 给出 $17-33 (上沿 $33 反映 INTC 历史周期中位 P/Sales 3.5x 应用到 FY2025 revenue). 三种方法的共同结论是当前 $82.57 显著高于合理区间, 但合理区间本身有 $3-33 的宽度.

我们把 DCF today PV $18-25 作为主锚, 把 SOTP 当作下沿压力测试 (如果 Foundry NPV 真的全部归零 + All Other 估值偏低), 把 peer multiple 当作上沿 sanity check (如果用 INTC 历史周期顶部倍数). 主锚选 DCF 是因为它把 Bull / Base / Bear 三情景的概率分布显式列出, 比单一倍数法更透明. 这与当前股价 $82.57 比较, downside 是 -68 to -78% (取主锚 $18-25), 不是 v3.5 写的 -76 to -82% (那个是用 $15-22 区间得出的偏低估算).
...
我们承诺以下任一发生 → 立即写 v3.8, 不在 v3.7 上叠补丁:
...
本版为 **v3.7, 数据截止 2026-04-27**. 关键未验证变量 (任一显著触发将更新到 v3.8): **Q2 2026 actual results** (7-8 月 release vs guidance revenue $13.8-14.8B / GAAP EPS $0.08 / Non-GAAP EPS $0.20), **AMD Q1 2026 release** (2026-04-29, 三路径预测 Path A 80% / Path B 15% / Path C 5%), **18A yield 实际数字** (Intel 主动披露 vs 维持"on track" 表述), **NVIDIA Vera reference design** (2026 Q3-Q4 GTC reveal vs 当前 Rubin NVL8 选 Xeon 6 partial reverse), **Intel Foundry external wafer pull** (Microsoft Cobalt 2 LOI 转 binding PO, Apple A20 NDA 公开化等). 触发后立即 update, 不在 v3.7 上叠补丁.

完整 v3.6 与 v3.5 修正对比 + 给付费读者诚实评估见**附录四 (Source Room: 版本演进与方法学说明)**.
