# Intel v4.4: Agentic AI 给了 Xeon 新叙事, 但 $82.57 仍买入太多成功
...
**评级语言定位**: 审慎关注 (高争议) 在框架标准评级表中的位置 — 介于"中性关注" (合理估值, ±10% 区间) 与"标准审慎关注" (-10% 以下 + conviction 强) 之间. "(高争议)" 反映 4/4 不 BUY 但 0/4 主动单边 SELL 的分歧状态. 行动 avoid / watch / wait for reset, 不是单边 SELL.

**市场为什么兴奋**. Intel 在过去 13 个月把股价从 2025 年 3 月低点的 $19 推到 2026 年 4 月 24 日收盘的 $82.57, 涨幅 +335%. Q1 2026 财报 (2026-04-23 release) 给了市场新弹药: 收入 $13.6B 同比 +7%, DCAI $5.1B 同比 +22% (5 年来最强 server 增长), Non-GAAP gross margin 41.0% **同比 +180bp** (vs Q1 2025 39.2%, 不是 +430bp — 后者是 vs FY2025 全年 36.7% 的口径错置); DCAI 端 Xeon 6 被 NVIDIA DGX Rubin NVL8 选为 host CPU (这是 DCAI / Xeon 产品线胜利, 不是 Foundry external customer 订单), Google Cloud 公开 Xeon 6 多年合作. **新增叙事**: Morgan Stanley Tech Research 框架图显示 AI workflow 从 chatbot → RAG → coding assistant → multi-tool agent → research agent → complex orchestration, CPU processing 在总 latency 中占比从 ~15% 升至 ~92%; Georgia Tech / Intel 论文 (《A CPU-Centric Perspective on Agentic AI》) 验证 CPU-side tool processing 在某些 agentic workloads 中可占 90.6% 总延迟. 这给 DCAI Q1 +22% 提供了更大的解释框架 — 可能不只是周期反弹, 也是 agentic AI 让 CPU attach rate / utilization / ASP 重新上行的早期信号.

**我们为什么仍然不买**. (i) Intel Foundry external revenue 仍仅 $174M (季度年化 <$1B); Foundry operating loss -$2.4B; Foundry NPV 加权 -$9/share. CPU bottleneck 叙事不解决 Foundry 商业化问题. (ii) CPU 受益 ≠ Intel 独享 — AMD EPYC / AWS Graviton / Microsoft Cobalt / Google Axion / NVIDIA Grace+Vera / Arm ecosystem 都受益. (iii) Latency share ≠ hardware revenue share — 很多 CPU latency 来自 API waiting / I/O / sandbox startup / Python 执行, 可通过软件调度 (论文已展示 micro-batching speedup) 解决, 不一定全部转化为 "多买 Xeon CPU". (iv) 当前 $82.57 已经提前买了大量成功 — 即使把 DCAI bull case 概率上修至 20%, today PV 仍只到 $25.5 (vs $82.57 = -69%).

**估值结论**. 三种独立方法都指向"显著低于 $82.57" 的方向, 但区间宽度大. 我们把 DCF today PV $23-28 作为主锚 (中点 $25.5), SOTP $4-18 作为下沿压力测试 (反映 DCAI segment 估值), peer multiple $20-38 作为上沿 sanity check (反映 ARM / Foundry 略改善). 5 年退出价 (Bear $11.5 × 32.5% + Base $34 × 47.5% + Bull $68 × 20%) 加权 $33.5, 区间 $30-40.

**行动**. avoid / watch / wait for reset, 不是 high-conviction SELL. SELL conviction 弱化 (赔率从早期 80%/20% 降至 55%/45%). 4 种主流投资风格全部不 BUY, **0/4 主动单边 SELL** (Long-short 从 "SELL with caveat" 降温到 "tactical short only / lower conviction", 因 agentic CPU narrative 提高 short squeeze 风险). 这是 0/4 BUY + 0/4 单边 SELL + 4/4 avoid 或 watch 的分歧状态, 对应**审慎关注 (高争议)** 评级.

**6 个触发条件 (6-12 个月监控)**. DCAI 连续 3 季度 +20%+ YoY (确认 trajectory 转折); Foundry quarterly external revenue 突破 $500M (商业化进度 confirm); 18A yield 公开数据 >70% in 2026 H2; AMD Q1 2026 release / NVIDIA Vera reference design 公布; CHIPS Act / 政府持股政策变化; **Agentic CPU attach-rate trigger**: AI server / agentic workload 的 CPU:GPU attach ratio 明显上升 + Intel Xeon 在该增量中保持/提升 share + DCAI ASP 上升 (不只是 volume) + hyperscaler 增加 Xeon-based agent orchestration clusters. 任一显著触发, 立即更新下一版.

**数据时效**: 本报告数据截止 2026-04-27, 涵盖 Intel Q1 2026 actual (2026-04-23 release) + Morgan Stanley CPU latency share 框架图 + Georgia Tech / Intel CPU-centric agentic AI 论文. 完整版本演进 (v3.0 → v4.3 修正历史) 移到 Source Room (附录四), 不进入主阅读流.
...
第二条, GM 改善 +10pp (35% → 45%). 历史基准率 15-20%. 18A 量产 + scale 可能. Q1 2026 Non-GAAP GM 已经从 Q1 2025 的 39.2% 升到 Q1 2026 的 41.0% (**+180bp YoY**, 注: 这是真实 YoY 口径, 不是 +430bp; +430bp 是 vs FY2025 全年 36.7% 的口径错置). 但需要持续到 FY2026 全年才能算 GM trajectory 转折.
...
收入 $13.6B 同比 +7%. GAAP EPS **-$0.73** (含 impairment / restructuring 约 $3-4B 一次性). Non-GAAP EPS $0.29 同比 +123% (反映 underlying operating). **GAAP gross margin 39.4% 同比 +250bp** (vs Q1 2025 36.9%). **Non-GAAP gross margin 41.0% 同比 +180bp** (vs Q1 2025 39.2%). 注: 若与 FY2025 全年 Non-GAAP GM 36.7% 比较则改善 +430bp, 但那不是 YoY 口径 (Q1 2025 vs Q1 2026), 是单季度 vs 上年全年, 应区分使用.
...
- Q1 Non-GAAP gross margin 41.0% 同比 +180bp (vs Q1 2025 39.2%; 注: 与 FY2025 全年 36.7% 比较则 +430bp, 但那不是 YoY 口径), 削弱"GM 结构性收缩" 论点 (但需要持续到 FY2026 全年)
...
**SOTP 一表汇总** (按 4.32B 摊薄股本, **当前估值锚 (v4.4)反映 DCAI 受 agentic CPU bull case 上修**):
...
四种 peer multiple 计算 (**v4.4 简化表述**, 全部含 agentic CPU partial premium +10%):

| Peer 方法 | Base P/Sales | + agentic premium | INTC equity | Per share |
|----------|-------------|------------------|-------------|-----------|
| IDM 同业中位 (Samsung Semi + SK Hynix avg) | 2.15x | 2.37x | $84B | **$19** |
| Foundry 同业中位 (GlobalFoundries + UMC avg) | 2.85x | 3.14x | $124B | **$28** |
| INTC 自己历史周期中位 (主区间 3.5-4.0x, 中点 3.75x) | 3.75x | 3.75x (历史已含 cycle premium) | $157B | **$36** |
| ROIC 调整 (3.5x × (1-30% reverse-moat discount) = 2.45x) | 2.45x | 2.7x | $102B | **$23** |

**4 方法等权平均**: ($19 + $28 + $36 + $23) / 4 = **$26.5/share** (中位).

**主区间表达 $20-38**:
- 下沿 $20 ≈ IDM peer ($19) 取整, 反映"如果 Intel 完全没有 agentic CPU premium, 仅作 IDM 同业估值"
- 上沿 $38 ≈ Foundry peer ($28) + INTC 历史周期中位 ($36) 上沿, 反映"agentic CPU partial validation + 历史周期顶部"
- 中位 $26-28 与 today PV 主锚 $25.5 接近, 形成 cross-validation

**$45 的处理**: INTC 历史周期顶部 P/Sales 4.5x (而非中位 3.75x) 算出 $45/share, 这是 full validation pressure-test, **不进主区间**, 仅作 sensitivity analysis (对应 §14.7 验证四阶段表的 "full validation" 阶段). 主区间统一 $20-38.

注: peer multiple 比 today PV 略高, 因为它没有反映 Intel 当前正在亏钱 (operating loss 比 peers 高). 所以我们采用 today PV $23-28 (v4.4 主锚) 作为更准确的估值锚, peer multiple $20-38 作为上沿 sanity check.
...
### 3.5 三方法 cross-validation (当前估值锚)

| 方法 | 公允估值区间 | 中位 | 在结论中的角色 |
|------|------------|-----|-------------|
| SOTP | **$4-18** | $10-12 | 下沿压力测试 (反映 DCAI segment 估值改善) |
| 三情景概率加权 DCF (today PV) | **$23-28** | $25.5 | **主锚** (反映 bull 概率 20% + 5y exit $33.5) |
| Peer multiple 加权 | **$20-38** | $26-28 | 上沿 sanity check (含 agentic CPU partial premium) |
...
### 3.6 反向 stress test: 当前 $82.57 隐含什么假设 (当前估值锚 (v4.4), 从 $25.5 出发)

倒过来问: 当前 $82.57 隐含的 5 年假设是什么? 这是检验估值合理性的关键步骤. v4.3 从 v4.4 主锚 today PV $25.5 出发计算, 不再用 v3.7 的旧值 $19.76.
...
Foundry 是当前估值争议最大的 segment. 市场默认给 Foundry segment 隐含估值 +$30/share (按 TSMC-like multiple). 我们的估算是 -$9 to -$5/share. 差距 -$35 to -$39/share, 解释当前股价 $82.57 vs 公允 $25.5 (v4.4 主锚) 之间 gap 的相当大部分. 这一节深入这个差距.
...
期权值: 12.5% × $15 = $1.88/share. 如果 KS-spinoff trigger fire (Tan 公开转向 / 投行 pitch / Board strategic review), 概率 jump 至 35%, 期权值跳升至 $5.25/share, 新增约 **+$3.4/share**. 在 v4.4 主锚 $25.5 下, 公允可能上修至约 **$28-32**. 评级维持审慎关注 (高争议), Long-short 仍是 tactical only. 但这仍显著低于 $82.57, **不是 BUY signal**.
...
Path A (高概率 80%, Polymarket 78%): AMD beat consensus EPS $1.45+, server share +1-2pp. Intel bear thesis 维持, 我们今日 PV **$25.5** (v4.4 主锚) 不变. 评级反应: 维持审慎关注 (高争议). 股价反应: AMD +5-8% (intraday), INTC -3-5% (sympathy).
...
Foundry spinoff 期权 (校准 12.5% × $15 = $1.88/share) 不大但事件触发后估值跳升. Trigger fire 后概率 jump 至 35%, 期权值 $5.25/share. 在 v4.4 主锚 $25.5 下, 公允可能上修至约 **$28-32** (新增 +$3.4/share).
...
当前 $82.57 vs 清算 floor $8-25 = -70 to -90% 下行空间. 公允 $25.5 (v4.4 主锚) vs 清算 floor = +2 to +220% (公允锚位于清算上限以上).
...
反身性反向链: 任一 catalyst miss → 客户信心降 → 18A 客户流失 → Foundry NPV 下修 → 股价进一步跌 → 信用评级下调 → 反身性 reset 到 $25-40 (当前估值锚 (v4.4), vs v3.7 估算 $20-35).
...
### 8.5 4 视角综合 (当前估值锚 (v4.4), Long-short 降温)
...
| Tan spinoff trigger fire | 立即重做估值, spinoff prize +$3.4/share (12.5%×$15→35%×$15=$5.25), 公允上修至 **$28-32** (v4.4 主锚 $25.5 + $3.4) |
...
我们承诺以下任一发生 → 立即写 v4.1, 不在 v4.4 上叠补丁:
...
如果回答"不合理", 需要给出比 **$25.5** (v4.4 主锚) 显著更高的公允价值, 但用不依赖三层叙事 (AI 回归者 / 政府 puts / Tan 奇袭) 的硬数据支撑.
...
### 11.3 Q2 2026 release 后会改变什么 (当前估值锚 (v4.4), 用 today PV $25.5 出发)
...
**Q2 actual miss guidance (Non-GAAP EPS < $0.15)**: Q1 反弹被证明是单季度 noise. 我们会下修 Bear 概率从 32.5% 至 40-45%, 公允从 **$25.5 (v4.4 主锚) 下修至约 $20-23** (Bear 概率 +5-10pp 拉低加权 5y exit). 评级强化"审慎关注 (高度高估)", 部分视角触发 short consideration. KS-DCAI 同时检查.
...
| Q1 2026 | +7% | +22% | +123% | +180bp YoY (Non-GAAP GM 39.2% → 41.0%) |
...
### 11.5 v4.3 base case 对 Q2 的暗含假设 (替代 v3.6 旧版本)

我们 v4.4 base case (5y exit **$34**, 概率 47.5%, 中点已 v4.0 重锚反映 agentic CPU partial) 暗含的 Q2-Q4 2026 假设:
...
风险点: Intel Foundry 5 年内"放弃 leading-edge" 概率 30-40%. 如果发生, Foundry NPV 进一步恶化 -$5 to -$10/share, 公允从 **$25.5 (v4.4 主锚) 下修至约 $18-20**.
...
**本报告数据截止 2026-04-27**. 若以下任一关键变量触发, 将更新下一版:

- **Q2 2026 actual results** (7-8 月 release vs guidance revenue $13.8-14.8B / GAAP EPS $0.08 / Non-GAAP EPS $0.20)
- **AMD Q1 2026 release** (2026-04-29, 三路径预测 Path A 80% / Path B 15% / Path C 5%)
- **18A yield 实际数字** (Intel 主动披露 vs 维持"on track" 表述)
- **NVIDIA Vera reference design** (2026 Q3-Q4 GTC reveal vs 当前 Rubin NVL8 选 Xeon 6 partial reverse)
- **Intel Foundry external wafer pull** (Microsoft Cobalt 2 LOI 转 binding PO, Apple A20 NDA 公开化等)
- **Agentic CPU attach-rate** (Xeon 在 AI server / agentic workload 中的 share + ASP + DCAI margin 持续改善)

完整版本演进 (v3.0 → v4.3) + 各版本修正对比 + 给付费读者诚实评估 + 7 个方法学教训, 移到 Source Room (附录四).

---

## 第十四部分: Agentic AI 的 CPU bottleneck — DCAI 新 bull case, 但不是 Foundry 的免死金牌
...
|------|----------|--------------|------------|---------|
| **Narrative (当前位置)** | Morgan Stanley 框架图 + Georgia Tech / Intel CPU-centric paper | DCAI bull case 成立, 但只是叙事 | +$3-6 PV (已含在 v4.4 主锚 $25.5) | 维持 avoid / watch (高争议) |
...
> **重要声明 (v4.3 新增)**: 下面的 [DM-XXX] reference 列表是为内部 audit 用途, 包含**两类**条目: (a) **当前 v4.3 模型使用的数据点** (Intel FY2025 10-K, Q1 2026 release 等), (b) **早期 v3.x / v4.0-v4.2 估值的历史引用** (例如 DM-VAL-DCF-001 旧版 5y exit $29 / today PV $19.76, DM-AUX-* 系列旧 Bear 37.5% × $5 等). 后者标记为 **DEPRECATED / Historical reference only**, 不是当前模型的一部分, 仅供方法论 audit 追溯使用. **当前主锚是 today PV $25.5 (区间 $23-28) / 5y exit $33.5 (区间 $30-40)**, 详见 §3.2 + DM-VAL-DCF-002.

下面的 reference list 是学术论文的完整版, 每个 reference 标注数据点 + 来源. 主文读者不需要逐条阅读.
...
[DM-INTC-Q1-26-006 Q1 2026 Non-GAAP gross margin 41.0% (+180bp YoY vs Q1 2025 39.2%)]
...
[DM-VAL-DCF-001-DEPRECATED 旧版 v3.7 三情景 5y exit $29 / today PV $19.76 — 仅历史参考, 当前主锚见下方 DM-VAL-DCF-002][DM-VAL-DCF-002 v4.3 三情景概率加权 DCF: Bear 32.5% × $11.5 + Base 47.5% × $34 + Bull 20% × $68 = 5y exit $33.49 / today PV $25.5 (区间 $23-28)]
...
**第 2 项**: 版本号残留清理. v3.6 多处仍写"立即写 v4.1, 不在 v3.5 上叠补丁" (从 v3.5 复制过来未改), 现统一改为"立即写 v4.1, 不在 v4.4 上叠补丁".
...
为了让 Q1 2026 的"DCAI +22% / Non-GAAP GM +180bp YoY / Xeon 6 选为 NVIDIA Rubin NVL8 host" 这组数据有 historical context, 我们对比历史上 4 个半导体公司的 turnaround 第一季度 vs 后续 trajectory.
...
我们 base case (**47.5% Base / 5y exit $34**, v4.4 重锚) 隐含的是 "AMD 2017 路径但更慢" — INTC 在 6-12 个月有 1-2 setback quarter, 然后 trajectory 改善但未达到 pre-2020 peak. Bear case (**32.5% / 5y exit $11.5**, v4.4 重锚) 隐含的是 "IBM 2014 路径" — single segment 强但整体仍下滑. Bull case (**20% / 5y exit $68**, v4.4 重锚, 含 agentic CPU partial validation) 隐含的是 "AMD 2020 + Zen 2 路径" — 18A 真正 deliver + Foundry external 突破 + 多 segment 同步加速.
...
A: Bloomberg consensus $112 反映 sell-side analyst 平均 target. Sell-side analyst 的 target 通常 (i) 跟随 momentum (股价涨他们追涨 target), (ii) 受 IB 业务利益冲突 (公司客户关系), (iii) 12-mo horizon 而不是 5y horizon. 我们的 today PV $25.5 (v4.4 主锚, 区间 $23-28) 是 8% WACC 折现 5y exit $33.49 后的现值, 反映 5y 时间窗口 + 三情景概率加权 + agentic CPU bull case 概率上修. 两者不可直接比较 — 时间 horizon 不同 (12-mo vs 5y), 方法不同 (单点 target vs 概率加权), 角度不同 (sell-side momentum-following vs buy-side fundamental). 我们建议读者把 Bloomberg consensus $112 当作"市场短期共识情绪指标", 把我们的 $25.5 当作"5y 概率加权公允估值". 两个都有用, 不互相替代.
...
A: 会, 但只有 Path C (5% 概率) 显著改变. Path A (80% 概率, AMD beat consensus): KS-AMD 维持 87.5%, INTC bear thesis 维持, 我们 today PV $25.5 (v4.4 主锚) 不变. Path B (15% 概率, AMD in-line): 公允微调 +$1. Path C (5% 概率, AMD miss): KS-AMD 大幅下降, INTC bear weakened, 公允可能上修至 $30-38, 评级从"高争议" 改为"标准审慎关注". 4-29 release 后 24 小时内我们必须回填. 如果 Path C 实现, 我们立即更新下一版.

**Q9: 你提到 Tan spinoff trigger fire 后公允上修, 这是 actionable BUY signal 吗?**

A: 不是 BUY signal. 在 v4.4 主锚 $25.5 下, 如果 spinoff trigger fire (Tan 公开转向 / 投行 pitch / Board strategic review), 期权概率从 12.5% jump 至 35%, 期权值跳升 $1.88 → $5.25 = **新增 +$3.4/share**, 公允可能从 $25.5 上修至**$28-32**. 但即使 trigger fire, 公允 $28-32 仍远低于当前 $82.57 (-61 to -66% downside). Spinoff trigger 是"维持审慎关注 (高争议) 但 SELL conviction 进一步弱化, Long-short 维持 tactical only" 的信号, 不是 BUY 信号. 真正 BUY 入场需要 (i) 股价 reset 至 $30-40 (公允锚附近) (ii) Foundry external trajectory 显著改善 (iii) DCAI 连续 3 季度 +20%+. 三者同时满足才考虑 BUY.
