...
Q2 2026 完整指引 (Intel Q1 2026 release): revenue $13.8-14.8B (同比微增), GAAP EPS $0.09, Non-GAAP EPS $0.20 (季度环比 Non-GAAP 下降 -31% from Q1 $0.29). 这意味着 H1 2026 Non-GAAP EPS run-rate $0.49, 年化 ~$0.98 (假设 H2 维持 H1 速度).
...
资产负债表项分项列出 (避免早期"净现金 / 投资" 措辞误导): Cash + ST investments +$11.5B / Gross debt -$53B / Net debt adjustment -$41.5B, 对应 -$9.6/share. Intel 当前是 net debt 状态, 不是 net cash.

政府 puts 期权 (按 §1.5 拆项分析后的净值, 不是 BS 直接 +$5-8) 0 to +$2/share.

加总 (segment 加总 + Cash/ST + 政府 puts - Gross debt): $13-67B equity value, 对应 $3-15/share. SOTP 中位约 $8-10/share. 这是最 conservative 的估值方法, 因为 Foundry segment 在 base case 下贡献负值.
...
按 Intel Q1 2026 release 的 Q2 完整指引 (revenue $13.8-14.8B, GAAP EPS $0.09, Non-GAAP EPS $0.20):
...
## 第十三部分: v3.6 与 v3.5 修正对比 + 给读者的诚实声明

### 13.1 v3.6 修正了什么

我们这版 (v3.6) 在 v3.5 基础上做了 6 项必改 + 4 项可选优化, 因为 v3.5 仍有几个会被专业读者抓住的问题. 修改清单:
...
**必改 2: SOTP 现金 / 债务表述**. v3.5 写"净现金 / 投资 mark-to-market $11.5B", 这容易误导读者以为 Intel 是 net cash 公司. 实际 Intel Q1 2026 net debt 是 -$41.5B. v3.6 拆分清楚: Cash + ST investments +$11.5B / Gross debt -$53B / Net debt -$41.5B.
...
**必改 6: Q2 2026 guidance GAAP / Non-GAAP**. v3.5 只写"Q2 2026 Non-GAAP EPS guidance $0.20". v3.6 完整列出: revenue $13.8-14.8B, GAAP EPS $0.09, Non-GAAP EPS $0.20.

**可选 1: 执行摘要压缩**. v3.5 执行摘要太长 (1500+ 字), v3.6 压缩到约 800 字, 按"市场为什么兴奋 → 我们为什么不买 → 估值结论 → 行动 → 触发条件" 5 段式组织.

**可选 2: "反护城河" 措辞**. "反护城河" 容易被理解为"Intel 完全没有护城河". v3.6 改为"负经济利润 / ROIC below WACC", 同时明确"Intel 仍有技术资产 / x86 生态 / 政府支持 / 客户关系, 但这些资产当前没有转化成高于资本成本的经济利润".
...
**可选 4: 附录精简**. v3.5 附录二的 DM-AUX 索引 150+ 条 + DM-INTC 80+ 条 = 230+ 条逐条 reference, 对读者很重. v3.6 把附录分成两版: 附录二是"公开数据源列表 (主文版)" 按主题分组, 附录三是"完整 audit-grade reference (内部 source room)" 保留全部 230+ 条.

### 13.2 v3.6 还没有解决的问题 (诚实声明)

我们承认 v3.6 仍有以下没有完全解决的问题, 在写未来 v3.7 时应该改进:

第一, **Q2 2026 数据未到**. v3.6 base case 假设 Q2 in-line guidance, 但实际 Q2 release 在 7-8 月. 如果 Q2 actual 显著 deviate, v3.6 主结论需要 update.

第二, **AMD Q1 2026 数据未到** (报告完成 2 天后, 4-29). v3.6 用 Path A 80% (AMD beat consensus) 作为 base. 如果 4-29 实际是 Path C (低概率 5%), v3.6 公允需要从 $19.76 上修至 $25-32.

第三, **18A yield 实际数据仍未公开**. v3.6 维持"未验证变量" 立场, 但如果 Intel 在 Q2 / Q3 主动披露具体 yield 数字, v3.6 base case 需要重新校准.
...
这五个问题中任何一个的 update, 都会触发 v3.7. 我们承诺立即 update, 不在 v3.6 上叠补丁.
...
**正确使用方式**: 把 v3.6 当作"高争议状态的当前判断", 不是"未来 5 年的确定预测". 监控 Kill Switch (§9), 等 Q2 2026 / 5-1 AWS / NVIDIA GTC 等 catalyst 实际数据到达后, 我们会写 v3.7. 不要把 v3.6 公允 $19.76 当成"应该立即 SELL 进入 short" 的信号.
