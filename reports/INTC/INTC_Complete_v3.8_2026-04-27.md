# Intel: Agentic AI 给了 DCAI 新 bull case, 但当前 $82.57 仍偏高 (v3.8)

**当前股价**: $82.57 (2026-04-24 收盘, 盘中高 $85.22)
**今日 PV 区间**: $23-28 (主锚 $25.5, 8% WACC 折现) / **5 年退出价区间**: $30-40 (加权 $37)
**评级**: 审慎关注 (高争议) — 从 v3.7 维持评级, 但 conviction 进一步弱化
**5 年退出价期望回报**: 约 -55% (基于 5y exit value 加权 $37 vs $82.57)
**今日 PV 折现后隐含 downside**: 约 -69% (基于 today PV 主锚 $25.5 vs $82.57)
**行动建议**: avoid / watch / wait for reset (不变, 但 single-sided SELL 论点进一步削弱)

---

## 执行摘要

**v3.8 vs v3.7 关键变化**: 加入"Agentic AI CPU bottleneck" 这个新 bull case 变量 (Morgan Stanley directional estimate + Georgia Tech / Intel CPU-centric agentic AI 论文支持). 这个变量上调 DCAI bull case 概率, 从 v3.7 的 15% 升至 18-22% (中点 20%), 同时 Bear 概率从 37.5% 下修至 30-35% (中点 32.5%). 加权 5y exit value 从 v3.7 的 $29 上修至 $37, today PV 主锚从 $19.76 上修至 $25.5. 但即便如此, 公允价值仍显著低于当前 $82.57, 评级维持审慎关注 (高争议), 行动 avoid / watch / wait for reset.
...
**估值结论 (v3.8 重锚)**. 三种独立方法都指向"显著低于 $82.57" 的方向, 但区间宽度大. 我们把 DCF today PV $23-28 作为主锚 (中点 $25.5, 反映 v3.8 上修 bull 概率 + 上修 5y exit), SOTP $4-18 作为下沿压力测试 (略上修反映 DCAI segment 估值), peer multiple $20-38 作为上沿 sanity check (略上修反映 ARM / Foundry 略改善). 5 年退出价 (Bear $11.5 × 32.5% + Base $34 × 47.5% + Bull $68 × 20%) 加权 $37, 区间 $30-40.

**行动**. avoid / watch / wait for reset, 不是 high-conviction SELL. v3.8 vs v3.7: SELL conviction 进一步弱化 (赔率从 65%/35% 降至 55%/45%, 仍倾向 SELL 但 caveat 更多). 4 种主流投资风格仍然全部不 BUY, 但只有 1/4 (Long-short) 主动 SELL with caveat, 其余 3/4 是 avoid 或 WATCH. 加上 v3.8 新加 agentic CPU bull case, single-sided SELL 论点进一步削弱.

**6 个触发条件 (6-12 个月监控, v3.8 新增 agentic CPU 条件)**. DCAI 连续 3 季度 +20%+ YoY (确认 trajectory 转折); Foundry quarterly external revenue 突破 $500M (商业化进度 confirm); 18A yield 公开数据 >70% in 2026 H2; AMD Q1 2026 release / NVIDIA Vera reference design 公布; CHIPS Act / 政府持股政策变化; **Agentic CPU attach-rate trigger (v3.8 新增)**: AI server / agentic workload 的 CPU:GPU attach ratio 明显上升 + Intel Xeon 在该增量中保持/提升 share + DCAI ASP 上升 (不只是 volume) + hyperscaler 增加 Xeon-based agent orchestration clusters. 任一显著触发, 我们立即写 v3.9, 不在 v3.8 上叠补丁.

**版本说明**. 这是 v3.8, 在 v3.7 基础上加入 Agentic AI CPU bottleneck 章节 (§14) + 上修 DCAI bull case 概率 + 上修 5y exit value + 新增第 6 个监控条件. 数据截止 2026-04-27, 涵盖 Intel Q1 2026 actual + Morgan Stanley directional estimate (CPU latency share 框架图) + Georgia Tech / Intel CPU-centric agentic AI 论文.
...
**SOTP 一表汇总** (按 4.32B 摊薄股本):

| 项目 | EV / Equity value | Per share |
|------|-------------------|-----------|
| CCG (含原 NEX edge) | $50-60B | $11.6-13.9 |
| DCAI (含原 NEX networking) | $5-22B | $1.2-5.1 |
| Intel Foundry NPV | -$15B to +$5B | -$3.5 to +$1.2 |
| All Other (Mobileye 74% + Altera) | $10-12B | $2.3-2.8 |
| Cash + ST investments | +$11.5B | +$2.7 |
| Gross debt | -$53B | -$12.3 |
| Government puts (net) | $0-8B | $0-2 |
| **Equity value 加总** | **$13-67B** | **$3-15** |

SOTP 中位约 $8-10/share. 这是最 conservative 的估值方法, 因为 Foundry segment 在 base case 下贡献负值, All Other 估值偏低 (Mobileye 持股 mark-to-market 后市值波动大, Altera 残余估算 $1-2B 区间宽).
...
三种方法都指向"显著低于 $82.57" 的方向, 但区间宽度大. 真实情况不是"三种方法严格 converge 到 $20-25", 而是: 保守 SOTP 给出 $4-18 (反映 Foundry segment 在 base case 下贡献负值, 假设较严格), DCF 主锚给出 $23-28 (含 unconditional upside 项 + agentic CPU bull case partial weight), peer multiple 给出 $20-38 (上沿 $38 反映 INTC 历史周期中位 P/Sales 3.5x + agentic CPU 略上修 EV/Sales 倍数). 三种方法的共同结论是当前 $82.57 显著高于合理区间, 但合理区间本身有 $4-38 的宽度.
...
要让公允达到 $80/share (即 justify 当前 $82.57), 需要再加 +$30 的 upside, 包括 18A yield >70% in 2026 H2 (低概率), Apple A20 NDA 2026 H2 公开 (低概率), Microsoft Cobalt 2 60K+ wafer (中概率), 政府 puts call upside (低概率). 多个低概率条件需要同时成立. 我们不把 $80+ 视为 base case, 而是低概率 extreme bull case. 历史上半导体公司同时实现 5+ 个独立 bullish events 的案例不到 1% (基于 1990-2024 全部公开案例).
...
我们承诺以下任一发生 → 立即写 v3.9, 不在 v3.8 上叠补丁:
...
第二件: **当前价格已经提前买了大量转型成功**. 三方法主锚 DCF today PV $18-25 vs 当前 $82.57 = -68 to -78% downside (主锚). SOTP 下沿 $3-15 / Peer multiple 上沿 $17-33 三方法都指向"显著低于 $82.57". Q1 2026 数据 partial reverse 让 conviction 弱化但方向不变. 即使 Bull case (15% 概率) 5y exit $65, vs 当前 $82.57 仍 -21% 下行. 反向 stress test 显示 $50 公允实现概率 < 5%, $80+ 公允是低概率 extreme bull case (历史半导体公司 5+ bullish events 同时成立 < 1%).

第三件: **行动不是 SELL, 是 watch / wait for reset**. 4 投资风格视角全部不 BUY, 但只有 1/4 (Long-short) 主动 SELL with caveat. Reset window 6-12 个月, 5 catalyst 联合 fire 概率 40-50%. Reset 触发后估值跳升路径已量化 (1 catalyst -$2 to -$3, 2 catalysts -$5 to -$8, 3+ catalysts -$10 to -$15). 监控 8 条 Kill Switch, 触发后立即写 v3.8.
...
| GAAP EPS | -$0.73 | $0.09 | 转正 (+$0.82 swing) |
| Non-GAAP EPS | $0.29 | $0.20 | -31% |
| Non-GAAP gross margin | 41.0% | 不公开数字, 但隐含改善 | TBD |

GAAP EPS Q1 -$0.73 → Q2 +$0.09 的 +$0.82 swing 主要来自 impairment / restructuring 一次性项目消失. Non-GAAP EPS Q1 $0.29 → Q2 $0.20 的 -31% 季度环比下降则反映 underlying operating sustainability 问题. 两个数字给出相反方向的信号.
...
### 11.3 Q2 2026 release 后会改变什么

Q2 2026 release 预计在 2026 年 7-8 月. 三种情景对 v3.6 主结论的影响:

**Q2 actual beat guidance (Non-GAAP EPS > $0.25)**: 这会 partial confirm Q1 是 trajectory 转折. 我们会上修 Bull 概率从 15% 至 20%, 公允从 $19.76 上修至 $21-23. 评级仍 avoid, 但 SELL conviction 进一步弱化.

**Q2 actual in-line guidance (Non-GAAP EPS $0.18-0.22)**: 维持 base case, 公允 $19.76 不变. 行动建议不变.

**Q2 actual miss guidance (Non-GAAP EPS < $0.15)**: Q1 反弹被证明是单季度 noise. 我们会下修 Bear 概率从 37.5% 至 40-45%, 公允从 $19.76 下修至 $16-18. 评级 avoid → 部分视角 SELL. KS-DCAI 同时检查 (DCAI 是否同步走弱).
...
风险点: Intel Foundry 5 年内"放弃 leading-edge" 概率 30-40%. 如果发生, Foundry NPV 进一步恶化 -$5 to -$10/share, 公允从 $19.76 下修至 $13-15.
...
v3.6 reset 时间预期 (6-12 个月) 比历史可比短, 主要因为当前 catalyst clock 更明确 (5 catalysts 联合 fire 概率 40-50% in 6 个月). reset 幅度 (-58 to -68%) 略低于历史 2000 case (-81%) 是因为 (i) Q1 2026 partial reverse 提供 conviction 弱化 (ii) 政府 puts 提供 floor (iii) 4 投资风格视角中 1/4 SELL with caveat 不是单边 SELL.
...
v3.7 当前主锚 today PV $18-25, 5y exit $26-35, 加权 5y exit $29; v3.7 明确认为当前 $82.57 已经提前反映了大量转型成功. v3.8 加 agentic CPU bull case 后, 可以把 DCAI bull case 概率上调, 但要把 $82.57 合理化, 需要的不只是"CPU 重要性上升", 还要看到 Intel **捕获份额, 收入, 利润率和 ROIC 的连续改善**.

### 14.5 估值情景重锚 (v3.8 vs v3.7)
...
但考虑到 v3.8 新增 unconditional upside (agentic CPU 是 sustained tailwind 不是 single event), 给 +$2-5 的额外调整, today PV 主锚 $25.5 (区间 $23-28).
```

vs $82.57: 5y exit downside -55% (vs v3.7 -65%), today PV downside -69% (vs v3.7 -76%). Conviction 弱化 -10pp 左右, 但方向不变.
...
### 14.7 真正成为"重大利好"需要的连续验证

agentic CPU bull case 要从 v3.8 的"额外加权 +$5-6/share" 变成"重大利好" (today PV 从 $25.5 上修至 $40+, 评级从 avoid/watch 改为 neutral/watch), 必须满足至少 3 个条件:

第一, **DCAI 连续 3-4 季度 +20%+ 以上增长**, 证明 Q1 2026 不是单季度补库存.

第二, **Xeon 在 AI / agentic infrastructure 的 attach rate 明显提升**, 而不是被 AMD / ARM 抢走. 这需要 hyperscaler 公开 CPU mix 数据 (例如 AWS / Microsoft / Google 在 AI server 中的 Xeon 占比), 或第三方 (Mercury Research, IDC) confirm Xeon attach rate 上升.

第三, **DCAI margin 与 ROIC 改善**, 证明 CPU demand 不是低价抢量. 具体看 DCAI segment OPM 是否从 FY2025 的 2-9% 升到 15%+ (比如 FY2027), Xeon ASP 是否上升而不是平均售价压缩.

最好再看到: NVIDIA Vera / hyperscaler reference design 继续采用 Xeon (而不是切换到 ARM); Foundry 端至少不能继续恶化 (否则 DCAI 改善被 Foundry cash burn 抵消).

如果这些**同时出现**, 才可以把 INTC 估值主锚从 $23-28 上修到 $30-45 today PV, 评级从 avoid / watch 改成 neutral / watch.
...
v3.8 的 today PV $25.5 vs $82.57 = -69% downside, 仍然是显著高估, 但 conviction 比 v3.7 的 -76% downside 弱化了 7pp. 这反映 agentic CPU 是一个 valid bull case 但还需要数据验证.
...
**第 2 项**: 版本号残留清理. v3.6 多处仍写"立即写 v3.6, 不在 v3.5 上叠补丁" (从 v3.5 复制过来未改), 现统一改为"立即写 v3.8, 不在 v3.7 上叠补丁".
...
**正确使用方式**: 把 v3.7 当作"高争议状态的当前判断", 不是"未来 5 年的确定预测". 监控 Kill Switch (§9), 等 Q2 2026 / 5-1 AWS / NVIDIA GTC 等 catalyst 实际数据到达后, 我们会写 v3.8. 不要把 v3.7 公允 $19.76 当成"应该立即 SELL 进入 short" 的信号.
...
1. **当前股价 $82.57 vs 公允区间**. 今日 PV $18-25 (主锚 $19.76), 5y exit $26-35 (加权 $29). 今日 PV downside -76%, 5y exit downside -65%. 区分两个数字, 不要混淆.
...
5. **行动是 avoid / watch / wait for reset, 不是 high-conviction SELL**. 4 投资风格视角 (质量投资 / Special situations / Deep value / Long-short) 全部不 BUY, 但只有 1/4 (Long-short) 主动 SELL with caveat. 5 catalyst 6-12 个月联合 fire 概率 40-50%. 触发后 reset 幅度 -55 to -65%. 监控 KS-DCAI / KS-FOUND-EXT / KS-spinoff / KS-PRICE.
...
**Q1: 你说 today PV $19.76 / 5y exit $29, 但 Bloomberg consensus 12-mo target 是 $112. 谁更可信?**

A: Bloomberg consensus $112 反映 sell-side analyst 平均 target. Sell-side analyst 的 target 通常 (i) 跟随 momentum (股价涨他们追涨 target), (ii) 受 IB 业务利益冲突 (公司客户关系), (iii) 12-mo horizon 而不是 5y horizon. 我们的 today PV $19.76 是 8% WACC 折现 5y exit $29 后的现值, 反映 5y 时间窗口 + 三情景概率加权. 两者不可直接比较 — 时间 horizon 不同 (12-mo vs 5y), 方法不同 (单点 target vs 概率加权), 角度不同 (sell-side momentum-following vs buy-side fundamental). 我们建议读者把 Bloomberg consensus $112 当作"市场短期共识情绪指标", 把我们的 $19.76 当作"5y 概率加权公允估值". 两个都有用, 不互相替代.
...
**Q5: 为什么不写 high-conviction SELL? 你估算 today PV $19.76 vs 当前 $82.57 = -76% downside, 这不是 SELL 信号吗?**

A: -76% downside 是 directional 判断, 不是 actionable SELL recommendation. 4 个原因不写 high-conviction SELL: (i) Q1 2026 partial reverse 让 conviction 弱化 (从早期 80%/20% 降至 65%/35%); (ii) 政府 puts 提供下行保护 (即使 strike $10-15 而非 $25-30, 仍存在); (iii) Lift size 难把握 (历史半导体周期股 SELL 通常 -30% 后 dead cat bounce +20-30%); (iv) 4 投资风格视角 (质量投资 / Special situations / Deep value / Long-short) 中只有 1/4 (Long-short) 主动 SELL with caveat, 其余 3/4 是 avoid 或 WATCH. 这种"4/4 不 BUY 但只有 1/4 SELL" 的分歧反映高争议, 不应该简化为 SELL.
...
A: 会, 但只有 Path C (5% 概率) 显著改变. Path A (80% 概率, AMD beat consensus): KS-AMD 维持 87.5%, INTC bear thesis 维持, 我们 today PV $19.76 不变. Path B (15% 概率, AMD in-line): 公允微调 +$1. Path C (5% 概率, AMD miss): KS-AMD 大幅下降, INTC bear weakened, 公允可能上修至 $25-32, 评级从"高争议" 改为"标准审慎关注". 4-29 release 后 24 小时内我们必须回填. 如果 Path C 实现, 我们写 v3.8.
