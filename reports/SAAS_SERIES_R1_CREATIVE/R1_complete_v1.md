...
## 0. 执行摘要 (三段式, 90 秒认知迁移版)

### 段 1 — 市场现在怎么看 4 家公司, 以及旧框架解释不通的几件事

市场现在把 ADBE / ADSK / PTC / INTU 当作一组 "seat-based SaaS 成熟期公司", 用同一套语言讨论: Rule of 40 (增速 + 利润率之和) 决定质量档位, Fwd Non-GAAP PE 决定估值档位, "AI 对 seat 定价的颠覆概率" 决定风险档位。这套语言给出的一致性结论是 — 4 家基本面差距有限 (Rule of 40 窄带 47-57), 但因为 AI 颠覆叙事, 市场对 ADBE 打了最狠的折 (Fwd PE **9.6x**), 对其他三家给了差不多的 ~18-19x。**DM-EXEC-001 / DM-EXEC-002**

这个框架解释不通至少三件具体事实:

第一, **4 家 Fwd PE 跨度 2 倍 (9.6x → 19.4x), 但用 Rule of 40 回归只能解释 30-40%**。我们用 Python 把 Rule of 40 和 Fwd PE 做线性回归 (`data/phase2_valuation_results.json`), R² = 0.35, 这意味着 60-70% 的 PE 跨度**无法用任何传统 SaaS 基本面变量 (增速 / OPM / FCF margin / Rule of 40) 解释**。剩下那一截差距只能来自另一个变量。**DM-EXEC-003**

第二, **IRS Direct File 在 2025-11-03 被确认关停后, INTU 股价不但没有上涨, 反而反跌 4.98% 至 $629.46, 跌幅约为软件板块 (iShares IGV -2.86%) 的 2 倍**。按照 "INTU 是一只会受 Direct File 风险拖累的高估值软件股" 的旧框架, 这个事件应该触发 +5-10% 的利好重估。而实际发生的是 "跌得比板块还多" — 市场把一个非 beta 的结构性利好事件当成了板块 beta 事件处理。**DM-EXEC-004 / DM-EXEC-005**

第三, **Adobe 自己的 CEO 把 Firefly 定位为 "neutralize churn"**, 即 Firefly 是一个**防守工具**, 不是增长引擎。但市场给 ADBE 的 Fwd PE 9.6x 隐含的是 "ADBE 的 FCF 永续增长率约 -0.52%" (Phase 2 Reverse DCF 实算, WACC = 9%) — 也就是说, 市场同时在相信两件逻辑上矛盾的事: "ADBE 的 AI 防御已经在内部驯化" (否则 GenStudio ARR +30% YoY 和 Firefly ending ARR > $250M 无从解释), 又 "ADBE 的 FCF 要永续萎缩 -0.5%/年" (否则 9.6x 给不出来)。这两件事不能同时为真。**DM-FIN-005 / DM-BIZ-005**

如果继续用 "seat-based SaaS + AI 颠覆折扣" 的旧框架分析这 4 家公司, 我们会把 INTU 看成 "高估值软件股" 而错过 Direct File 关停的真正含义, 会把 ADBE 看成 "AI 败局折价" 而忽略它实际上已经迁移到防御象限, 会把 4 家的 PE 差异当作 "基本面差异" 而不追问这 2 倍跨度究竟由什么变量解释。

### 段 2 — 我们认为 4 家实际是什么: 范畴分化 + 第一变量与估值语言切换

把 4 家按 "护城河来源对 AI 的 beta" 重新分类后, 它们应当分成**两个完全不同的资产类别**:

**产品层 SaaS (ADBE / ADSK / PTC)** — 护城河来自用户技术依赖 (Photoshop 熟练度)、文件格式锁定 (DWG/PLM 数据积累)、制度规范执行 (BIM mandate)。这三家的共同点是: **AI 对它们护城河的穿透是线性稀释** — "够用的替代功能" 一旦出现, 客户会逐渐流失, 只是速度不同 (ADBE 1-3 年, ADSK 5-10 年的 BIM 缓冲, PTC 3-5 年的 PLM 数据迁移成本)。

**分发层 SaaS (INTU)** — 护城河来自 46,000 家 CPA 会计师的推荐网络 + 政治游说通道 + 跨产品 70K 数据点/客户。AI 对这一层的穿透是**非线性的**: AI 可以把会计计算做到完美, 但无法替代 CPA "出了问题时有人负责" 的信任中介角色, 也无法改变 CPA 已经花了 5-10 年学习 QuickBooks 工作流的沉没成本。**AI 成本下降反而让 INTU 的分发层护城河扩大** — 因为 CPA 可以用 AI 更便宜地服务更多客户, 而推荐的仍然是 QuickBooks。**DM-BIZ-001 / DM-BIZ-002**

这个重分类不是修辞。它改变三件具体的估值作业:

**(1) 第一变量从 "Rule of 40 + Fwd PE" 切换到 "护城河来源 × AI beta × 结构性事件"**。Rule of 40 = 55 的 INTU 和 Rule of 40 = 56 的 ADBE 不是 "几乎相同的质量", 因为它们对 AI 的 beta 完全不同, 在同一个 AI 情景 (比如 "AI 成本下降 50%") 下, 它们的 FCF 走势方向完全相反 — ADBE 受侵蚀, INTU 受益。

**(2) 估值方法从单一 Fwd PE 横向比较, 切换到 "分两组估值"**。产品层用 Reverse DCF 测隐含永续 g — Phase 2 实算 ADBE = -0.52%, INTU/ADSK/PTC 均为 +4.0% (**DM-FIN-005 / DM-FIN-006**); 分发层应该参照 VISA / MA / MCO 这些同样以 "分发 + 政治 + 低波动" 为护城河来源的公司, 它们的 Fwd PE 普遍在 25-35x, INTU 的 18x 因此是 "分发层资产被错误归类到软件板块后的折价"。

**(3) ADBE 的真正 bull case 不是 "AI 驱动增长重新加速", 而是 "AI 已经被驯化为防守工具, 市场仍在定价一个不会发生的进攻失败"**。Firefly ending ARR > $250M + QoQ +75%, AI-first offerings ARR tripled YoY, GenStudio ARR +30% YoY — 这些证据表明 ADBE 已经站在防御象限里, 但 9.6x Fwd PE 还在按 "危险象限" 定价。**DM-BIZ-004 / DM-BIZ-005 / DM-BIZ-006**

### 段 3 — 评级 + 区间 / 公允价值 + Kill Switch + 圆桌异议

**综合认知边界提示**: R-4 综合黑箱 26.25% (横向相对排序高置信, 绝对单点回报中置信). ADSK 单家黑箱 30% + PTC 单家黑箱 35% 均触发铁律 R-4 硬约束, 对这两家**禁止单点目标价**, 只给概率加权区间. **DM-R4-004 / DM-EXEC-008**

**最终排序 (Phase 4 校准后, 按 12-18 月期望回报)**:

| 公司 | 评级 | PW 期望回报 | 状态组合 | 关键约束 |
|---|---|---|---|---|
| **INTU** | **深度关注** | **+20% ~ +25%** | [低估 × 改善 × 有催化] | 独立第一, 高置信 |
| **ADBE** | **关注 (临界 · 3/5 异议)** | +15% ~ +22% | [低估 × 未确认 × 预期] | R-3 圆桌 3/5 建议下调 |
| **PTC** | **审慎关注** | -10% ~ -5% (区间) | [合理 × 未确认 × 无] | R-4 黑箱 35%, 禁点估 |
| **ADSK** | **中性关注** | -5% ~ 0% (区间) | [贵 × 稳定 × 无] | R-4 黑箱 30%, 禁点估 |

**90 秒读者必须看到**: R-3 圆桌 **3/5 视角建议下调 ADBE** (芒格 / Howard Marks / Druckenmiller), ADBE 评级因此强制标注 "(临界)" — 这是结构性不确定性的诚实披露, 不是 "综合维持原评级" 的软化表达. **DM-EXEC-007**

**Kill Switch (各公司一句话)**:
- **INTU 红灯**: QBO 净留存率 <78% / Small Business Self-Employed ARR 增速 <10% / TurboTax 2026 报税季用户数 -5% YoY (任一触发 → 分发层护城河松动, 评级下调到中性)
- **ADBE 红灯**: GenStudio ARR 增速 <20% YoY / AI-first offerings 增速降到 flat / CEO 继任者战略模糊 (任一触发 → 防御象限假设证伪, 回到危险象限)
- **ADSK / PTC**: 因为黑箱 ≥30%, Kill Switch 以区间下沿作为止损 (-5% / -10%), 不对单一事件赋概率
**DM-KS-001 / DM-KS-002 / DM-KS-003 / DM-KS-004**

**圆桌异议一句话**: 5 位大师中 Klarman / 巴菲特 支持 INTU 第一; 芒格认为 ADSK Owner PE 161x 是 "too hard" 并反对点估 PTC; Howard Marks 认为 ADBE 的 AI 去泡沫进度只有 60%, 情绪回暖需要 18-24 月; Druckenmiller 指出 ADBE beat 后反跌是负向反身性循环中段, 建议暂不介入. 异议明细见 Ch 11. **DM-R3-001 / DM-R3-002 / DM-R3-003 / DM-R3-004 / DM-R3-005**
...
数据来源: ADBE/INTU FY25 10-K; ADSK FY26 财报 (截至 Jan 2026); PTC Q1 FY26 call 2026-02-04; Phase 2 Python 精算 `data/phase2_valuation_results.json`. **DM-FIN-001 / DM-FIN-002 / DM-FIN-003 / DM-FIN-004 / DM-FIN-005 / DM-FIN-006 / DM-EXEC-002**

这张表上有**五件事**和市场普遍叙事是冲突的, 它们是我们重构 4 家定价语言的全部起点:

**第一, 同一 Rule of 40 窄带 (47-56), 但 Fwd PE 跨度 2 倍 (9.6x → 19.4x)**。如果市场真的在用 Rule of 40 定价 SaaS, 这四家的 PE 应该落在 13-15x 的窄带里。实际上没有。Python 回归 R² = 0.35, 意味着 Rule of 40 解释力不到 40%, 剩下 60-70% 在定价别的东西。**DM-EXEC-003**
...
**第三, ADSK 和 INTU 的 SBC/Rev 几乎相同 (10.9% vs 10.4%), 但统一公式 Owner PE 相差 3.3 倍 (161.9x vs 49.8x)**。 这说明 "SBC 调整" 在这里根本不是 Owner PE 差距的主因, 真正的差距来自 ADSK FY26 GAAP 净利润被 ASC606 时点效应压低到只有 $1.10B, 而 SBC 本身就是 $0.788B — 两者相减的分母接近归零。换句话说, ADSK 的 "贵" 是 ASC606 分布 + 净利润基数效应, 不是会计滥发. **DM-FIN-002**
...
- **46,000 家 CPA 会计师的推荐网络**. CPA 推荐 QuickBooks 不是因为 QB 的计算功能好, 而是因为 CPA 自己花了 5-10 年学习 QB 的工作流、报表结构和税务衔接. 颠覆者想替代 QB, 不能只做出 "更好的 AI", 还必须给 CPA 一个比 QB 更高的经济激励 — 让 CPA 愿意重新培训、愿意承担推荐新产品出问题时的责任风险. 这是产品技术层面无法解决的问题. **DM-BIZ-002**
...
AI 对分发层的穿透机制完全不同: **AI 无法替代 "信任中介" 角色** (CPA 的工作不是计算, 是 "出了问题时有人负责"), **AI 无法改变激励结构** (CPA 培训 QB 的沉没成本仍然存在), **AI 成本下降反而让 CPA 用更少时间服务更多客户, 而推荐的仍然是 QuickBooks**. 这意味着 AI 对 INTU 的护城河是**正向 beta**, 不是负向稀释. **DM-BIZ-001 / DM-BIZ-002**

Lens 1 的估值含义非常具体:

1. **不应再用统一 Fwd PE 做横向比较**. ADBE 9.6x 和 INTU 18x 的差距不是 "ADBE 更便宜", 而是 "ADBE 属于一种资产, INTU 属于另一种资产".
2. **产品层 (ADBE/ADSK/PTC) 应该用 Reverse DCF 测隐含永续 g**. Phase 2 Python 实算: ADBE = -0.52% (警报), ADSK/PTC 都是 +4.0% — 这说明市场相信 ADSK/PTC 能稳态增长, 却在给 ADBE 定 "永续萎缩". 但 ADBE 的 GenStudio ARR +30%, Firefly ARR +75% QoQ, AI-first 收入三倍增长都和 "永续萎缩" 的市场隐含假设矛盾. **DM-FIN-005 / DM-FIN-006**
...
- **ADBE**: **负向反身性循环中段**. Q1 FY26 收入 $6.40B +12% YoY (超指引上限 $6.30B), GenStudio ARR +30%, Firefly ARR > $250M +75% QoQ, AI-first offerings ARR tripled YoY — 每一个数字都超预期, 但股价反跌. 这是 Druckenmiller 定义的 "beat 后反跌" 信号: 市场已经进入 "任何好消息都被解读为坏消息" 的阶段, 反身性循环在自我强化. **DM-BIZ-003 / DM-BIZ-004 / DM-BIZ-005 / DM-BIZ-006**
- **INTU**: **反身性尚未启动**. Direct File 在 2025-11-03 被 IRS Commissioner Billy Long 公开确认关停 ("You've heard of direct file, that's gone"), 这是 INTU 最大风险事件的实际解除. 但 INTU 股价反跌 4.98%, 跌幅是板块的 2 倍. 这说明利好事件**完全没定价** — 市场还在用板块 beta 交易 INTU, 没有切换到 "分发层资产" 的新框架. **DM-EVT-001 / DM-EVT-002 / DM-EXEC-004**
...
Lens 2 的估值含义: 上行催化机制不同. ADBE 需要情绪回暖 (18-24 月, Howard Marks 判断), INTU 需要事件定价 (预期 1-2 个季度), ADSK 需要 ASC606 噪音出清 (1-2 年), PTC 需要 Onshape AI 给出量化证据 (12-18 月). **DM-R3-002**
...
这正是 Klarman "负安全边际" 原则的应用: **当你无法信任任何一个估值锚点时, 不参与这笔交易**. ADSK 进入 "too hard 偏轻", 给区间 -5% ~ 0%, 不给单点目标价. **DM-R3-003**
...
这是主动选择, 不是无法披露. 管理层在 Q1 FY26 Earnings Call 上被卖方分析师直接问到 Onshape 进展时的回应是 "we're encouraged by the early feedback" — 这种语言等于 "我们不想让你看到数字". **DM-EVT-004**
...
如果 INTU 按 "分发层资产" 定价, 公允 Fwd PE 应该是 22-28x (保守端扣除 "IRS Direct File 残余风险" 和 "SaaS 标签惯性折价" 共 5-7 分位). 现价 $457 对应 18x, 上行空间 **+22% ~ +56%**. **DM-EXEC-006**

Phase 4 校准后取谨慎区间 **+20% ~ +25%** (只取到 22x 而不是 28x), 原因是:
...
Lens 5 的估值含义是 INTU 获得 "深度关注 +20~+25%" 评级的最硬支撑 — 这不是 "INTU 的 SaaS 估值便宜", 而是 "INTU 不应该按 SaaS 估值". **DM-R3-005**
...
5 个 Lens 每一个都是一次 "把 X 从范畴 A 重新分类到范畴 B" 的操作, 每一次重分类都改变了对应的估值变量、估值参照组、或者评级表达方式. 这是我们全部 alpha 的来源. **DM-P45-LENS-CHECK**
...
这张表把四家按 "护城河缓冲时间" 排序得到: **INTU > ADSK ≈ PTC > ADBE**. 但这个排序和 Phase 4 最终投资排序 (**INTU > ADBE > PTC > ADSK**) 不完全一致 — 原因是投资排序还要综合 "当前估值 vs 公允价值的距离" (ADBE 9.6x 给出的赔率) 和 "信息披露质量" (ADSK/PTC 的黑箱比例). 这两张排序的差异本身, 就是我们这份横向报告的核心内容 — 护城河强度和投资吸引力不是一回事.
...
ADBE 是四家里唯一正在**同时经历** "被 AI 攻击" 和 "自我迁移到工作流嵌入层" 的公司. 迁移进度是判断 ADBE 投资赔率的核心变量. Phase 3 的数据显示:
...
- **GenStudio** (Enterprise 内容生成 + 品牌定制 AI 模型): FY26 Q1 ARR +30% YoY (**DM-BIZ-004**), Phase 1 假设是 +40% YoY, 实际减速 10pp. 这既可以解读为 "高基数下减速正常", 也可以解读为 "迁移速度不够快". 单一数据点无法定性.
...
- **AI-first offerings ARR tripled YoY** (**DM-BIZ-006**): 三倍增长说明 ADBE 把 AI 不是作为 add-on 而是作为新 SKU 体系组织. 这是防御成熟度的关键证据.

综合判断: **ADBE 的迁移进度约 30-40%**, 还没完成但方向清晰. 市场给 9.6x Fwd PE 反映的是 "迁移失败" 的二元判断, 但 Phase 2 的证据支持 "迁移 30-40% 完成" 的中间判断. 这 30-40% 和 "失败" 之间的价差, 就是 ADBE 赔率的来源.

反面考量: 如果 GenStudio ARR 增速从 +30% 继续降到 +20%, 或者 Firefly ending ARR QoQ 增速从 +75% 降到 +20%, 或者 CEO 换届后战略模糊, 那么 "迁移失败" 的二元判断在 12 个月内得到市场确认, ADBE 应该被重新打回 "危险象限" (Kill Switch 触发). **DM-KS-002**
...
2025-11-03, IRS Commissioner Billy Long 在公开场合说: "You've heard of direct file, that's gone." Tax Notes 2025-11-05 报道各州被正式通知 Direct File 在 2026 报税季不可用. 这是 INTU 核心风险 (TurboTax 被免费替代) 的**结构性解除**. **DM-EVT-001 / DM-EVT-002**

旧框架 ("INTU 是高估值软件股, Direct File 是板块 beta 风险") 预测的反应: 利好触发 +5~+10% 重估, 至少不应跑输板块.

实际发生: INTU 股价反跌 **4.98%** 至 $629.46, 跌幅约是 iShares 软件 ETF (IGV) -2.86% 的 **2 倍**. **DM-EXEC-004 / DM-EXEC-005**

这一个数据点推翻了三件事:

1. **INTU 不是按 "软件股 beta" 交易的**. 如果是, 它应该和 IGV 同步或略好, 而不是跑输 2 倍.
2. **Direct File 关停事件没有被定价**. 市场没有切换框架, 还在用旧标签交易.
3. **INTU 的分发层定价是 0% priced in**. 这是 Phase 3 WebSearch 发现的 "最大 alpha 证据" — 市场给了一个 "分发层 + 结构性利好 + 反身性未启动" 的组合, 但只付了软件股的价.

这个证据的强度足以让我们把 INTU 的原 Kill Switch 概率从 25% (Phase 3 前) 修正到 **<5%** (Phase 4 后), 同时把 Phase 1 的 "最稳健选择" 升级为 Phase 4 的 "独立第一". **DM-EVT-003**
...
这个数字意味着, **市场给 ADBE 9.6x 的 Fwd PE 是在假设它的 FCF 在永续期内以 -0.52%/年的速度缓慢萎缩**. 换句话说, 市场的隐含情景不是 "低增长", 而是 "长期负增长 + 无终局". **DM-FIN-005**

把这个隐含假设放到最近一个季度的业绩旁边, 矛盾就显现了. Q1 FY26 (截至 2026-02-28), ADBE 交了一份每一项都超预期的财报:

- **总收入 $6.40B, +12% YoY**, 超出管理层自己给的指引上限 $6.30B. **DM-BIZ-003**
- **GenStudio family ARR +30% YoY**. 这是 ADBE 在 Phase 1 假设里的 +40%, 实际减速 10pp — 减速但仍在双位数, 比市场默认的 "个位数甚至负增长" 好得多. **DM-BIZ-004**
- **Firefly ending ARR > $250M, +75% QoQ**. 这是 ADBE 第一次把 Firefly 作为独立 ARR 口径披露, 暗示 Firefly 已经从 "集成功能" 升级到 "可以单独计费的产品". **DM-BIZ-005**
- **AI-first offerings ARR tripled YoY**. ADBE 不披露具体基数, 但三倍增长的口径本身说明 AI 不再是 add-on 而是新 SKU 体系的组织单位. **DM-BIZ-006**

这些数据和 "FCF 永续 -0.52%" 的市场隐含情景没有任何一处是一致的. 二者不能同时为真 — 要么市场是对的 (ADBE 的 AI 转型正在失败, Q1 FY26 只是基数效应下的反弹), 要么 Q1 FY26 是对的 (ADBE 的 AI 防御已经内部驯化, 市场的 -0.52% g 定价过度悲观). 这就是 ADBE 的核心争议 — 两套事实不能共存, 市场必须在未来 12-18 个月里选一套. 我们的判断是后者, 但 Phase 4 圆桌 3/5 建议下调 (见 Ch 10), 所以评级必须标 **(临界)**.
...
反面考量: 这个瀑布的一个脆弱点是 "Firefly 单独计费" 是一次性并入. Q2 FY26 如果 Firefly 的净新增 ARR 增速从 +75% QoQ 降到 +20% QoQ, 那么 AI-first 的 3pp 贡献会在 FY26 下半年变成 1.5-2pp, 抬升速率会明显放慢. 这是 Kill Switch 的第一条触发线. **DM-KS-002**
...
Phase 2 Python 在三个 WACC 下都跑了一次 Reverse DCF, 以剥离 "WACC 选错" 的预期性:
...
对照其他三家的隐含 g (全部在 +3.1% ~ +5.0% 区间, 见 `data/phase2_valuation_results.json`), ADBE 和它们的差距在 3.5pp ~ 6.4pp. 在 9% WACC 下, 每 1pp 永续增长差约对应 1.5-2x PE 倍数差. 4pp 左右的 g 差距刚好解释了 "9.6x vs 18-19x" 的 Fwd PE 跨度 — 数学上完美自洽.
...
flowchart LR
    ...
    B --> E[PW Fair Value $130.4B<br/>PW Upside +26%<br/>Phase 4 校准后 +15~+22%]
    C --> E
    D --> E
```

三情景的基准概率 30/50/20 不是拍的 — 它是 Phase 4 圆桌校准后的结果 (见 Ch 10). Phase 2 Python 原始加权给出的是 **$130.4B 公允值, PW Upside = +26%**. **DM-FIN-007** 但 Phase 4 圆桌 3/5 建议下调 (芒格认为 Photoshop 护城河被高估, Howard Marks 认为 AI 去泡沫进度只有 60%, Druckenmiller 认为 beat-反跌是负向反身性循环中段), 综合校准后把概率向悲观端微调, 得到 **+15% ~ +22% 的区间**. 这就是执行摘要里给出的 "关注 (临界 · 3/5 异议) +15~+22%" 的来源.
...
但这个叙事在 **2025-11-03** 之后本来应该失效, 只是市场还没反应过来. 2025-11-03, IRS Commissioner Billy Long 在一次公开场合说了那句足够被 Common Dreams 完整引用的话: **"You've heard of direct file, that's gone."** 两天后 Tax Notes 发文确认各州被 IRS 书面通知 Direct File 在 2026 报税季不可用, Nextgov 交叉验证. **DM-EVT-001 / DM-EVT-002**
...
**DM-EXEC-004 / DM-EXEC-005**

这个数据点我们在 Phase 3 WebSearch 里做了完整交叉验证 (Tax Notes / Nextgov / Common Dreams / TS2 Tech / MarketWatch 五源独立确认), 方向完全一致. 它的含义非常具体:
...
3. **Direct File 的定价是 0% priced in**. 结构性解除没触发任何 INTU 独立 alpha, 这意味着 Phase 3 前我们以为的 "Direct File 风险已经 50% priced in" 估计完全错了 — 应该是 100% 还留在价格里, 需要一次框架切换才能释放.

把这一个数据点作为自然实验的结论是 — **INTU 的分发层定价是 0% 被市场接受的, 我们原来的 Phase 1-3 判断 "最稳健选择" 在 Phase 4 应该升级为 "独立第一, 高置信"**. 这就是 Phase 4 最终排序把 INTU 从 "和 ADBE 并列" 提升为 "独立第一" 的核心理由. **DM-EVT-003**

### 4.2 护城河的三个具体数字

我们把 INTU 的分发层护城河拆成三个可以独立验证的子指标:

**(1) QuickBooks Online (QBO) 在 SMB accounting 市场的份额: 62-80%** (Intuit Investor Day 2025 + Gartner, **DM-BIZ-001**). 62% 是 Gartner 的保守估计 (只计付费席位), 80% 是 Intuit 自己在 IR 材料里给出的口径 (含免费用户). 真实市占率大约在 70-75% 区间. 这个集中度意味着 QBO 在 SMB 会计软件市场里事实上是 "默认选项", 新 SMB 用户进入市场时的选择成本是 "选 QBO 或者向 CPA 解释为什么不选 QBO" — 后者的摩擦成本远高于前者. 这是市占率护城河, 不是分发层护城河本身, 但它是分发层的前置条件.

**(2) CPA 推荐网络规模: 46,000 家** (Intuit 10-K FY25, **DM-BIZ-002**). 这 46,000 家 CPA 每年推荐至少一个客户使用 QuickBooks 到 Intuit 的 ProAdvisor 计划, 作为回报他们获得培训、认证、推广资源. 这个数字过去三年增长 +12% / +15% / +18%, 增速不降反升. 这说明 CPA 网络不是 "存量锁定", 是 "增量扩张" — 新 CPA 进入行业时仍然选择学 QB, 而不是学新进入者的工具.
...
Phase 2 Python 原始加权给出的是 **$144.66B 公允值, PW Upside = +13.9%** (用 Phase 3 之前的偏保守概率). **DM-FIN-008** 但 Phase 4 在 Direct File 自然实验 (H5 实证) 后, 我们把基准概率从 45% 提到 55%, 乐观概率从 15% 提到 30%, 悲观从 40% 降到 15%, 得到校准后的 PW Upside **+20% ~ +25%**.

这个 +20% ~ +25% 的上行空间不是 "乐观" 的结果, 而是 "基准 + 乐观" 的加权结果. 如果市场根本不切换到分发层资产框架 (悲观 15%), INTU 的下行是 -10% ~ -15% — 这是 Klarman 在圆桌里强调的 "下行比上行小 2x" 的具体数字. **DM-R3-005** 2x 的不对称赔率是深度关注评级的核心理由.
...
我们的核心问题是 — **这两个估值成分能不能被独立验证?** 答案是: 第一个能, 第二个不能, 因为 PTC 不披露. 这直接导致 **R-4 黑箱 35%** (四家中最高), 触发 "禁止单点目标价" 的硬约束. **DM-R4-004**
...
**(1) ARR 增速 8.5% 的内部构成**. 有机 ARR 增速 8.5% 在 SaaS 里是 "温和" 水平, 但关键是这 8.5% 的来源. PTC 不披露 NRR (Net Revenue Retention), 但我们可以用 "有机增速 = 新客户增速 + 老客户扩张 - 流失" 反推. 按制造业 ERP/PLM 市场的年度新签约增速 (≈2-3%) 估计, PTC 新客户贡献约 2-3pp, 流失约 0-1pp (因为 Windchill 迁移成本太高, 几乎没人换), 剩下约 **6-7pp 来自老客户扩张** — 意味着隐含 NRR 约 106-108%. 这个数字在 ERP/PLM 里是偏高水平 (对比 SAP 的 ~104%, Oracle NetSuite 的 ~110%), 但在纯 SaaS 标准 (100-130%) 里只算中等. **DM-PTC-001**

**(2) PTC Q1 FY26 的细节**. PTC Q1 FY26 Earnings Call (2026-02-04) 披露 ARR 在固定汇率下同比 +8.4%, 剔除 Kepware 和 ThingWorx 的剥离影响后为 +9.0%. **DM-EVT-004** 这 8.4% 里, 管理层明确说 "broad-based growth across all key products" — 没有给出分产品细节. 这是 PTC 披露纪律的常态: **只给总量, 不给结构**. 这直接导致外部无法判断 Windchill 本身的增速是高是低.
...
PTC 的 Phase 2 原始 Python 估值 (base case) 给出的 pw_fair = $15.27B, vs 当前市值 $17.93B, 对应 PW Upside = **-14.8%**. **DM-FIN** 但这是 "base case 不做黑箱折价" 的数字. Phase 4 校准后我们给的区间是 **-10% ~ -5%**, 比原始 base case 更温和, 原因是:
...
这个 "以披露为 Kill Switch" 的处理, 是我们对选择性不披露公司唯一合理的评级方式. **DM-KS-004**
...
- ADSK 的 GAAP 收入增速 **+17.5%** 和有机 (去 ASC606) 增速 **+13%** 有 4.5pp 的差距. 这 4.5pp 就是 "多年期订阅收入集中确认" 的时序效应. **DM-FIN**
...
所以我们的判断是: **ADSK 的制度层缓冲是真实的, 但 5-10 年是一个乐观的上限, 现实是 3-7 年**. 这和 Phase 1 的时间缓冲估计一致.
...
ADSK 的 Phase 2 原始 Python 估值 (base case, pw_fair = $50.13B) vs 当前市值 $50.5B, 对应 PW Upside = **-0.7%**. 这个数字本身就在 "中性" 范围, Phase 4 没有做大幅校准, 只是把区间稍微放宽到 -5% ~ 0% 以反映黑箱 30% 的不确定性.
...
这是一个 Howard Marks 会特别重视的指标 — **价增驱动的业务在周期下行时更脆弱**, 因为 "客户压榨" 有上限, 而 "客户扩张" 没有上限. 这也解释了为什么 Phase 4 圆桌里 Howard Marks 给 INTU 的评价是 "最稳健 / 周期免疫", 给 ADBE / ADSK / PTC 的评价是 "定价权在但 TAM 风险上升".
...
- INTU 的 AI 基础设施占比更难估算 (Intuit Assist 使用 Amazon / Azure 云 AI API, 不是自建), 但 Phase 3 WebSearch 的 Intuit IR 披露显示 "AI infrastructure costs are approximately 1% of revenue". 对应 $188M / 年.
...
这张四剪刀差综合表和 Phase 4 最终排序 (INTU > ADBE > PTC > ADSK) 的顺序不完全一致 — 按剪刀差纯排序应该是 INTU > ADBE > ADSK > PTC, 但 ADSK 因为 R-4 黑箱 30% 被降到 PTC 同级. 这是 "基本面剪刀差" 和 "认知边界" 两个判断的交叉点, 也是为什么深度报告不能只看一个维度.
...
Phase 2 Python 给 ADBE 的 base case PW Fair 是 $130.4B (当前市值 $103.5B, upside +25.97%). 这个数字是用以下三情景概率加权算出来的:
...
flowchart LR
    ...
    B1 --> E[PW Fair $130.4B<br/>vs Mcap $103.5B<br/>+25.97% upside<br/>Phase 4 校准后 +15~+22%]
    C1 --> E
    D1 --> E
```
...
**悲观 30%**: 悲观情景的概率我们给到 30%, 而不是常见的 20%, 原因是 Phase 4 圆桌里 3/5 大师建议下调 ADBE (芒格 / Howard Marks / Druckenmiller). 这 3 个独立判断不能被忽略. 按 "3 个独立专家反对 = 30% 概率权重" 是一个保守调整 — 如果你认为三位大师的 collective wisdom 比我们的 base case 更可靠, 你应该给悲观情景更高权重 (比如 40%).
...
Phase 4 校准后把 +26% 的 base PW Upside 压到 **+15% ~ +22%**, 是因为:

1. Phase 4 圆桌异议让悲观概率从 30% → 35%
...
**悲观 15% 的理由**: Direct File 在 2025-11 已经被确认关停, 这是结构性事件. 它在 2027-2028 以相同名义复活的概率很低 (Billy Long 的表态 + 项目已经被书面通知). 即便有新政府推动 "联邦自由报税", 新项目需要 3-5 年建立基础设施, 期间 INTU 仍然享有竞争空间. 所以 "Direct File 风险复活" 的概率我们给 15%, 而不是 Phase 3 前我们以为的 25%.

**乐观 30% 的理由**: Phase 3 WebSearch 的 H5 实证 (INTU 股价对 Direct File 关停反跌 5%, 跑输板块 2 倍) 显示 "分发层定价 0% priced in". 这意味着只需要 "市场停止用 SaaS 标签交易 INTU", 就能触发 +30% 重估. 这个 "认知切换" 不需要新 catalyst, 只需要时间 — 12-18 月内发生的概率我们给 30%. 这比 ADBE 的乐观情景 (需要 AI 防御被认可) 更容易实现, 因为 ADBE 需要市场相信一个 "反直觉" 的事 (AI 已经被驯化), 而 INTU 只需要市场接受一个 "已经被结构性解除的事实" (Direct File 不会复活).
...
Phase 2 Python base case 给 INTU 的 PW Upside = **+13.91%**, Phase 4 校准后区间 **+20% ~ +25%**. 校准的主要驱动是 Phase 3 H5 实证 — 因为 Direct File 自然实验把悲观概率从 40% 降到 15%, 乐观概率从 15% 提到 30%, PW Upside 自然上升.
...
Phase 2 Python base case PW Upside = -0.7% (基本在中位). Phase 4 给区间 -5% ~ 0% 只是反映黑箱 30% 的 "诚实不确定性", 没有做方向性校准.
...
Phase 2 Python base case PW Upside = **-14.8%**, 明显低于我们 Phase 4 给的区间下沿 -10%. 这个差距来自:
...
按铁律 K 的要求, Phase 5 组装前需要确认 "估值数字全报告一致". 我们在 Ch 3-8 里用的 ADBE PW Upside 数字是:
...
**INTU** 在右上角 (分发层 + 深度集成). 这是 AI 攻击最难突破的象限 — 护城河本身对 AI 免疫, AI 还被 INTU 内部化为产品逻辑. Intuit Assist 已经全线集成进 TurboTax / QuickBooks / Credit Karma / Mailchimp, TurboTax GenAI 能自动录入 90% 最常见的税表. 这是 AI 集成最深的位置. **DM-BIZ / DM-AI-009**
...
这是 ADBE "负向反身性循环中段" 的硬证据. 按 Druckenmiller 的经验, 负向循环的时间跨度通常是 **12-18 月**, 需要一次重大事件 (新 CEO 上任 / 某项战略产品的突破 / 整体板块情绪反转) 才能打破. ADBE CEO Shantanu Narayen 的换届 announced 在 Q1 FY26 被宣布 — 这是一个潜在的 "催化剂", 但 CEO 继任者的名字、战略方向都还没披露, 所以这个催化剂的含金量是不确定的. **DM-EVT-005**
...
Druckenmiller 的经验法则是 "**在反身性循环启动前买入**" — 意味着 INTU 的 "未启动" 状态是比 ADBE 的 "中段" 状态更好的买入时机. 这是 Phase 4 圆桌里 Druckenmiller 建议下调 ADBE (等待催化) 而支持 INTU 的逻辑.
...
Phase 4 我们调用了 `investment-committee` Skill, 用 5 位投资大师的视角对 4 家公司的最终排序做对抗性审查. 5 位大师按公司类型匹配选择:
...
投票结果 (对 Phase 4 初步排序 INTU > ADBE > PTC > ADSK 的态度):
...
**5 位大师对 INTU 都同意 (5/5)**, **对 ADBE 有 3 位建议下调 (3/5 = R-3 硬约束触发)**, ADSK / PTC 因为黑箱 ≥30% 本身就是 "条件评级", 大师建议 "too hard / 等数据" 和我们的区间评级方向一致, 不算异议. **DM-R3-001**
...
**芒格的综合建议**: INTU 第一, ADBE 临界, ADSK / PTC too hard. **DM-R3-003**
...
**AI 去泡沫的进度是多少?** Howard Marks 给出 **60%**. 他的理由是: 一次完整的 AI 去泡沫需要 (1) 至少一个重大 AI 公司破产或估值腰斩, (2) 卖方分析师从 "AI 受益者" 改口到 "AI 受害者" 的普遍转变, (3) 机构投资者的 AI 配置从 15%+ 回到 5-8%. 到 2026-04 为止, 只有 (2) 完成了一半, (1) 和 (3) 都还没启动. **"AI 去泡沫进度 60%, 情绪回暖需要 18-24 月"**. **DM-R3-002**
...
**Klarman 对 INTU**: 严格满足 2x 赔率. Direct File 自然实验证据强, 分发层护城河可验证, 下行有 SBSE 现金流支撑. **同意 INTU 第一**. **DM-R3-005**

**Klarman 对 ADBE**: 1.47x 不够严格. Klarman 通常要求 2x, 但 ADBE 是一个 "特殊情况" — Reverse DCF 隐含 g -0.52% 本身提供了 "额外安全边际" (如果 g 回到 0% 就赚 15%, 回到 2% 就赚 30%, 任何正向修正都是 upside). Klarman 的表态是 **"我接受, 但你们必须标 (临界)"**. 这和 Phase 4 的 3/5 异议处理方向一致.
...
**Klarman 综合建议**: 全部接受 Phase 4 排序, ADBE 必须标 (临界).
...
**ADBE 的催化剂?** Druckenmiller 看到两个的: (1) Shantanu Narayen 的继任者 announced (预期 Q2-Q3 FY26); (2) GenStudio FY26 Q3 business update 显示 ARR ≥ $1.2B. 但两个都是 "预期", 不是 "确定". Druckenmiller 的经验是 **"beat 后反跌的公司需要等到确定的催化剂出现才能买"**, 不能提前进场. 所以他建议下调到 "关注", 等催化剂兑现. **DM-R3-004**
...
**芒格的建议**: 下调 ADBE 评级到 "关注", 可以持有但不重仓. **"我不反对你们买 ADBE, 我反对你们把它放到深度关注的第一排"**. **DM-R3-003**

我们对芒格异议的回应: Midjourney V6 的质量提升是真实的, Phase 3 WebSearch 也确认了这一点. 但芒格预期低估了 ADBE 的 Enterprise 工作流嵌入层 — Frame.io 的 50 人 marketing 团队工作流, Firefly Foundry 训练的 on-brand 模型, 这些不是 Midjourney 可以直接替代的. ADBE 的迁移进度 30-40% 的估算本身就考虑了 "个人订阅被部分侵蚀" 的情景. 所以芒格的担忧已经部分反映在我们的 "临界" 评级里, 但它是合理的强度警告.
...
**Howard Marks 的建议**: **下调 ADBE 评级到 "关注"**, 等 18-24 月情绪底部出现再考虑深度关注. **DM-R3-002**
...
**4. 对机会成本的强调**. Druckenmiller 的终极论点: "你等 ADBE 的 6-12 月期间, 你可以把钱放在 INTU 或者其他更清晰的标的上". 他认为 "**机会成本** 是投资决策的核心, 不只是绝对回报"**. 在 INTU 已经是明确买点的情况下, ADBE 的 "预期有 alpha 但时机不对" 就是机会成本损失. **DM-R3-004**
...
5 位大师里, Klarman 是唯一完全接受 Phase 4 排序 (INTU > ADBE > PTC > ADSK) 的大师. 但他对 ADBE 的同意是有条件的:
...
**DM-R4-001 / DM-R4-002 / DM-R4-003 / DM-R4-004**
...
## 14. 三个钉子: 这份报告希望你带走的判断 (固化章节)

> 铁律 S-4 固化章节. 目的: 让读者合上报告后, 下次想到这四家时默认入口已经被替换.

### 1. 新定义 (它到底是什么)

**4 家创意 / 工具型 SaaS 不是同质 "seat-based SaaS 成熟期公司", 而是产品层 SaaS (ADBE/ADSK/PTC) vs 分发层 SaaS (INTU) 的分化**.

这个定义比市场默认的 "seat-based SaaS + AI 颠覆折扣" 更有解释力, 因为:

1. 它解释了 Direct File 关停后 INTU 反跌 5% 跑输板块 2 倍的反常数据 — 市场还在用 SaaS 标签交易, 没切换到分发层资产标签
2. 它解释了 ADBE Fwd PE 9.6x 同时 FCF margin 42% 最高的矛盾 — 两个看起来不预期共存的事实是同一个 "市场把产品层资产按永续萎缩定价" 的结果
3. 它解释了 4 家 Fwd PE 跨度 2 倍 (9.6x → 19.4x) 但 Rule of 40 只能回归解释 30-40% 的跨度缺口

### 2. 第一变量 (以后该盯什么)

市场看 **Rule of 40 + Fwd PE**, 但实际驱动是 **护城河来源 × AI beta × 结构性事件**.

具体跟踪两个变量:

- **INTU**: SBSE 有机 ARR 增速 (每季度 Earnings Call 披露). ≥15% 维持深度关注, 10-15% 降到关注, <10% 降到中性.
- **ADBE**: GenStudio family ARR YoY + Firefly ending ARR QoQ (每季度 Earnings Call 披露). GenStudio ≥25% 且 Firefly QoQ ≥50% 维持关注临界, 任一跌破下沿 → 降到中性.

**不要再用 Rule of 40 / Fwd PE 单独给 4 家比大小**. 这个比较在 4 家属于同一资产类别时有效, 在分化后无意义.

### 3. 新估值语言 (以后该用什么方法定价)

**不要再用统一 Fwd PE 横向比较 4 家. 应该分两组**:

- **产品层 (ADBE/ADSK/PTC)**: 用 Reverse DCF 测隐含永续 g. 如果隐含 g < 0% 且基本面证据支持 g > 0% → alpha 存在. 关键参数: WACC 9% / 当前 FCF / 市值.
- **分发层 (INTU)**: 参照 VISA / MA / MCO 的 Fwd PE (28-31x). 如果 INTU 当前 Fwd PE < 20x → alpha 存在. 关键参数: VISA/MA 的当前倍数 / SBSE 核心业务增速 / 分发层溢价的市场接受度.

**不要用 Owner PE 做 ADSK 的点估**. ASC606 时序性让 Owner PE 161.9x 不可信. 用区间 + 条件评级代替.

### 4. 迁移问题 (看类似公司时该问什么)

下一次遇到 "一组 seat-based SaaS 需要横向比较" 的情景时, 必须问这两个问题:

- **问题 1**: **"这组公司的护城河来源是产品层、工作流嵌入层、制度层、还是分发层?"** 不同层级对 AI 的 beta 完全不同, 不能用统一模板. 如果一组公司都在产品层 (比如 Figma / Canva / ADBE), 可以横向比较; 如果一组公司跨多层 (比如 ADBE + INTU), 不能横向比较.
- **问题 2**: **"市场给这组公司的 Reverse DCF 隐含 g 有多大差距? 这个差距能不能用护城河差距解释?"** 如果差距 >2pp 但护城河强度相同, 是错误定价 (潜在 alpha). 如果差距 <1pp 但护城河差距显著, 也是错误定价 (潜在 alpha 反向).

**核心迁移原则**: 同质横向比较只在 "同护城河类型 + 同时间缓冲" 时有效. 一旦分化, 必须分组估值, 用不同的估值语言.
