...
| 部分 | 标题 | 章节 | 字符 |
|:----:|------|------|:----:|
| I | 公司定位与生态 | Ch01-Ch05 | 67.7K |
| II | 财务与价格含义 | Ch06-Ch10 | 70.3K |
| III | 竞争与AI深度 | Ch11-Ch15 | 80.4K |
| IV | 中性校准 | Ch16-Ch19 | 69.3K |
| V | 综合产出 | Ch20-Ch24 | 56.8K |
...
- **DRAM价格**: 现货价YoY峰值+171%(2025 Q3)，FY2025均值约+120%，暴涨中但即将见顶。[硬数据: MU v1.0 shared_context]
...
graph TB
    subgraph 6层雷达仪表板["🔍 AMD半导体6层雷达 (2026-02)"]
        direction TB
        L1["L1 存储价格<br/>DRAM +171% YoY(Q3峰值)<br/>HBM涨幅放缓<br/>⚠️ 接近峰值"]
...
[硬数据: MU shared_context] DRAM现货价YoY涨幅达+171%(2025 Q3峰值，FY2025均值约+120%)，但环比增速已在放缓。HBM3E溢价从峰值的3-4x DRAM开始收窄。[合理推断: 存储周期规律] 历史上DRAM价格拐点领先半导体整体周期6-9个月。当前涨幅已超2017-2018超级周期的+130% YoY峰值，[主观判断: 周期经验] 这意味着即使AI需求提供结构性支撑，存储价格的"增速"正在见顶。对AMD的含义: HBM4供给定价权在存储厂(Samsung/SK Hynix)手中，MI400系列的BOM成本存在上行风险。
...
[合理推断: 当前类比] 当前DRAM峰值+171% YoY(2025 Q3)已超过2018峰值(+130%)，WFE创新高，三寡头同步扩产——这些表面指标与2018年高度相似。如果严格遵循历史模式，存储价格应在6-9个月内见顶(即2026H2)，然后12-18个月内传导至半导体整体衰退(2027H2-2028H1)。
...
- 台海冲突概率市场(Polymarket, 截止2026-12-31): 市场存续中 [硬数据: MCP polymarket_events]
- 台海冲突短期概率市场(Polymarket, 截止2026-03-31): 市场存续中 [硬数据: MCP polymarket_events]
- "Will China blockade Taiwan by June 30": 市场存续中, 截止2026-06-30 [硬数据: MCP polymarket_events]
- 共识概率范围: 冲突风险~12%, 军事冲突~16% [硬数据: TSM shared_context Polymarket引用]
...
三个间接信号的综合指向: **AI CapEx周期的持续性和地缘风险是AMD定价的两大外生变量, 但预测市场共识倾向于"短期可控"**(台海冲突风险<15%, GPU价格区间对称而非单边下行)。[主观判断: 基于三组Polymarket信号的方向性综合, 非精确概率]
...
| 稀释股数 | 1,630M | [硬数据: FMP key-metrics FY2025加权平均] |
...
[硬数据: 计算方法 -- EV = 股价 x 1,630M股 - $1.1B净现金 + 调整; 隐含Year10 FCF = EV x 65% / 5.27; 隐含Revenue = FCF / margin; CAGR = (Rev_10 / $34.6B)^0.1 - 1]
...
| DRAM价格 | +171% YoY(2025 Q3峰值), 三寡头同步扩产 | 顶峰初段 | [硬数据: MU shared_context] |
...
[硬数据: Polymarket 2026-02-11, "Taiwan Strait crisis" markets]

[合理推断: 台海概率从2025年底~30%降至13%,反映地缘紧张暂时缓和。但对AMD而言,13%的台海风险仍然是存量定价因素。AMD 100%依赖TSMC(与NVDA相同),但NVDA在CoWoS分配中排第1(60%),AMD仅排第4(11%)。如果紧张局势升温但未达冲突阈值,TSMC可能在产能分配上进一步向NVDA倾斜,AMD受挤压。]

**AI CapEx持续性——间接定价信号:**

[硬数据: Goldman Sachs 2026-01] 2026年AI公司投资预计超$500B。Hyperscaler CapEx可能达$600B。[硬数据: Introl Blog 2026-01]

但Polymarket目前无直接"AI泡沫"或"AI CapEx放缓"赌盘,这使得AI持续性的市场定价信号缺失。[合理推断: 预测市场在"gradual slowdown"类事件上表现不佳——它们更擅长"binary events"(如冲突/不冲突)。AI CapEx放缓不太可能是一个突然事件,更可能是2-3个季度的渐进过程,因此预测市场在此维度的信号价值有限。]
...
Polymarket数据:
- 台海冲突概率市场(Polymarket, 截止2026-12-31) — 市场存在 [硬数据: MCP polymarket_events]
- "China x Taiwan military clash before 2027?" — 市场存在 [硬数据: MCP polymarket_events]
- TSM shared_context引用: 台海冲突概率12%，军事冲突16% [硬数据: TSM shared_context Polymarket]

台海冲突概率采用: **14%**(冲突风险12%与军事冲突16%的均值) [合理推断: 取两个市场均值]
...
# Part IV: 中性校准 (Ch16-Ch19)

> Part IV采用独立对抗视角，旨在校准前三部分可能存在的确认偏差。
...
> 行为金融学视角: 检测认知偏差、构建黑天鹅场景，并对核心假设进行极端压力测试
...
> 中性核查视角: 对报告引用的关键数据进行独立验证，检测准确性和内部矛盾
> **核查范围**: 18+数据点 | 财务+竞争+聪明钱+交叉验证
...
以下数据偏差已在相关章节中修正，修正后使用原始来源标注:

| # | 位置 | 原数据 | 修正后 | 状态 |
|---|------|--------|--------|:----:|
| 1 | Ch06 | DIO "152天" | DIO 165天(FY) / 140天(TTM)，已区分口径 | ✅ |
| 2 | Ch03 | DRAM "+171% YoY"无时点 | 峰值+171%(2025 Q3)，FY2025均值约+120% | ✅ |
| 3 | Ch07/Ch08 | 稀释股数不一致 | 统一为1,630M(FY2025加权平均) | ✅ |
| 4 | Ch11 | EPYC份额"41%" | 已澄清: 41%为营收份额，28%为出货量份额 | ✅ |
| 5 | Ch14 | AI GPU份额"7-10%" | 已澄清: 15-20%(DC revenue口径) | ✅ |
| 6 | Ch11 | N2良率"70-80%" | 已修正: 当前65-70%，80%为目标良率 | ✅ |
| 7 | Ch15 | Fisher清仓$23.4B | 已修正: 减持19.8%，金额未经独立验证 | ✅ |

### 18.5.2 口径澄清

| # | 位置 | 原表述 | 澄清 |
|---|------|--------|------|
| 1 | Ch06 | "DC营收$5.4B" | 来源为IR材料而非10-K，标注为[合理推断] |
| 2 | Ch10 | "SBC抵消率77.3%" | TTM口径，FY口径为80.3% |
| 3 | Ch15 | "分析师PT $257" | 33家平均值，范围$120-$380 |

### 18.5.3 内部矛盾桥梁

1. **SOTP $143 vs 概率加权$151.6**: SOTP为静态快照，概率加权包含增长轨迹和情景概率
2. **五引擎信号偏空(-0.425) vs PMSI 64.85(中性偏积极)**: 五引擎权重分散，PMSI地缘模块(40%权重)主导
3. **分析师看多($257) vs 内部人净卖出**: 经典信息不对称信号，增加估值不确定性
...
> 中性回检视角: 维度覆盖度审查、数据验证、结构性挑战识别、CQ置信度校准
...
| # | 位置 | 验证数据 | 结果 | 说明 |
|---|------|---------|:----:|------|
| **F01** | Ch01.3 | 四分部合计约$29.6B vs 总营收$34.6B的$5B差额 | ✅ | 正常分部间调整，非数据错误 |
| **F02** | Ch02.2.2 | AMD排TSMC第4优先级 | ✅ | 基本正确，2026年Broadcom可能与AMD并列 |
| **F03** | Ch03.1 | DRAM +171% YoY时点标注 | ✅ | 已修正: 明确标注为2025 Q3峰值，FY2025均值约+120% |
| **F04** | Ch05.1.1 | MI455X晶体管320B | ✅ | 多源交叉验证(Tom's Hardware/heise.de) |
| **F05** | Ch06.3 | OCF $7.71B / NI $4.34B = 1.78x | ✅ | FMP cashflow数据一致 |
| **F06** | Ch07/Ch08 | 稀释股数一致性 | ✅ | 已统一为1,630M(FY2025加权平均) |
| **F07** | Ch04.1 | Polymarket AMD市场0个 | ✅ | 2026-02-11确认无直接AMD市场 |

[硬数据: 验证来源为原文交叉对比 + MCP数据重新验证]

**验证总结**: 7项检查全部通过。F03(DRAM时点)和F06(稀释股数)为精度提升修正，已在相关章节中直接修正，不改变核心结论。
...
基于结构性挑战的评估，对8个核心问题的置信度进行校准:

| CQ | 校准方向 | 最终置信度 | 校准理由 | KS关联 |
|:--:|:-------:|:--------:|---------|:------:|
| **CQ1** | ↓ | 中-低(45%) | ASIC侵蚀使MI400竞争力评估偏乐观。Vera Rubin 2.6x机架性能差距+UALink 1.0不成熟度 | KS-PROD-1 |
| **CQ2** | ↓ | 中(55%) | Forward 20x的合理性依赖利润率假设验证(GPU/CPU mix风险) | — |
| **CQ3** | ↓ | 低-中(40%) | ROCm生态差距→定价权→利润率链条: CUDA 50:1开发者差距→AMD必须折价30-40% | KS-ECO-1 |
| **CQ4** | → | 中(50%) | ASIC威胁识别充分，三情景量化后维持中性 | — |
| **CQ5** | ↑ | 中-高(65%) | EPYC份额41%→50%路径论证充分: Venice 256核+Intel困境+Mercury Research趋势 | — |
| **CQ6** | → | 中(50%) | Q4后-17%需等待库存歧义解读(DIO趋势+Q1指引) | TS-TIMING-1 |
| **CQ7** | ↓ | 中-低(45%) | DC margin>30%假设未拆分GPU/CPU mix。GPU占比超50%时margin可能<28% | KS-MARGIN-1 |
| **CQ8** | ↓ | 中-低(45%) | 承重墙#1(利润率)+#3(ASIC)结构性挑战。$213隐含25-30% FCF margin可信度不足 | — |

[合理推断: CQ置信度校准基于7项结构性挑战与各CQ的映射关系]

**校准总结**: 1个↑上调(CQ5), 5个↓下调(CQ1/2/3/7/8), 2个→维持(CQ4/6)。最强CQ5(EPYC 65%)，最弱CQ3(ROCm 40%)。整体校准方向: "中性"，CQ加权置信度47.1%。

[主观判断: 中性校准的核心价值是识别结构性盲点(S01/S03/S04)并系统性对抗确认偏差，而非发现事实性错误]
...
1. **维度覆盖度**: 18项必选维度中13项完整(✅), 5项基本覆盖(⚠️), 0项缺失(❌)。覆盖度95%+。

2. **So What完整性**: 7个抽查样本中6个PASS。最强So What: Ch03.4库存信号(双解读+概率+验证窗口)和Ch07.4承重墙(量化倒塌影响-30~40%)。

3. **数据验证**: 7项事实性检查全部通过。F03(DRAM时点)和F06(稀释股数)已在相关章节中修正为精确表述，均为精度提升而非方向性错误。

4. **结构性挑战**: 7项中4项高严重性(S01/S03/S04/S05)。核心依赖: DC利润率GPU/CPU mix、ROCm生态折价、ASIC侵蚀量化、SOTP折价逻辑。

5. **CQ校准**: CQ5上调至65%(最强)，CQ3下调至40%(最弱)。CQ加权置信度47.1%。整体校准方向: "中性"。

### 结构性挑战处理

**已修正数据(已在相关章节中直接修正)**:
- F03: Ch03 DRAM涨幅已补充时点标注(2025 Q3峰值)
- F06: Ch07/Ch08稀释股数已统一为1,630M

**结构性挑战(保留于本章, 指导综合产出)**:
- S01: DC利润率GPU/CPU mix三情景建模 → Ch20条件估值
- S03: ROCm生态折价量化 → Ch21 KS设计
- S04: ASIC侵蚀传导模型 → Ch22 TS设计
- S05: SOTP折价敏感性 → Ch20估值校准

### 方法论反思

[主观判断: 中性校准的价值]

1. **识别结构性盲点**: S01/S03/S04/S05是分析"未充分量化的步骤" — 不是"错"，而是"不够深"。

2. **系统性校准**: 5个CQ下调是对可能的确认偏差的对抗。AMD投资论文高度依赖"利润率能否从20%→28%"和"ASIC侵蚀是否<30%"，这两个问题存在结构性不确定性。

3. **诚实面对不确定性**: 承认分析边界，"不知道就说不知道"。综合产出将在此基础上给出条件化的价格含义。
...
1. **Bear Case**: Ch16构建13个钢人论证，最严峻Bear场景指向$96-148区间(vs当前$213)。高概率风险: AI GPU利润率陷阱(65%)、ASIC侵蚀(70%)。

2. **认知偏差**: Ch17检测到3个系统性偏差——锚定偏差(估值围绕$213收敛)、确认偏差(ROCm数据选择性引用)、叙事偏差("追赶成功"的线性外推)。独立估值中枢$139.87。

3. **数据核查**: Ch18对18+数据点独立验证，多数准确。轻微口径偏差已在相关章节修正。数据诚信评级B+(72%)。

4. **维度回检**: Ch19确认18项维度覆盖度95%+。识别7项结构性挑战(4项高严重性)。5/8 CQ置信度下调，CQ加权置信度47.1%。

### CQ置信度最终校准

| CQ | 方向 | 最终 | 关键驱动 |
|:--:|:----:|:----:|---------|
| CQ1 | ↓ | 45% | ASIC侵蚀+MI400执行风险 |
| CQ2 | ↓ | 55% | 利润率假设验证不足 |
| CQ3 | ↓ | 40% | ROCm生态折价 |
| CQ4 | → | 50% | ASIC量化建模 |
| CQ5 | ↑ | 65% | EPYC论证充分 |
| CQ6 | → | 50% | 库存歧义需观察 |
| CQ7 | ↓ | 45% | GPU/CPU mix利润率 |
| CQ8 | ↓ | 45% | 承重墙#1/#3 |

---

# Part V: 综合产出 (Ch20-Ch24)
