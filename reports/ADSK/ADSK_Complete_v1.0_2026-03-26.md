...
**Autodesk是BIM设计软件的事实垄断者(Revit 63.5%份额),受20+国家BIM mandate结构性保护,有机增速12-13%可维持2-3年。$239的价格在赌三件事: (1)BIM mandate持续扩展而非转向IFC开放标准, (2)AI增强定价权(+3-5%/年)而非蚕食AutoCAD seat, (3)MFG/水务等新垂直能支撑增速不跌破10%——这三个业务变量决定了ADSK值$190(单引擎成熟)还是$300(多引擎增长重加速)。** [v3.0重写: 业务判断优先]
...
### 争议1: ADSK的增速能否维持12%+还是会滑向8%? [v3.0重写: 从SBC口径争议→业务增速争议]

**Bull方**: BIM mandate在20+国家提供结构性底线+Flex转型释放ARPU+MFG/水务新垂直扩展→有机增速12-13%可维持3-5年。Forward Non-GAAP PE 19x是SaaS板块最低——偏便宜。

**Bear方**: AECO增速已从+15%降至+11%→mandate扩展边际递减。AutoCAD seat可能被AI自动化蚕食(drafting自动化)。MFG落后PTC(Creo份额更高)。增速可能从12%滑向8%→PE不会扩张。

**我们的判断**: BIM mandate是高概率(80%)增长底线,但不是确定性底线。**增速能否在12%以上企稳取决于两个业务变量**: (a)新垂直(MFG/水务)能否贡献+2-3pp增速补充AECO减速, (b)AI定价权(+3-5%/年)能否抵消潜在的seat蚕食(-1-2%/年)。PE 19x在增速12%+时偏低, 在增速8%时合理。*(SBC口径差异(Standard PE 19x vs Owner PE 37x)是财务维度的分歧, 详见估值章节五。)*
...
## 三、最关键驱动因素 [v3.0重写: 业务变量优先]

**变量1: 有机增速能否在12%+企稳 (影响±30%估值)** [v3.0: 新增为#1]

AECO(50%收入)增速从+15%降至+11%。如果MFG/水务新垂直能贡献+2-3pp→总增速维持12-13%→PE可能从19x扩张至22-24x(+15-25%)。如果新垂直增速不达标+AECO继续减速→总增速滑至8-9%→PE维持或压缩至16-17x(-10%)。增速是PE扩张/压缩的**自变量**——SBC收敛是增速带来的**附属结果**。
...
**附注: SBC收敛** [v3.0: 从#1降至附注]

> SBC/Rev 10.9%在SaaS行业属于中低水平(CRWD 23%/DDOG 22%/NOW 15%)。回购η=0.11偏低, 但这是Starboard治理改革后可能改善的制度性问题。SBC收敛路径取决于增速和人员效率——如果变量1(增速)企稳→SBC/Rev预计自然从10.9%降至8-9%(FY2030)。SBC影响估值约±$25(10%), 远小于增速的±30%影响。详见Ch7 SBC分析。

---

## 四、最关键风险 / Kill Switch

### Kill Switch Top 5 [v3.0调整: 按业务影响排序]

| 优先级 | KS | 触发条件 | 当前→红线 | 如果触发 |
|:------:|----|---------|---------|---------|
| **#1** | **KS-1** | **有机增速<8% 连续2Q** | 13%→8% | 增长引擎失速→PE压缩 **(±30%)** |
| **#2** | **KS-7** | **Revit份额<55%** | 63.5%→55% | 护城河崩塌→-30% **(±30%)** |
| **#3** | **KS-9** | 新SEC/DOJ调查 | 无→任何 | 立即降至"审慎关注" **(±25%)** |
| #4 | KS-AI | AI蚕食AutoCAD seat>5%/年 | ~0%→>5% | 核心产品萎缩 **(±15%)** |
| #5 | KS-4 | SBC/Rev>13% 连续2Q | 10.9%→13% | 成本失控信号(但增速放缓后应收敛) **(±10%)** |
