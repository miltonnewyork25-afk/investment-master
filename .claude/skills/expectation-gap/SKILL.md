...
description: 预期差识别器 v2.1 — 三层信号(财务+行业趋势+聪明钱)+5种定价错误模式检测+强制知识前置加载
trigger: /expectation-gap
---

# 预期差识别器 v2.1 (Expectation Gap Detector)

> **v2.1升级**: 增加Step 0(强制知识前置加载)+5种已发现定价错误模式的结构化检测。
> **核心教训(v2.0→v2.1)**: WDAY v1.0分析时错过了行业趋势/聪明钱/范式转移/叙事适用性——因为skill没有"启动检查"步骤,每次从空白开始而非从积累的知识开始。
...
## ★ Step 0: 启动检查 (v2.1新增 — 解决"每次从空白开始"问题)

> **为什么加**: WDAY v1.0分析时,没有加载NOW的教训(行业趋势背离)、PTC的发现(叙事错误归因)、ADBE的数据(资本回报差异)。结果是在v1.0中错过了这些关键角度,只有在v2.0重跑时才发现。
> **解决方案**: 每次分析前强制加载知识库,确保已发现的模式不被遗忘。
...
**★ 3.4 五种定价错误模式强制检测 (v2.1新增)**

> **为什么加**: 每次分析都应该对照已发现的模式检查,而不是每次从零发现。

```
对照 knowledge/expectation_gap_patterns.yaml 中的5种PEP模式逐一检查:

□ PEP-001 叙事错误归因: Step 0.2已预检。如果narrative_applicability=misattributed,
  此处标注"叙事不适用"并量化: 如果去除叙事折扣,估值应该多少?

□ PEP-002 资本回报盲区: 计算SBC覆盖率+净缩股率,与同行业已完成卡片对比。
  如果本公司与同行估值相近但资本回报差异>100pp → 标注异常

□ PEP-003 速度溢价: 在R域的趋势不可逆性评估中,
  比较"实际渗透率"vs"市场隐含渗透率"→ 如果差距>20pp → 标注时间套利

□ PEP-004 转型阵痛当衰退: 当增速放缓但OPM扩张+FCF增长+回购加速时,
  检查: 是否匹配Adobe 2012模式? 如果匹配 → 标注"可能是转型不是衰退"

□ PEP-005 催化剂金字塔: 对于转型/turnaround公司,
  标注催化剂处于哪一层(技术/经济/商业/规模)→ 市场在哪一层定价?

输出: matched_patterns: [PEP-XXX, ...]  写入YAML卡片G域
```
...
| 检查项 | 要求 | 严重度 |
|--------|------|--------|
| **Step 0已执行** | **patterns.yaml已读取+叙事适用性已预检** | **BLOCK** |
| **Step 0启动简报已输出** | **用户看到行业/同行/模式匹配摘要** | **BLOCK(Tier 2+)** |
...
| **G域有5种模式检测结果** | **PEP-001~005逐一检查,标注matched/not_matched** | **BLOCK(Tier 2+)** |
...
| **分析完成后更新patterns.yaml** | **新卡片加入completed_cards+新模式加入(如有)** | **BLOCK** |
