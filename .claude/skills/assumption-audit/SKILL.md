...
### M1.4b 循环依赖检测 (v2.1, EVO-AAPL-004)

> **来源**: AAPL B1↔B8循环依赖发现 — iPhone增长(B1)依赖Services增长(B8)依赖iPhone安装基数(B1)。这不是两个独立信念，是一个共振系统。

**在M1.4一致性矩阵的基础上，强制执行以下检测**:

#### Step 1: 构建信念依赖图

对每个信念B_i，标注其**前提依赖**: B_i成立需要哪些其他信念先成立？
...
🔴 **检测到循环依赖**:
- 环路1: B1(iPhone增长) → B8(Services加速) → B1 (长度=2)
- 含义: 市场隐含的4.5x增长断层要求两个非独立信念同时成立
- 概率影响: P(B1∧B8) ≠ P(B1)×P(B8)，实际联合概率需按条件概率计算
...
对每个循环依赖对:
- **相关系数估计**: ρ(B_i, B_j) — 基于历史数据或逻辑推理
- **联合概率修正**: P(B_i∧B_j) = P(B_i) × P(B_j|B_i)，而非P(B_i) × P(B_j)
- **对估值影响**: 如果独立假设高估了联合成功概率，期望估值需向下修正多少？

#### Step 4: Mermaid可视化

```mermaid
graph LR
    B1[B1: 收入CAGR] <-->|🔴循环| B8[B8: Services加速]
    B1 -->|单向| B3[B3: 利润率]
    B1 -->|单向| B4[B4: 增长年限]
    B5[B5: 终值] -.->|独立| B1
...
**质量门控**: 循环依赖检测结果必须写入M1.4输出。如检测到环路，必须在M1.5翻转分析中按条件概率(而非独立概率)计算联合失败影响。
...
*假设审计框架 v2.1 — 三种角度拆解隐含假设: 市场信什么、管理层说什么、问题是什么*
*v2.1 (EVO-AAPL-004): M1.4b循环依赖检测 — 信念依赖图→拓扑排序→环路检测→条件概率修正*
