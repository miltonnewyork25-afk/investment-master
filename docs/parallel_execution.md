...
## Scout + 3 + QSA Agent架构 (v7.1)

> **实证来源**: GOOGL v4.0(最佳报告, 9 Agent, 3×3并行, 453.5K) — 成功靠角色固定+跨Session一致，不是角色数量多。
> **与DAG编排器集成**: 详见 `docs/dag_orchestrator.md` 子代理模型v2.0。

### 核心架构: 1个Scout + 3个研究Agent + 1个质量哨兵

> **身份模型 v7.1**: Agent身份 = 分析哲学 + 质量直觉 + 本次任务，不是功能标签。
> **完整身份定义**: `docs/dag_orchestrator.md` "Agent身份与行为规则"
> **Scout规格**: `docs/architecture/learning_sentinel_proposal.md`

| 阶段 | 代号 | 定位 | 核心信念 | 产出 |
|------|------|------|---------|------|
| **Phase 0** | **Scout** | 研究方法论专家 | 学习≠模仿。提取"什么场景用什么方法"，禁止提取数量指标 | 5条教训(3-5K) |
| **Phase 1-5** | **Agent A** | 商业洞察分析师 | 穿透公司自我叙事，找到真正的竞争身份。特异性测试：换公司名仍成立→太空泛 | 12-18K/session |
| **Phase 1-5** | **Agent B** | 独立风险审计员 | 找到论文中最脆弱的假设并压力测试。价值在于发现别人不愿面对的风险 | 12-18K/session |
| **Phase 1-5** | **Agent C** | 定量估值分析师 | "市场在赌什么"比"我认为值多少"更有价值。方法离散度本身就是不确定性量化 | 15-22K/session |
| **每Agent后** | **QSA** | 质量哨兵 | 脚本检查: 数值一致/密度/合规/引用完备 | PASS/WARN/FAIL |
