...
## 8. 动态研究框架（v2.0 — 适用于所有公司）

> **核心转变**: 从"固定模板填模块"到"问题驱动，按AI优势动态分配深度"。
> 每份报告的结构因公司而异，取决于哪些问题AI能产出真正有价值的洞察。

### 8.1 AI优势分类器

每个投资者关心的问题，先判断AI的分析优势：

| 分类 | AI能力 | 处理方式 | 示例 |
|------|--------|---------|------|
| **深挖区** | 强优势 | 投入主要篇幅，充分发挥 | 技术路线拆解、供应链映射、跨公司模式、**可能性空间映射**(发现系统)、Reverse DCF隐含假设检验 |
| **呈现区** | 有能力组织但无预测优势 | 呈现数据和思考框架，不下结论 | 财务趋势、竞争格局、行业周期位置 |
| **诚实区** | 弱或无优势 | 明确标注"AI无法可靠回答"，列出应追踪的信号 | 目标价、买卖时机、管理层是否诚实、政治走向 |

### 8.2 问题路由流程（Phase 0.5执行）

```
Step 1: 列出投资者最关心的5-10个问题
Step 2: 每个问题标注 [深挖] / [呈现] / [诚实]
Step 3: [深挖]问题 → 分配主要章节和Agent资源（每个≥8K字符）
Step 4: [呈现]问题 → 组织已有数据，呈现框架（每个2-5K字符）
Step 5: [诚实]问题 → 说明为什么无法回答+应追踪什么信号（每个≤1K字符）
```
...
| 问题 | TSLA | TSM | COST |
|------|------|-----|------|
| "技术路线前景？" | [深挖] FSD/电池/Optimus技术拆解 | [深挖] 制程节点路线图+良率分析 | [诚实] 零售技术非关键变量 |
| "估值是否合理？" | [诚实] 可能性太宽，无法给有意义的数字 | [呈现] 历史PE区间+隐含增速 | [呈现] 会员费DCF相对可靠 |
| "竞争格局？" | [深挖] 中国EV+FSD竞争者全景 | [深挖] Intel/三星追赶分析 | [深挖] 会员制零售护城河 |
| "管理层可信度？" | [诚实] Musk不可预测 | [呈现] 历史CAPEX兑现率 | [呈现] 管理层换届稳定性 |
| "周期位置？" | [诚实] 非周期公司 | [深挖] 半导体周期全维度 | [诚实] 消费必需品弱周期 |

> **关键**: 同一个问题（如"估值"）在COST是[呈现]（DCF相对可靠），在TSLA是[诚实]（给数字就是精确的错误）。框架是动态的，不是一套模板套所有公司。

### 8.4 AI深挖区的典型优势领域

以下领域AI有系统性优势，应优先分配研究资源：

1. **技术路线深度拆解** — 从底层技术到商业化路径，串联多层因果链
2. **供应链/产业链映射** — 上下游传导关系、瓶颈识别、交叉验证
3. **跨公司模式识别** — "LRCX周期领先TSM 2季度"这类人类难以系统追踪的模式
4. **可能性空间穷举** — 组合爆炸是AI优势，人类倾向线性外推
5. **矛盾信号检测** — 管理层法说会说法 vs 财务数据 vs 行业数据的不一致
6. **证据方向追踪** — 跨季度、跨来源的信号积累和方向判断
7. **反事实分析** — "什么应该发生但没有发生"的系统性检查

### 8.5 与Phase结构的集成

Phase结构保留（提供节奏和门控），但每个Phase的内容由问题路由决定：

| Phase | 固定任务 | 动态任务（由路由决定） |
|-------|---------|-------------------|
| **0** | 数据预取 + 可能性宽度评估 | — |
| **0.5** | **问题路由**（列出问题→标注深挖/呈现/诚实）| — |
| **1** | 公司定位（Part 1） | [深挖]问题的初步分析 |
| **2** | 财务数据组织 | [深挖]问题深入 + Reverse DCF(价格隐含假设) |
| **3** | 竞争分析 | [深挖]问题继续 + [呈现]问题整理 |
| **4** | 对抗审查（强制） | [诚实]问题明确标注 + 偏差检测 |
| **5** | 综合产出 | 按路由结果组装最终报告 |

### 8.6 报告产出的统一变化（适用于所有公司）

**不再产出**（对所有公司，不仅仅是宽概率公司）:
- 精确目标价（改为"价格隐含假设"或"条件估值范围"）
- 精确评分（如56.6/100，改为定性评估）
- 仓位建议（已在v2.0移除）
- 操作触发条件

**保留不变**: 数据质量要求、三层标注、Fast Gate门控

**新增**: 每份报告开头的"问题路由表"（投资者一眼看到哪些问题有深度分析、哪些被诚实回避）

---

## 9. 与v9.0扬长避短的协调

### 9.1 "不预测未来" vs "映射可能性" — 不矛盾

v9.0说"AI不擅长预测未来"，发现系统做"映射未来可能性空间"。这两者的边界是：

| | 预测（AI弱项，v9.0退出） | 映射（AI强项，发现系统深挖） |
|---|---|---|
| **本质** | 给出未来具体数字 | 穷举未来可能的结构形态 |
| **示例** | "TSLA 2027年收入$180B" | "TSLA可能变成5种形态的公司" |
| **精度** | 伪精度（给了数字但一定不准） | 结构性精确（覆盖可能性但不赌哪个） |
| **AI优势** | 无（和掷骰子差不多） | 强（组合爆炸是AI优于人脑的核心） |

**核心原则**: 映射"公司可能变成什么" = [深挖区]。预测"公司将变成什么" = [诚实区]。

### 9.2 发现系统是v9.0在宽概率公司的具体实践

发现系统(Parts 2-7)不是独立于v9.0的替代框架，而是v9.0的[深挖区]在宽概率公司上的展开：

| 发现系统组件 | v9.0分类 | 说明 |
|---|---|---|
| Part 2 可能性映射 | **[深挖]** | AI擅长组合穷举 — 人类倾向线性外推 |
| Part 3 开放问题 | **[深挖]** | AI擅长识别关键未知 — 问题本身是最有价值的输出 |
| Part 4 证据方向 | **[深挖]** | AI擅长跨季度信号积累 — 人类记忆有限 |
| Part 5 转折点 | **[深挖]** | AI擅长二阶变化检测 — 人类关注一阶 |
| Part 6 价格含义 | **[深挖]** | = Reverse DCF — v9.0 Phase 5核心 |
| Part 7 未知清单 | **[深挖]** | 诚实列出不知道什么 — 反映v9.0"诚实退出"精神 |

### 9.3 按可能性宽度分层适用

| 宽度 | 公司示例 | v9.0基础 | 发现系统叠加 | Phase 5输出 |
|------|---------|---------|------------|-----------|
| **宽(7-10)** | TSLA/PLTR | 全适用 | Parts 2-7全量 | 可能性映射+Reverse DCF+追踪信号 |
| **中(4-6)** | GOOGL/META | 全适用 | 可选Parts 3,5,6 | v9.0标准+可能性附录 |
| **窄(0-3)** | COST/TSM/LRCX | 全适用 | 不使用 | v9.0标准(Reverse DCF+KS+TS) |

**关键**: v9.0的扬长避短原则对所有公司生效。发现系统只是宽概率公司多出的[深挖]维度，不替代v9.0的基础结构（问题路由、定性评估、追踪信号、纠错回流等）。
...
### 10.2 密度门控 (EVO-INTC-004)

> **来源**: INTC 270K/PW=8得4.1=密度最高(1.52/100K), 但对比PLTR(389K)/APP(452K)发现系统报告, 可能性空间映射不充分。

**发现系统字符指引**:

| PW | 建议字符范围 | 密度目标(质量/100K) | 参考 |
|:--:|:----------:|:-----------------:|------|
| 7 | 350K-450K | 0.9-1.1 | APP 452K/4.1 |
| 8 | 300K-400K | 1.0-1.2 | PLTR 389K/3.6, INTC 270K/4.1 |
| 9 | 350K-450K | 0.85-1.0 | TSLA 415K/3.8 |

- **硬下限**: CG1 80%地板(350K×80%=280K)
- **建议下限**: 300K (PW≥7时, CG1-density WARN)
- **密度>体量原则仍有效**: 270K得4.1 > 452K得4.1 = 等质量时选密度。但发现系统特殊性: 需要足够空间映射可能性空间, 不宜过度压缩
