...
- 同一方向尝试**3次**仍无法形成有证据链的结论 → **停止+升级**(不要沉没成本谬误)
- 切换时优先选择研究问题清单中status=pending且priority最高的问题

### 并行Agent调度纪律 (借鉴Superpowers dispatching-parallel-agents)

**何时启动并行Agent**:
- ≥3个独立研究问题，问题间零共享状态
- 各问题可独立理解，不需要跨域context
- 各Agent不会修改相同的文件

**NEVER并行的场景**:
- 问题间有因果关联（解决一个可能改变另一个的结论）
- 需要整体理解才能判断（thesis形成/评级判断/估值综合）
- 探索性研究（问题还未定义清楚）

**每个并行Agent的任务描述必须**:
- **聚焦**: 单个问题域，不跨界
- **自包含**: 包含所有所需context，不假设Agent有对话历史
- **输出明确**: 定义预期交付物（数据表/分析段落/验证结论）
- **边界清晰**: "NEVER修改其他文件" / "NEVER输出超过500字"

**Never delegate understanding**: Agent执行数据收集和事实验证。thesis判断、评级决策、估值综合**NEVER**委托给子Agent——这些是需要完整context的综合判断。

**结果整合**:
1. 等待所有Agent完成（Don't peek中间输出）
2. 检查是否有重叠/矛盾的发现
3. 综合所有Agent结果形成统一判断（在主线程完成，不委托）
4. 更新Research State Board和问题清单
