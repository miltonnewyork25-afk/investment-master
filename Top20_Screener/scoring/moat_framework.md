...
> 本框架用于**自动批量筛选**(Python驱动, 几百只股票横向对比)。
> 单股深度分析(Tier 1-3)使用 `docs/company_quality_scoring.md` 的C维度(6类护城河, 人工评分/30分)。
> 两者互补: Screener筛出候选 → 深度框架逐只评估。
...
| 版本 | 日期 | 更新内容 |
|------|------|----------|
| v2.1 | 2026-03-06 | 新增与深度分析框架(company_quality_scoring.md)交叉引用 |
