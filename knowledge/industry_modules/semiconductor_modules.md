...
extensions:
  ...
  M11:
    name: "供应链交叉验证"
    definition: "通过上下游公司的业绩/指引/库存/订单交叉验证被调研公司的真实状态——避免管理层叙事和单点数据失真"
    questions:
      - Q1: "上游供应商(衬底/设备/原料)的出货量、定价和指引是否支持公司的收入增长故事?"
      - Q2: "下游客户/模块商的采购量、库存水平和终端订单是否与公司披露一致?"
      - Q3: "同一价值链中，谁在'吃掉'超额利润? 公司的份额是在上升还是被上下游挤压?"
    kpis:
      - KPI1: "上游验证比 — 关键供应商YoY增速 vs 公司采购增速 — 偏差<10%=一致, >20%=存疑"
      - KPI2: "下游验证比 — 主要客户/模块商收入增速 vs 公司对应分部增速 — 偏差<15%=一致"
      - KPI3: "价值链利润分布 — (公司OPM × 收入) vs (上游+下游OPM × 收入) — 判断谁在集中利润"
    consistency_check: |
      双向验证规则:
      (a) 如果公司说'需求强劲'但上游(如AXT InP衬底/应用材料设备)出货停滞→矛盾→存疑
      (b) 如果公司说'客户订单饱满'但下游模块商(如旭创/光迅)库存堆积→矛盾→存疑
      (c) 同行业可比的季度收入增速差距>20pp=数据异常→需要解释
      (d) 如果公司的GM改善但同一价值链其他环节GM恶化→可能是一次性转移/会计调整
    kill_switch:
      threshold: "连续2Q出现上游或下游的验证比偏差>30% 或 同行业可比公司收入方向相反"
      action: "管理层叙事可信度存疑 → 降低CQ置信度 → 增加共识解构权重"

  M12:
    name: "真实出货 vs 渠道库存"
    definition: "通过渠道库存、DSO、应收账款异常、分销商数据区分'真实终端需求'和'渠道填充'——避免周期顶部误判"
    questions:
      - Q1: "公司的收入增长是'sell-in'(卖给渠道)还是'sell-out'(卖给终端)?"
      - Q2: "主要客户的库存水平趋势? 有无超额订货迹象(关税避险/供应紧张恐慌)?"
      - Q3: "如果明天停止新订单，当前渠道库存能支撑多少个月的终端销售?"
    kpis:
      - KPI1: "客户DIO趋势(天) — 主要下游客户的库存天数 — 同比>+20天=积压信号"
      - KPI2: "公司DSO趋势(天) — 应收账款周转 — 同比>+15天=收款放缓/渠道压货"
      - KPI3: "渠道/终端比率 — sell-in/sell-out比 — >1.2=渠道在吸收超额库存"
    consistency_check: "公司收入增速>>下游客户终端销售增速+下游DIO上升=渠道填充; 公司DSO上升>15天+收入加速=应收账款质量恶化"
    kill_switch:
      threshold: "下游客户合计DIO连续2Q上升>20天 或 公司DSO上升>20天且收入加速 或 分销商库存预警"
      action: "sell-in与sell-out脱钩 → 下调未来季度收入假设 → 增加周期顶部概率"
