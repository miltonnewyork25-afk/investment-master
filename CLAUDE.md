...
**当前版本**: v19.2 (2026-03-18) | **健康监控**: `bash tests/framework_health_check.sh` + `bash scripts/quality_health_check.sh`
**v19.2新增**: **铁律N证据链完整性** — 质量下降的核心根因是因果推理密度从9.28/万字(KLAC)降到2.94(ADBE), 断言替代了推理。每个核心论点必须: ≥1硬数据+≥1因果推理+≥1反面考量。因果密度<3.0/万字=证据链断裂
**v19.1修正**: 恢复≥150K×系数广度下限 + DM≥0.5密度门控双保险。广度防遗漏, 密度防注水
**v19.0变化**: 铁律K估值统一性(MCO教训) | 铁律L DM硬门控(ADBE教训) | 铁律M反膨胀 | 质量健康检查自动触发
