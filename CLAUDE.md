...
| **Tier 3** | "深度/全面" | 多会话 | 密度优先(见下) | `docs/deep_dive_protocol.md` |

**Tier 3质量目标 (v19.0: 密度>体量)**:
- **字符数不设最低线** — KLAC 248K得4.5分 > ADBE 264K得~3.0分。字符数从不是质量代理变量
- **密度目标**: DM锚点≥0.8/千字 + 独立洞见≥3个/万字 + 章节间零重复
- **参考范围**: 150K-350K (自然长度, 不刻意追求也不刻意压缩)
- **禁止凑数**: 宁可180K高密度, 不要300K+重复填充。每章必须有独立论点, 不能重述前章结论
...
**禁止**: 跳过tier3_launch.sh直接开始Phase 0 | 忽略launch_brief中的参考范围 | 用字符数衡量完成度(v19.0: 密度>体量)
...
**当前版本**: v19.0 (2026-03-18) | **健康监控**: `bash tests/framework_health_check.sh`
**v19.0变化 (质量修复)**: 字符最低目标取消→密度优先 | 铁律K估值统一性(MCO教训) | 铁律L DM密度≥0.8硬门控(ADBE教训) | 铁律M反膨胀纪律(单章≤15%+章节独立性+按需加载skill)。**核心转向: 分析密度>报告长度, 框架为分析服务而非反过来**
