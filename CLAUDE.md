...
**v11.0的失败**：定义了"什么是深度"，但没有强制"如何产生深度"。
结果：知道要深度，但实际执行时仍然数据罗列。

**v12.0的解决**：深度思考协议（强制前置）
- 在调用API之前，必须完成第一性原理思考
- 在写报告之前，必须完成护城河、产品矩阵、竞争格局分析
- 在引用分析师之前，必须解构他们的框架并找到盲区

**强制执行文件**：`skills/core/deep_thinking_protocol_v1.md`
