...
> **口令**: 先别急着证明它是好公司，而要先看股价在买什么；再判断这笔被买下来的未来是真是假、值不值、会不会被提前买太满。

**四个最高目标** — 一份报告必须回答这四件事，否则不够好:
1. **识别当前股价到底在买什么**
2. **找到最能解释公司与股价的关键变量**
3. **找到市场最可能错看的那一层**
4. **形成可证伪、可跟踪、可更新的投资判断**
...
1. **深度优先** — 宁可少写3个低解释力角度, 也不要浅写1个高解释力角度
2. **主线不砍** — token不足时先砍背景, 不砍主线和承重墙
3. **禁止硬写** — 没有证据链时宁可留空白, 不要用流畅文字掩盖无依据判断
4. **结论分级** — 所有判断标记: 硬结论(有硬数据) / 弱结论(合理推断, 标注证伪条件) / 猜测(不进入主结论)
5. **主线优先于模板** — 模板是底盘, 主线问题才是驾驶方向
6. **逼近决定性矛盾** — 如果只能保留3个问题应该是哪3个? 1个承重墙是哪个?
7. **背景服务结论** — 不能服务于解释股价/赚钱逻辑/市场分歧/结论证伪的段落, 默认压缩
8. **篇幅跟随解释力** — 不同公司重要部分不同, 动态分配, 不机械均匀
9. **诚实的不完整 > 虚假的完整** — 允许暂时不完整, 不允许把不确定性用漂亮话遮住
10. **反自欺检查** — 提交前检查: 有没有稀释主线/跳过承重墙/弱推断写成硬事实/为填模板补无证据结论
11. **研究服务决策, 不服务表演** — 每段至少服务: 市场在买什么/怎么创造价值/错看哪里/赔率好不好/该跟踪什么
...
> **最终口令**: 真正的好投资, 不是找到卓越公司, 而是找到"卓越公司仍有安全边际"的时刻。