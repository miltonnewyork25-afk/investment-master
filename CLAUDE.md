...
### -0.3. 数据源强制调用 (Data Source Registry) [v8.0新增]

**⚠️ 在路由完成后、正式分析前必须执行数据获取**
...
路由完成
    ...
    ▼
┌─────────────────────────────────────────────┐
│  加载 data_source_registry_v1.yaml          │
│  读取所有 API 配置和调用示例                 │
└─────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────┐
│  执行 P0 必选调用 (6个)                      │
│  ├─ 100baggers: generate_summary ⭐必须      │
│  ├─ FMP: profile                            │
│  ├─ FMP: quote                              │
│  ├─ FMP: income_statement_ttm               │
│  ├─ FMP: balance_sheet_ttm                  │
│  └─ FMP: cash_flow_ttm                      │
└─────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────┐
│  执行 P1 建议调用                            │
│  ├─ 100baggers: compare_companies (对比时)  │
│  ├─ 100baggers: get_strategy_report         │
│  ├─ FMP: financial_scores                   │
...
```bash
# ⭐ 100baggers 财报摘要 (必须调用，提供独特的7维度评分)
...
| 数据源 | 端点 | 状态 | 数据点 |
|--------|------|------|--------|
| 100baggers | generate_summary | ✅ | 7维度评分, 杜邦分析 |
| FMP | profile | ✅ | Beta=1.88, 市值=$1.4T |
| FMP | quote | ✅ | 价格=$420, P/E=180 |
| FMP | income_statement_ttm | ✅ | 收入=$96B, 净利=$5B |
| FMP | balance_sheet_ttm | ✅ | 总资产=$134B |
| FMP | cash_flow_ttm | ✅ | OCF=$16B, FCF=$7B |

---
```

**为什么 100baggers 是必须的**：
- FMP 不提供的独特数据：7维度雷达图评分、领先指标触发、杜邦三级拆解
- 这些是分析质量的关键输入，缺失会导致分析不完整
...
### P0 硬性前置条件 (3项) [v7.0新增] - 未通过则禁止继续
0a. ✓ **前置学习完成**: 已搜索并学习5-10篇顶级分析师报告
0b. ✓ **分析师学习笔记已输出**: 包含观点全景表+分歧表+3个超越方向
0c. ✓ **超越方向已明确**: 不是泛泛而谈，是具体的方法/数据/视角超越

⚠️ **硬规则**: 如果0a-0c任一项未完成，禁止进入正式分析阶段
